...
function buildInstrumentMappings(){
 ...
 c.items.forEach(x=>{const prev=liveFoundation.mappings[x.key]||{};liveFoundation.mappings[x.key]={...prev,ticker:prev.ticker||x.ticker,isin:prev.isin||x.isin,currency:prev.currency||x.currency,exchange:prev.exchange||x.exchange,assetType:prev.assetType||x.assetType,name:x.name,account:x.account,updatedAt:new Date().toISOString(),source:prev.source||'masterdata'}});
...
function quoteFor(h){const quotes=liveState().quotes||[],key=h?._key||identity(h),m=liveFoundation.mappings[key]||{},norm=x=>String(x||'').toUpperCase().replace(/[\s:.-]/g,'');const symbols=[m.permanentRoute?.ticker,m.ticker,...Object.values(m.providerSymbols||{}),text(h,['symbol','ticker','providerSymbol','isin'],'')].map(norm).filter(Boolean);const q=quotes.find(x=>x.instrumentKey===key)||quotes.find(x=>symbols.includes(norm(x.symbol||x.ticker||x.providerSymbol||x.isin)));if(q)return nval(q,['price','latestPrice','last','value','close']);return nval(h,['latestPrice','price','currentPrice','lastPrice','kurs','quote'])}
...
function holdingValue(h){const q=quantity(h),p=quoteFor(h),fx=fxFor(h);if(q&&p&&fx)return {value:q*p*fx,method:'Livekurs × antal',live:true};const direct=explicitValue(h);if(direct)return {value:direct,method:'Senast känt värde',live:false};const c=costValue(h);if(c)return {value:c,method:'Anskaffningsvärde',live:false};return {value:0,method:'Saknar värde',live:false}}
...
function canonicalHoldings(){return rawHoldings().map((base,i)=>{const h={...base,...(admin.overrides[base._key]||{})};const r=holdingValue(h);return {...h,_i:i,_value:r.value,_method:r.method,_name:holdingName(h),_account:accountName(h),_type:assetType(h),_qty:quantity(h),_gav:gav(h),_fx:fxFor(h),_quote:quoteFor(h),_liveValue:Boolean(r.live)}})}
...
function liveValuationStats(){const hs=canonicalHoldings(),live=hs.filter(h=>h._liveValue).length,daySEK=hs.reduce((sum,h)=>{const q=liveQuoteFor(h),fx=fxFor(h);return sum+(q&&h._qty?num(q.change)*h._qty*fx:0)},0),updated=liveState().lastSync||liveState().updatedAt||'';return {live,fallback:hs.length-live,total:hs.length,coverage:hs.length?live/hs.length*100:0,daySEK,ledger:declaredTotal(),calculated:calculatedTotal(),updated:updated?new Date(updated).toLocaleString('sv-SE'):'Inte synkad'}}
...
function portfolio(){const all=canonicalHoldings(),t=total(),q=query.toLowerCase(),types=[...new Set(all.map(h=>h._type))].sort((a,b)=>a.localeCompare(b,'sv')),accounts=[...new Set(all.map(h=>h._account))].sort((a,b)=>a.localeCompare(b,'sv'));let hs=all.filter(h=>(typeFilter==='all'||h._type===typeFilter)&&(accountFilter==='all'||h._account===accountFilter)&&(!q||`${h._name} ${h._account} ${h._type}`.toLowerCase().includes(q)));const sorts={value_desc:(a,b)=>b._value-a._value,value_asc:(a,b)=>a._value-b._value,name_asc:(a,b)=>a._name.localeCompare(b._name,'sv'),name_desc:(a,b)=>b._name.localeCompare(a._name,'sv')};hs.sort(sorts[sortMode]);const d=diagnostics(),accountMap=new Map();all.forEach(h=>accountMap.set(h._account,(accountMap.get(h._account)||0)+h._value));let content='';
 if(portfolioTab==='overview'){const vs=liveValuationStats();content=`<section class="hero"><div class="eyebrow">Live Portfolio Valuation 11.14.1</div><h2>${kr(t)}</h2><p>${all.length} innehav · ${vs.live} livevärderade · ${vs.fallback} med senast känt värde.</p><div class="liveHeroStats"><div><span>Live-täckning</span><b>${pct(vs.coverage)}</b></div><div><span>Dagens förändring</span><b class="${vs.daySEK>=0?'positive':'negative'}">${vs.daySEK>=0?'+':''}${kr(vs.daySEK)}</b></div><div><span>Ledger-bas</span><b>${kr(vs.ledger)}</b></div><div><span>Senast värderad</span><b>${esc(vs.updated)}</b></div></div></section><div class="grid"><button class="card actionCard" data-tabgo="transactions"><b>Registrera köp/sälj</b><span>Uppdatera antal och viktat GAV.</span></button><button class="card actionCard" data-tabgo="admin"><b>Administrera portfölj</b><span>Ändra GAV, antal, nya innehav och kredit.</span></button></div>`}
...
function liveQuoteFor(h){const state=liveState(),mapping=liveFoundation.mappings[h._key]||{},norm=x=>String(x||'').toUpperCase().replace(/[\s:.-]/g,''),symbols=[mapping.permanentRoute?.ticker,mapping.ticker,...Object.values(mapping.providerSymbols||{}),text(h,['symbol','ticker','providerSymbol','isin'],'')].map(norm).filter(Boolean);return (state.quotes||[]).find(q=>q.instrumentKey===h._key)||(state.quotes||[]).find(q=>symbols.includes(norm(q.symbol||q.ticker||q.providerSymbol||q.isin)))||null}
...
function globalResolverState(){return merge({version:'11.14.1',status:'idle',lastRun:'',scanned:0,resolved:0,review:0,unresolved:0,errors:0,suggestions:[],routes:{},audit:[]},parse(safeGet(GLOBAL_RESOLVER_KEY))||{})}
...
function resolverTargets({all=false}={}){return instrumentMappingCoverage().items.filter(item=>{const h=canonicalHoldings().find(x=>x._key===item.key)||item,m=liveFoundation.mappings[item.key]||{},cls=classifyInstrument(h);if(cls.code==='fund')return false;if(all)return true;return validIsin(m.isin||item.isin)||!m.verified||num(m.identityConfidence||m.confidence)<92||!m.ticker||!m.exchange})}
...
async function runGlobalResolver({all=true,automatic=false}={}){const btn=document.getElementById('runGlobalResolver');if(btn){btn.disabled=true;btn.textContent='Verifierar 0 instrument…'}const targets=resolverTargets({all}),suggestions=[];let errors=0,processed=0;const previous=globalResolverState();persistGlobalResolver({...previous,version:'11.14.1',status:'running',lastRun:new Date().toISOString(),scanned:0,totalTargets:targets.length,resolved:0,review:0,unresolved:0,errors:0,suggestions:[]});for(let i=0;i<targets.length;i+=3){const batch=await Promise.all(targets.slice(i,i+3).map(resolveOneInstrument));suggestions.push(...batch);errors+=batch.reduce((n,x)=>n+x.errors.length,0);processed+=batch.length;const resolved=suggestions.filter(x=>x.autoSafe).length,review=suggestions.filter(x=>x.best&&!x.autoSafe).length,unresolved=suggestions.filter(x=>!x.best).length;persistGlobalResolver({...globalResolverState(),version:'11.14.1',status:'running',scanned:processed,totalTargets:targets.length,resolved,review,unresolved,errors,suggestions});if(btn)btn.textContent=`Verifierar ${processed}/${targets.length}…`;await new Promise(r=>setTimeout(r,40))}const resolved=suggestions.filter(x=>x.autoSafe).length,review=suggestions.filter(x=>x.best&&!x.autoSafe).length,unresolved=suggestions.filter(x=>!x.best).length;const state={...previous,version:'11.14.1',status:errors&&resolved+review===0?'error':unresolved?'partial':'success',lastRun:new Date().toISOString(),scanned:targets.length,totalTargets:targets.length,resolved,review,unresolved,errors,suggestions,routes:previous.routes||{},audit:[{date:new Date().toISOString(),scanned:targets.length,resolved,review,unresolved,errors},...(previous.audit||[])].slice(0,100)};persistGlobalResolver(state);const applied=applySafeResolverSuggestions();liveEvent(state.status==='success'?'success':'warning','Global Identity Resolver 11.14.1',`${resolved} säkra · ${applied} sparade · ${review} för granskning · ${unresolved} utan träff`,{errors});render();if(applied||resolved){try{await syncLiveData();runAutonomousPortfolioAnalysis({reason:'resolver-11.14.1'});render()}catch(e){liveEvent('warning','Resolver Execution Engine',`Identiteter sparades men automatisk livesynk blev delvis misslyckad: ${String(e?.message||e)}`)}}if(!automatic)openGlobalResolverReview();return {...state,applied}}
function applyResolverSuggestion(key,index=0){const state=globalResolverState(),row=(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString()};liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.14.1'],source:'global-resolver-approved-11.14.1',resolverVersion:'11.14.1',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();state.routes=state.routes||{};state.routes[key]=route;row.applied=true;row.appliedAt=new Date().toISOString();row.appliedCandidate=index;persistGlobalResolver(state);liveEvent('success','Global Identity Resolver 11.14',`${row.name} kopplades permanent till ${candidate.ticker||candidate.isin}`,route);return true}
...
function globalResolverPanel(){const st=globalResolverState(),coverage=instrumentMappingCoverage(),remaining=resolverTargets().length,saved=Object.keys(st.routes||{}).length;return `<section class="section globalResolverPanel"><div class="sectionHead"><div><div class="eyebrow">Global Identity Resolver 11.14.1</div><h2>Rätt instrument. Rätt börs. Permanent rutt.</h2></div><span class="healthScore">${coverage.complete}/${coverage.total}</span></div><p>Motorn verifierar identitet med ISIN, alternativa tickerformat, börsvarianter och flera providers. Den bästa fungerande rutten sparas permanent och återanvänds vid framtida live-synk.</p><div class="grid"><div class="card metric"><span>Återstår att lösa</span><b>${remaining}</b></div><div class="card metric"><span>Permanent sparade</span><b>${saved}</b></div><div class="card metric"><span>För granskning</span><b>${st.review}</b></div><div class="card metric"><span>Konflikter</span><b>${(st.suggestions||[]).filter(x=>x.conflict).length}</b></div></div><div class="foundationActions"><button id="runGlobalResolver" class="primary" type="button">Kör Global Identity Resolver 11.14.1</button><button id="reviewGlobalResolver" class="secondary" type="button" ${(st.suggestions||[]).length?'':'disabled'}>Granska och godkänn</button></div><div class="resolverSources"><span>Global Registry</span><span>OpenFIGI</span><span>Twelve Data</span><span>Alpha Vantage</span><span>Finnhub</span><span>ISIN + börsvarianter</span></div><p class="disclaimer">Antal, GAV, kredit, transaktioner och Portfolio Ledger ändras aldrig. Marknadsvärdet beräknas dynamiskt från livekurs/NAV × antal × FX.</p></section>`}
