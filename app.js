...
function emptyPermanentRegistry(){return {version:'11.15.29',updatedAt:'',storage:'IndexedDB',routes:{},audit:[]}}
function normalizePermanentRegistry(raw){
 ...
 registry.version='11.15.29';registry.storage='IndexedDB';
 return registry
}
...
async function hydrateResolverPersistentState(){
 try{
  ...
  if(restoredRun&&/^11\.15\.2[4-8]$/.test(String(restoredRun.version||''))){
...
async function savePermanentIdentityRoute(key,route,meta={}){
 ...
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.29',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
...
async function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.29',source:'permanent-identity-registry-11.15.29',updatedAt:new Date().toISOString()};restored++}
 }
 ...
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.29',`${restored} rutter återställda · ${migrated} migrerade till IndexedDB`,{})}
...
function resolverDiagnosticState(){
 ...
 const meta=merge({version:'11.15.29',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false,storage:resolverTraceStorage},raw);
 meta.lines=resolverDiagnosticLines();meta.storage=resolverTraceStorage;return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.29',startedAt:x?.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x?.heartbeat),currentInstrument:x?.currentInstrument||'',currentStep:x?.currentStep||'',expanded:!!x?.expanded,storage:'IndexedDB'};
...
function resolverRunDefaults(){return {version:'11.15.29',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,fundPending:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],providerCooldowns:{},message:''}}
function resolverRunState(){
 ...
 if(run.version!==VERSION){
  if(raw&&/^11\.15\.2[4-8]$/.test(String(raw.version||''))){run={...run,version:VERSION,message:run.message||'Körstatus migrerad till IndexedDB Unified State Recovery.'}}
...
const RESOLVER_PROVIDER_COOLDOWN_KEY='mastarklass_os_11_15_29_provider_cooldowns';
...
function registerProviderSuccess(name){const all={...resolverProviderCooldowns()};if(all[name]){delete all[name];persistResolverProviderCooldowns(all)}}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.29',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,fundPending:run.fundPending||0,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
async function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){
 ...
 if(!await savePermanentIdentityRoute(key,route,{source:'global-resolver-approved-11.15.29'})){if(!silent)alert('Permanent lagring i IndexedDB misslyckades. Instrumentet ligger kvar och ingen portföljdata har ändrats. Kopiera Resolver Chain Trace för exakt fel.');return false}
 ...
 liveEvent('success','Permanent Identity Registry 11.15.29',`${row.name} sparad permanent som ${route.ticker||route.isin} · ${route.exchange||'okänd börs'} · ${route.currency}`,verified);
