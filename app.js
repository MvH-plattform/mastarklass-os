...
const MANUAL_IDENTITY_KEY='mastarklass_os_11_15_33_manual_identity';
...
function manualIdentityState(){const raw=parse(safeGet(MANUAL_IDENTITY_KEY));return raw&&typeof raw==='object'?merge({version:'11.15.33',updatedAt:'',byKey:{},byFingerprint:{}},raw):{version:'11.15.33',updatedAt:'',byKey:{},byFingerprint:{}}}
...
function emptyPermanentRegistry(){return {version:'11.15.33',updatedAt:'',storage:'IndexedDB',routes:{},audit:[]}}
function normalizePermanentRegistry(raw){
 ...
 registry.version='11.15.33';registry.storage='IndexedDB';
 return registry
}
...
async function savePermanentIdentityRoute(key,route,meta={}){
 ...
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.33',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
...
async function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.33',source:'permanent-identity-registry-11.15.33',updatedAt:new Date().toISOString()};restored++}
 }
 ...
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.33',`${restored} rutter återställda · ${migrated} migrerade till IndexedDB`,{})}
...
function resolverDiagnosticState(){
 ...
 const meta=merge({version:'11.15.33',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false,storage:resolverTraceStorage},raw);
 meta.lines=resolverDiagnosticLines();meta.storage=resolverTraceStorage;return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.33',startedAt:x?.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x?.heartbeat),currentInstrument:x?.currentInstrument||'',currentStep:x?.currentStep||'',expanded:!!x?.expanded,storage:'IndexedDB'};
...
function resolverRunDefaults(){return {version:'11.15.33',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,fundPending:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],providerCooldowns:{},message:''}}
...
function persistResolverRun(x){
 ...
 // 11.15.33: cooldowns are monotonic state. A stale loop snapshot must never erase
 ...
 for(let i=run.cursor;i<batchEnd;i++){
  ...
  try{
   ...
   if(routeIsUsable(permanentRouteFor(item.key))){resolverDiag('Permanent träff','Instrument hoppas över','success');run=resolverRunState();run.cursor=i+1;run.processed++;run.skipped++;}
    ...
    const reused=findPermanentRouteByIsin(item.isin);resolverDiag('Permanent ISIN-återanvändning',`${item.isin} från ${reused.key}`,'success');await savePermanentIdentityRoute(item.key,{...reused.route,source:'isin-deduplicated-11.15.33'},{source:'isin-deduplicated-11.15.33'});run=resolverRunState();run.cursor=i+1;run.processed++;run.skipped++;run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length;
   }
   else{
...
async function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){
 ...
 if(!await savePermanentIdentityRoute(key,route,{source:'global-resolver-approved-11.15.33'})){if(!silent)alert('Permanent lagring i IndexedDB misslyckades. Instrumentet ligger kvar och ingen portföljdata har ändrats. Kopiera Resolver Chain Trace för exakt fel.');return false}
 ...
 liveEvent('success','Permanent Identity Registry 11.15.33',`${row.name} sparad permanent som ${route.ticker||route.isin} · ${route.exchange||'okänd börs'} · ${route.currency}`,verified);
...
function resolverReviewHtml(){
 ...
 return `<div class="eyebrow">Global Identity Resolver 11.15.33</div><h2>ISIN-first Candidate Inspector</h2><p class="disclaimer">ISIN är primär identitetsnyckel. Ticker, börs, valuta och instrumenttyp används som sekundära kontroller. Kandidater utan ISIN kan granskas men räknas inte som verifierade identiteter.</p><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`
}
