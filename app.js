...
window.mkRunPortfolioIdentityBridge=event=>{event?.preventDefault?.();const r=bridgePortfolioToIdentity();resolverDiag('Portfolio Identity Bridge 11.15.40',`${r.withIsin}/${r.total} med ISIN · ${r.withoutIsin} saknar · ${r.conflicts.length} konflikter`,r.conflicts.length?'warning':'success');render();return false};
...
function renderIdentityConflictDetails(){
 ...
 openModal(`<div class="sectionHead"><div><h2>Unified Identity Engine 11.15.39</h2><p>${report.withIsin}/${report.total} med ISIN · ${report.withoutIsin} saknar · ${conflicts.length} konflikter</p></div></div><h3>ISIN-konflikter</h3>${conflictHtml}<h3 class="topGap">Saknar ISIN</h3><div class="missingIsinList">${missingHtml||'<div class="notice okbox">Alla innehav har ISIN.</div>'}</div>`);
 return report
}
window.mkOpenIdentityConflictResolver=event=>{event?.preventDefault?.();renderIdentityConflictDetails();return false};
...
function resolverReviewHtml(){
 ...
 return `<div class="eyebrow">Global Identity Resolver 11.15.40</div><h2>ISIN-first Candidate Inspector</h2><p class="disclaimer">ISIN är primär identitetsnyckel. Ticker, börs, valuta och instrumenttyp används som sekundära kontroller. Kandidater utan ISIN kan granskas men räknas inte som verifierade identiteter.</p><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`
}
