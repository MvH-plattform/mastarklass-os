...
function identityValidationState(){return merge({version:'11.15.35',lastRun:'',status:'not-run',total:0,withIsin:0,withoutIsin:0,duplicateConflicts:0,unsafeVerifiedRoutes:0,persistenceOk:false,issues:[]},parse(safeGet(IDENTITY_VALIDATION_KEY))||{})}
function persistIdentityValidation(x){x.version='11.15.35';x.lastRun=new Date().toISOString();safeSet(IDENTITY_VALIDATION_KEY,JSON.stringify(x));return x}
function runIdentityIntegrityValidation({silent=false}={}){
 ...
 const probeKey='__mk_identity_validation_probe__',probe={version:'11.15.35',time:new Date().toISOString()};
 let persistenceOk=false;
 try{safeSet(probeKey,JSON.stringify(probe));persistenceOk=parse(safeGet(probeKey))?.version==='11.15.35';localStorage.removeItem(probeKey)}catch{}
 ...
 resolverDiag('Identity Validation 11.15.35',`${withIsin}/${hs.length} med ISIN · ${duplicateConflicts} dubblettkonflikter · ${unsafeVerifiedRoutes} osäkra verifieringar`,result.status==='success'?'success':'warning');
...
function resolverReviewHtml(){
 ...
 return `<div class="eyebrow">Global Identity Resolver 11.15.35</div><h2>ISIN-first Candidate Inspector</h2><p class="disclaimer">ISIN är primär identitetsnyckel. Ticker, börs, valuta och instrumenttyp används som sekundära kontroller. Kandidater utan ISIN kan granskas men räknas inte som verifierade identiteter.</p><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`
}
