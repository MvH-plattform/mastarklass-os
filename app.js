...
function marketRoutingState(){return merge({version:'11.15.47',lastRun:'',status:'not-run',total:0,routed:0,quoteReady:0,navReady:0,fxReady:0,needsPrice:0,needsNav:0,needsFx:0,routes:{},missingPrice:[],missingNav:[],missingFx:[],blockedQuoteRoutes:[]},parse(safeGet(MARKET_ROUTING_KEY))||{})}
function persistMarketRouting(x){x.version='11.15.47';x.lastRun=new Date().toISOString();safeSet(MARKET_ROUTING_KEY,JSON.stringify(x));return x}
...
function buildMarketRoutingBridge({log=true}={}){
 ...
 if(log)resolverDiag('ISIN Market Routing 11.15.47',`${routed}/${hs.length} rutter · ${quoteReady} kurser · ${navReady} NAV · ${needsFx} saknar FX · ${blockedQuoteRoutes.length} NAV-rutter skyddade`,routed===hs.length?'success':'warning');
 return result
}
...
function buildSmartRouterState({log=true}={}){
 ...
 if(log)resolverDiag('Smart Provider Router 11.15.47',`${items.length} instrument · ${quoteRoutes} kursrutter · ${navRoutes} NAV-rutter · tillit ${result.averageConfidence}%`,lowConfidence?'warning':'success');
 return result
}
...
function renderSmartRouterReport(){const s=buildSmartRouterState({log:false});const rows=s.items.slice().sort((a,b)=>a.confidence-b.confidence).map(x=>`<div class="smartRouteRow ${x.confidenceLevel}"><div><b>${escapeHtml(x.name)}</b><small>${escapeHtml(x.account)} · ${escapeHtml(x.pricing.toUpperCase())} · ${escapeHtml(x.providerAction)}</small><small>${escapeHtml((x.reasons||[]).join(' · ')||'Saknar underlag')}</small></div><strong>${x.confidence}%</strong></div>`).join('');openModal(`<div class="sectionHead"><div><h2>Smart Provider Router 11.15.47</h2><p>${s.total} instrument · ${s.quoteRoutes} kursrutter · ${s.navRoutes} NAV-rutter</p></div></div>${smartRouterSummaryHtml()}<h3>Instrument sorterade efter lägst datatillit</h3><div class="smartRouteList">${rows}</div><p class="disclaimer">NAV-instrument skickas endast till NAV-spåret. Prisproviders används bara för kursinstrument som saknar giltig cache. Portföljdata, antal och GAV ändras inte.</p>`);return false}
...
function adaptiveQuoteEngineState(){return merge({version:'11.15.47',lastRun:'',status:'not-run',attempted:0,freshCache:0,delayedCache:0,providerHits:0,fallbackHits:0,failed:0,providerPlan:{},items:[]},parse(safeGet(ADAPTIVE_QUOTE_KEY))||{})}
function persistAdaptiveQuoteEngine(x){x.version='11.15.47';x.lastRun=new Date().toISOString();safeSet(ADAPTIVE_QUOTE_KEY,JSON.stringify(x));return x}
...
function renderAdaptiveQuoteEngineReport(){const s=adaptiveQuoteEngineState(),rows=(s.items||[]).slice().sort((a,b)=>(a.status==='failed'?-1:1)-(b.status==='failed'?-1:1)||a.name.localeCompare(b.name,'sv')).map(x=>`<div class="smartRouteRow ${x.status==='failed'?'low':x.status==='cache'?'medium':'high'}"><div><b>${escapeHtml(x.name)}</b><small>${escapeHtml(x.account||'')} · ${escapeHtml(x.statusLabel||x.status)}</small><small>${escapeHtml((x.plan||[]).join(' → '))}</small>${x.error?`<small class="negative">${escapeHtml(x.error)}</small>`:''}</div><strong>${x.provider?escapeHtml(x.provider):'—'}</strong></div>`).join('');openModal(`<div class="sectionHead"><div><h2>Adaptive Quote Engine 11.15.47</h2><p>${s.attempted} behandlade · ${s.providerHits} providerträffar · ${s.freshCache+s.delayedCache} cacheträffar · ${s.failed} utan kurs</p></div></div><div class="grid"><div class="card metric"><span>Färsk cache</span><b>${s.freshCache}</b></div><div class="card metric"><span>Fördröjd cache</span><b>${s.delayedCache}</b></div><div class="card metric"><span>Nya providerträffar</span><b>${s.providerHits}</b></div><div class="card metric"><span>Misslyckade</span><b>${s.failed}</b></div></div><h3>Providerplan och utfall</h3><div class="smartRouteList">${rows||'<div class="empty">Ingen synk har körts ännu.</div>'}</div><p class="disclaimer">Motorn använder färsk cache först, provar därefter ihågkommen fungerande provider och sedan aktiva prisproviders. NAV-instrument skickas aldrig till kursproviders. Senast känt värde behålls som säker reserv.</p>`);return false}
...
async function fetchRoutedQuotes(){
 const c=providerConfig(),all=instrumentMappingCoverage().items.filter(x=>x.ticker||x.isin),limit=Math.max(1,num(c.maxSymbolsPerSync)||30),mappings=all.slice(0,limit),quotes=[],failures=[],state=adaptiveDataState(),engine={version:'11.15.47',status:'running',attempted:mappings.length,freshCache:0,delayedCache:0,providerHits:0,fallbackHits:0,failed:0,providerPlan:{},items:[]};let recovered=0,navRequired=0;
 for(const original of mappings){
  const holding=canonicalHoldings().find(h=>h._key===original.key),cls=classifyInstrument(holding||original),plan=providerPlanFor(original);engine.providerPlan[original.key]=plan;
  if(cls.pricing==='nav'){navRequired++;state.diagnostics[original.key]={lastTried:new Date().toISOString(),tested:['NAV Center'],lastError:navFor(holding||original)?'':'NAV saknas',successfulRoute:navFor(holding||original)?'Lokalt NAV':''};engine.items.push({key:original.key,name:original.name,account:original.account,status:'nav',statusLabel:navFor(holding||original)?'NAV-rutt klar':'NAV saknas',provider:'NAV Center',plan:['NAV Center']});continue}
  ...
  if(q){quotes.push(q);engine.items.push({key:original.key,name:original.name,account:original.account,status:q===cached?'cache':'provider',statusLabel:q===cached?'Fördröjd cache':'Ny kurs hämtad',provider:q.provider,plan});}else{failures.push({name:original.name,ticker:original.ticker,error:last,key:original.key});engine.failed++;engine.items.push({key:original.key,name:original.name,account:original.account,status:'failed',statusLabel:'Ingen kurs',provider:'',plan,error:last});}
 }
 state.lastRun=new Date().toISOString();state.summary={tested:mappings.length,updated:quotes.length,recovered,navRequired,failed:failures.length};persistAdaptiveData(state);engine.status=failures.length?'partial':'success';persistAdaptiveQuoteEngine(engine);resolverDiag('Adaptive Quote Engine 11.15.47',`${quotes.length}/${mappings.length} kursutfall · ${engine.providerHits} provider · ${engine.freshCache} färsk cache · ${engine.delayedCache} reservcache · ${failures.length} utan kurs`,failures.length?'warning':'success');
