...
const RESOLVER_RUN_KEY='mastarklass_os_11_15_14_resolver_run';
const RESOLVER_DIAG_KEY='mastarklass_os_11_15_14_resolver_diag';
const RESOLVER_DIAG_LINES_KEY='mastarklass_os_11_15_14_resolver_diag_lines';
const RESOLVER_DIAG_SEQ_KEY='mastarklass_os_11_15_14_resolver_diag_seq';
...
function permanentIdentityRegistry(){return merge({version:'11.15.14',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.13';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.13',source:'permanent-identity-registry-11.15.13',updatedAt:new Date().toISOString()};restored++}
 }
...
function resolverDiagnosticState(){
 const meta=merge({version:'11.15.14',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false},parse(safeGet(RESOLVER_DIAG_KEY))||{});
 meta.lines=resolverDiagnosticLines();
 return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.14',startedAt:x.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x.heartbeat),currentInstrument:x.currentInstrument||'',currentStep:x.currentStep||'',expanded:!!x.expanded};
 safeSet(RESOLVER_DIAG_KEY,JSON.stringify(meta));return meta
}
function nextResolverDiagnosticSeq(){
 const next=Math.max(0,num(safeGet(RESOLVER_DIAG_SEQ_KEY)))+1;
 safeSet(RESOLVER_DIAG_SEQ_KEY,String(next));return next
}
function appendResolverDiagnosticLine(line){
 const lines=resolverDiagnosticLines();
 lines.push(line);
 const clean=lines.slice(-300);
 if(!safeSet(RESOLVER_DIAG_LINES_KEY,JSON.stringify(clean))){
  try{console.error('[Resolver diagnostics] localStorage write failed',line)}catch{}
 }
 return clean
}
function resolverDiag(step,detail='',level='info'){
 const now=new Date(),line={seq:nextResolverDiagnosticSeq(),id:`${now.getTime()}-${Math.random().toString(36).slice(2,7)}`,iso:now.toISOString(),time:now.toLocaleTimeString('sv-SE',{hour:'2-digit',minute:'2-digit',second:'2-digit'}),step:String(step||''),detail:String(detail||''),level};
 const lines=appendResolverDiagnosticLine(line),d=resolverDiagnosticState();
 d.currentStep=line.step;d.updatedAt=now.toISOString();persistResolverDiagnosticMeta(d);d.lines=lines;updateResolverDiagnosticUI(d);console.log('[Resolver diagnostics]',line.seq,line.step,line.detail);return line
}
function resolverBootstrapDiag(step,detail='',level='info'){
 try{return resolverDiag(step,detail,level)}catch(error){
  try{console.error('[Resolver bootstrap diagnostic failed]',step,detail,error)}catch{}
  return null
 }
}
function clearResolverBootstrapWatchdogs(){
 resolverBootstrapWatchdogs.forEach(id=>clearTimeout(id));resolverBootstrapWatchdogs=[]
}
function armResolverBootstrapWatchdogs(runId){
 clearResolverBootstrapWatchdogs();
 [[250,'Bootstrap watchdog 250 ms'],[2000,'Bootstrap watchdog 2 s'],[8000,'Bootstrap watchdog 8 s']].forEach(([ms,label])=>{
  const id=setTimeout(()=>{
   const run=resolverRunState(),d=resolverDiagnosticState();
   resolverBootstrapDiag(label,`run=${run.status} ${run.processed}/${run.total} · steg=${d.currentStep||'–'} · id=${run.runId===runId?'match':'avvikelse'}`,ms>=8000?'warning':'info')
  },ms);resolverBootstrapWatchdogs.push(id)
 })
}
function installResolverBootstrapGuards(){
 ...
 window.addEventListener('error',event=>resolverBootstrapDiag('Globalt JavaScript-fel',`${event.message||'okänt fel'} · ${event.filename||''}:${event.lineno||0}:${event.colno||0}`,'error'));
 window.addEventListener('unhandledrejection',event=>resolverBootstrapDiag('Ohanterad Promise-rejection',String(event.reason?.stack||event.reason?.message||event.reason||'okänt fel'),'error'));
 resolverBootstrapDiag('Bootstrap-vakter installerade','window.error + unhandledrejection','success')
}
function beginResolverDiagnosticSession(){
 const d=resolverDiagnosticState();
 d.startedAt=new Date().toISOString();d.heartbeat=0;d.currentInstrument='';d.currentStep='Ny batch';persistResolverDiagnosticMeta(d);
 resolverDiag('──────── NY BATCH ────────',`Version ${VERSION}`,'success');return resolverDiagnosticState()
}
...
function toggleResolverDiagnosticLog(){
 const meta=resolverDiagnosticState();meta.expanded=!meta.expanded;persistResolverDiagnosticMeta(meta);
 const log=document.getElementById('resolverDiagnosticLog'),toggle=document.getElementById('toggleResolverDiagnosticLog');
 if(log)log.classList.toggle('expanded',meta.expanded);if(toggle)toggle.textContent=meta.expanded?'Komprimera loggen':'Visa hela loggen';
 return false
}
async function copyResolverDiagnostics(){
 const lines=resolverDiagnosticLines(),d=resolverDiagnosticState(),run=resolverRunState();
 const text=[`Mästarklass OS ${VERSION} Resolverdiagnostik`,`Rader: ${lines.length}`,`Status: ${run.status} ${run.processed}/${run.total}`,`Instrument: ${d.currentInstrument||'–'}`,`Steg: ${d.currentStep||'–'}`,'',...lines.map((x,i)=>`${String(x.seq||i+1).padStart(3,'0')} ${x.time} ${x.step}${x.detail?' — '+x.detail:''}`)].join('\n');
 try{await navigator.clipboard.writeText(text);resolverDiag('Logg kopierad',`${lines.length} rader`)}catch(error){resolverDiag('Kopiering misslyckades',String(error?.message||error),'error')}return false
}
function clearResolverDiagnosticLog(){
 safeSet(RESOLVER_DIAG_LINES_KEY,'[]');safeSet(RESOLVER_DIAG_SEQ_KEY,'0');
 const d=resolverDiagnosticState();d.heartbeat=0;d.currentInstrument='';d.currentStep='Rensad';persistResolverDiagnosticMeta(d);updateResolverDiagnosticUI({...d,lines:[]});return false
}
...
function startResolverHeartbeat(){
 stopResolverHeartbeat();let d=resolverDiagnosticState();d.heartbeat=0;persistResolverDiagnosticMeta(d);updateResolverDiagnosticUI(d);
 resolverHeartbeatTimer=setInterval(()=>{const x=resolverDiagnosticState();x.heartbeat=num(x.heartbeat)+1;persistResolverDiagnosticMeta(x);const beat=document.getElementById('resolverHeartbeat');if(beat)beat.textContent=String(x.heartbeat)},1000)
}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.14',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
function startResolverBatchFromUI(){
 ...
 beginResolverDiagnosticSession();installResolverBootstrapGuards();
 ...
 try{
  ...
  persistResolverRun(run);resolverBootstrapDiag('Bootstrap 07 Körstatus sparad','localStorage OK','success');
...
async function runGlobalResolver(runId){
 ...
 resolverBootstrapDiag('Worker 03 Körstatus läst',`${run.status} · id=${run.runId===runId?'match':'avvikelse'}`);
...
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){const state=globalResolverState(),run=resolverRunState(),row=rowOverride||(run.suggestions||[]).find(x=>x.key===key)||(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.13'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.14'],source:'permanent-identity-registry-11.15.13',resolverVersion:'11.15.13',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.8',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
