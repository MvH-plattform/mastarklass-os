...
function emptyPermanentRegistry(){return {version:'11.15.31',updatedAt:'',storage:'IndexedDB',routes:{},audit:[]}}
function normalizePermanentRegistry(raw){
 ...
 registry.version='11.15.31';registry.storage='IndexedDB';
 return registry
}
...
async function hydrateResolverPersistentState(){
 try{
  ...
  if(restoredRun&&/^11\.15\.(?:2[4-9]|3[01])$/.test(String(restoredRun.version||''))){
...
async function savePermanentIdentityRoute(key,route,meta={}){
 ...
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.31',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
...
async function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.31',source:'permanent-identity-registry-11.15.31',updatedAt:new Date().toISOString()};restored++}
 }
 ...
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.31',`${restored} rutter återställda · ${migrated} migrerade till IndexedDB`,{})}
...
function resolverDiagnosticState(){
 ...
 const meta=merge({version:'11.15.31',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false,storage:resolverTraceStorage},raw);
 meta.lines=resolverDiagnosticLines();meta.storage=resolverTraceStorage;return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.31',startedAt:x?.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x?.heartbeat),currentInstrument:x?.currentInstrument||'',currentStep:x?.currentStep||'',expanded:!!x?.expanded,storage:'IndexedDB'};
...
function resolverRunDefaults(){return {version:'11.15.31',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,fundPending:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],providerCooldowns:{},rejections:{},identityOverrides:{},message:''}}
...
function persistResolverRun(x){
 ...
 // 11.15.31: cooldowns are monotonic state. A stale loop snapshot must never erase
...
async function resolveOneInstrument(item){
 ...
 const m=liveFoundation.mappings[item.key]||{},runForIdentity=resolverRunState(),manualOverride=runForIdentity.identityOverrides?.[item.key]||{};
 item={...item,name:manualOverride.name||item.name,ticker:manualOverride.ticker||m.ticker||item.ticker||'',isin:manualOverride.isin||m.isin||item.isin||'',exchange:manualOverride.exchange||m.exchange||item.exchange||'',currency:manualOverride.currency||item.currency||m.currency||''};
 resolverDiag('Lokal mapping klar',`${item.ticker||'–'} · ${item.isin||'–'} · ${item.exchange||'–'}${manualOverride.updatedAt?' · användarkompletterad':''}`);
 ...
 let unique=filterRejectedResolverCandidates(item.key,candidates).map(c=>({...c,score:resolverScore(item,c)})).sort((a,b)=>b.score-a.score);
 ...
 unique=filterRejectedResolverCandidates(item.key,[...new Map(candidates.filter(c=>c.ticker||c.isin).map(c=>[resolverCandidateKey(c),c])).values()]).map(c=>({...c,score:resolverScore(item,c)})).sort((a,b)=>b.score-a.score);
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.31',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,fundPending:run.fundPending||0,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
async function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){
 ...
 if(!await savePermanentIdentityRoute(key,route,{source:'global-resolver-approved-11.15.31'})){if(!silent)alert('Permanent lagring i IndexedDB misslyckades. Instrumentet ligger kvar och ingen portföljdata har ändrats. Kopiera Resolver Chain Trace för exakt fel.');return false}
 ...
 liveEvent('success','Permanent Identity Registry 11.15.31',`${row.name} sparad permanent som ${route.ticker||route.isin} · ${route.exchange||'okänd börs'} · ${route.currency}`,verified);
...
function resolverCandidateFingerprint(candidate){
 return [candidate?.isin,candidate?.ticker,candidate?.exchange,candidate?.provider].map(x=>String(x||'').trim().toUpperCase()).join('|')
}
function filterRejectedResolverCandidates(key,candidates){
 const rejected=new Set((resolverRunState().rejections?.[key]?.fingerprints)||[]);
 return (candidates||[]).filter(candidate=>!rejected.has(resolverCandidateFingerprint(candidate)))
}
function rejectResolverCandidates(key){
 const run=resolverRunState(),row=(run.suggestions||[]).find(x=>x.key===key);if(!row)return false;
 const existing=run.rejections?.[key]||{fingerprints:[],history:[]};
 const fingerprints=[...new Set([...(existing.fingerprints||[]),...(row.candidates||[]).map(resolverCandidateFingerprint).filter(Boolean)])];
 run.rejections={...(run.rejections||{}),[key]:{fingerprints,history:[...(existing.history||[]),{at:new Date().toISOString(),count:(row.candidates||[]).length}].slice(-20)}};
 run.review=Math.max(0,num(run.review)-(row.best?1:0));
 run.suggestions=(run.suggestions||[]).filter(x=>x.key!==key);
 persistResolverRun(run);resolverDiag('Review reject 01 Alla kandidater avvisade',`${row.name} · ${(row.candidates||[]).length} kandidater spärrade`,'warning');return row
}
function identityCorrectionHtml(row){const current=resolverRunState().identityOverrides?.[row.key]||{};return `<div class="eyebrow">Resolver 11.15.31 · Intelligent Review</div><h2>Komplettera identitet</h2><p class="hint">Alla nuvarande alternativ markeras som fel och föreslås inte igen för detta innehav. ISIN är den säkraste uppgiften.</p><div class="identityCorrectionSummary"><b>${esc(row.name)}</b><small>${esc(row.account)} · ${esc(row.assetType)}</small></div><form id="identityCorrectionForm" class="identityCorrectionForm"><input type="hidden" name="holdingKey" value="${esc(row.key)}"><label>ISIN <input name="isin" value="${esc(current.isin||'')}" placeholder="Ex. SE0000123456" autocapitalize="characters"></label><label>Ticker <input name="ticker" value="${esc(current.ticker||'')}" placeholder="Ex. LIME.ST" autocapitalize="characters"></label><label>Börs <input name="exchange" value="${esc(current.exchange||'')}" placeholder="Ex. Nasdaq Stockholm"></label><label>Valuta <input name="currency" value="${esc(current.currency||row.currency||'')}" maxlength="3" placeholder="SEK"></label><label>Exakt namn / sökfras <input name="name" value="${esc(current.name||row.name||'')}" placeholder="Instrumentets fullständiga namn"></label><label>Kommentar <textarea name="note" rows="3" placeholder="Varför alternativen är fel eller vad nästa sökning ska prioritera">${esc(current.note||'')}</textarea></label><div class="identityCorrectionActions"><button type="submit" class="primary" data-correction-action="retry">Avvisa och sök igen</button><button type="submit" class="secondary" data-correction-action="pending">Flytta till Behöver identitet</button><button type="button" class="secondary" id="cancelIdentityCorrection">Tillbaka</button></div></form>`}
function openIdentityCorrection(key){
 const run=resolverRunState(),row=(run.suggestions||[]).find(x=>x.key===key);if(!row)return;
 openModal(identityCorrectionHtml(row));
 document.getElementById('cancelIdentityCorrection')?.addEventListener('click',openGlobalResolverReview);
 document.getElementById('identityCorrectionForm')?.addEventListener('submit',async event=>{
  event.preventDefault();const submitter=event.submitter,action=submitter?.dataset.correctionAction||'retry',form=new FormData(event.currentTarget);
  const override={isin:String(form.get('isin')||'').trim().toUpperCase(),ticker:String(form.get('ticker')||'').trim().toUpperCase(),exchange:String(form.get('exchange')||'').trim(),currency:String(form.get('currency')||'').trim().toUpperCase(),name:String(form.get('name')||'').trim(),note:String(form.get('note')||'').trim(),updatedAt:new Date().toISOString()};
  const rejectedRow=rejectResolverCandidates(key);const next=resolverRunState();next.identityOverrides={...(next.identityOverrides||{}),[key]:override};
  if(action==='pending'){
   next.fundPending=num(next.fundPending)+1;next.suggestions=[...(next.suggestions||[]),{...rejectedRow,best:null,candidates:[],confidence:0,autoSafe:false,conflict:false,fundPending:true,manualPending:true,errors:[override.note||'Användaren avvisade samtliga kandidater och begärde kompletterad identitet.']}];
   persistResolverRun(next);resolverDiag('Review correction 03 Behöver identitet',`${rejectedRow.name} flyttad ur granskning`,'success');openGlobalResolverReview();return
  }
  persistResolverRun(next);resolverDiag('Review correction 02 Ny sökning',`${rejectedRow.name} · ISIN=${override.isin||'–'} · ticker=${override.ticker||'–'}`,'info');
  submitter.disabled=true;submitter.textContent='Söker…';
  try{const base=(next.targetItems||[]).find(x=>x.key===key)||instrumentMappingCoverage().items.find(x=>x.key===key)||rejectedRow;const result=compactResolverSuggestion(await resolveOneInstrument({...base,...override,key}));const latest=resolverRunState();latest.suggestions=[...(latest.suggestions||[]).filter(x=>x.key!==key),result];if(result.fundPending)latest.fundPending=num(latest.fundPending)+1;else if(result.best)latest.review=num(latest.review)+1;else latest.failed=num(latest.failed)+1;persistResolverRun(latest);resolverDiag('Review correction 04 Omsökning klar',`${result.name} · ${result.best?.ticker||result.best?.isin||'ingen träff'}`,result.best?'success':'warning');openGlobalResolverReview()}catch(error){resolverDiag('Review correction 05 Omsökning fel',String(error?.message||error),'error');alert('Omsökningen misslyckades. Uppgifterna är sparade och instrumentet ligger kvar för nytt försök.');openGlobalResolverReview()}
 })
}
function resolverReviewHtml(){const run=resolverRunState(),rows=(run.suggestions||[]).map(row=>{const best=row.best,applied=routeIsUsable(permanentRouteFor(row.key)),candidates=row.candidates||[];const candidateCards=best?`<div class="resolverCandidateDetails">${candidates.map((c,i)=>`<div class="resolverCandidateMini ${i===0?'selected':''}"><b>${i+1}. ${esc(c.ticker||c.isin||'Okänd identitet')}</b><span>ISIN: <strong>${esc(c.isin||'saknas')}</strong></span><span>${esc(c.name||'Namn saknas')}</span><span>${esc(c.exchange||'Okänd börs')} · ${esc(c.currency||row.currency||'Okänd valuta')} · ${esc(c.provider||'Okänd källa')} · ${num(c.score)}%</span></div>`).join('')}</div>`:'';return `<article data-resolver-row="${esc(row.key)}" class="resolverResult ${row.conflict?'conflict':row.autoSafe?'safe':best?'review':'missing'}"><div class="resolverTitle"><div><b>${esc(row.name)}</b><small>${esc(row.account)} · ${esc(row.assetType)}</small></div><span>${applied?'Permanent':best?`${num(row.confidence)}%`:'Ingen träff'}</span></div>${best?`<div class="resolverBest"><strong>${esc(best.ticker||best.isin)}</strong><p>${esc(best.name||'')} · ${esc(best.exchange||'Okänd börs')} · ${esc(best.currency||row.currency||'')}</p><p class="resolverIsin">ISIN: <b>${esc(best.isin||'saknas hos källan')}</b></p><small>${esc(best.provider)}${row.conflict?' · konflikt mellan källor':''}</small></div>${candidateCards}<div class="resolverActions"><button class="primary" data-resolver-apply="${esc(row.key)}" ${applied?'disabled':''}>${applied?'Sparad permanent':'Godkänn och spara permanent'}</button>${candidates.length>1?`<select data-resolver-choice="${esc(row.key)}">${candidates.map((c,i)=>`<option value="${i}">${i+1}. ${esc(c.ticker||c.isin)} · ISIN ${esc(c.isin||'saknas')} · ${esc(c.exchange||'okänd börs')} · ${esc(c.provider)} · ${num(c.score)}%</option>`).join('')}</select>`:''}<button class="secondary rejectCandidates" data-resolver-reject="${esc(row.key)}">Inget av alternativen stämmer</button></div>`:row.fundPending?`<div class="notice"><b>Behöver fondidentitet</b><br>${row.manualPending?'Flyttad hit efter att samtliga kandidater avvisades.':'Traditionell fond utan ISIN. Aktieproviders hoppades över.'} Lägg till ISIN eller NAV-identitet för ett nytt försök.</div><button class="secondary rejectCandidates" data-resolver-reject="${esc(row.key)}">Komplettera och sök igen</button>`:`<div class="notice">Ingen säker identitet hittades. ${esc((row.errors||[]).join(' · ')||'Ingen provider gav användbar träff.')}</div><button class="secondary rejectCandidates" data-resolver-reject="${esc(row.key)}">Komplettera identitet</button>`}</article>`}).join('');const categorized=Math.min(num(run.processed),num(run.newSafe)+num(run.review)+num(run.fundPending)+num(run.failed)+num(run.skipped));return `<div class="eyebrow">Global Identity Resolver 11.15.31</div><h2>Verifiera resultat</h2><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`}
function openGlobalResolverReview(){
 ...
 document.querySelectorAll('[data-resolver-reject]').forEach(btn=>btn.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();openIdentityCorrection(btn.dataset.resolverReject)}));
 document.querySelectorAll('[data-resolver-apply]').forEach(btn=>btn.addEventListener('click',async event=>{
  event.preventDefault();event.stopPropagation();
  const modalCard=document.querySelector('.modalCard'),scroll=modalCard?.scrollTop||0,key=btn.dataset.resolverApply,rowEl=btn.closest('[data-resolver-row]'),rowTop=rowEl?rowEl.getBoundingClientRect().top:0,sel=document.querySelector(`[data-resolver-choice="${CSS.escape(key)}"]`),idx=num(sel?.value);
