...
const RESOLVER_RUN_KEY='mastarklass_os_11_15_11_resolver_run';
const RESOLVER_DIAG_KEY='mastarklass_os_11_15_12_resolver_diag';
const RESOLVER_DIAG_LINES_KEY='mastarklass_os_11_15_12_resolver_diag_lines';
...
let resolverWorkerActive=false,resolverStartLocked=false,resolverHeartbeatTimer=null;
...
function permanentIdentityRegistry(){return merge({version:'11.15.12',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.12';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.12',source:'permanent-identity-registry-11.15.12',updatedAt:new Date().toISOString()};restored++}
 }
...
function resolverDiagnosticLines(){
 const parsed=parse(safeGet(RESOLVER_DIAG_LINES_KEY));
 return Array.isArray(parsed)?parsed.slice(-200):[]
}
function resolverDiagnosticState(){
 const meta=merge({version:'11.15.12',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false},parse(safeGet(RESOLVER_DIAG_KEY))||{});
 meta.lines=resolverDiagnosticLines();
 return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.12',startedAt:x.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x.heartbeat),currentInstrument:x.currentInstrument||'',currentStep:x.currentStep||'',expanded:!!x.expanded};
 safeSet(RESOLVER_DIAG_KEY,JSON.stringify(meta));return meta
}
function persistResolverDiagnosticLines(lines){const clean=(Array.isArray(lines)?lines:[]).slice(-200);safeSet(RESOLVER_DIAG_LINES_KEY,JSON.stringify(clean));return clean}
function resolverDiag(step,detail='',level='info'){
 const d=resolverDiagnosticState(),now=new Date(),line={id:`${now.getTime()}-${Math.random().toString(36).slice(2,7)}`,time:now.toLocaleTimeString('sv-SE',{hour:'2-digit',minute:'2-digit',second:'2-digit'}),step:String(step||''),detail:String(detail||''),level};
 const lines=persistResolverDiagnosticLines([...resolverDiagnosticLines(),line]);
 d.currentStep=line.step;d.updatedAt=now.toISOString();persistResolverDiagnosticMeta(d);d.lines=lines;updateResolverDiagnosticUI(d);console.log('[Resolver diagnostics]',line.step,line.detail);return line
}
function resetResolverDiagnostics(){
 const d={version:'11.15.12',startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),heartbeat:0,currentInstrument:'',currentStep:'Start',expanded:false};
 persistResolverDiagnosticLines([]);persistResolverDiagnosticMeta(d);resolverDiag('Diagnostik startad','Väntar på batch');return resolverDiagnosticState()
}
function diagnosticLineHtml(x,index){return `<div class="diagLine ${esc(x.level||'info')}"><span class="diagIndex">${index+1}</span><div><b>${esc(x.time)}</b> ${esc(x.step)}${x.detail?`<small>${esc(x.detail)}</small>`:''}</div></div>`}
function updateResolverDiagnosticUI(d=resolverDiagnosticState()){
 ...
 if(beat)beat.textContent=String(d.heartbeat||0);if(step)step.textContent=d.currentStep||'Redo';if(instrument)instrument.textContent=d.currentInstrument||'–';if(count)count.textContent=String((d.lines||[]).length);
 if(log){const wasNearBottom=log.scrollHeight-log.scrollTop-log.clientHeight<80;log.classList.toggle('expanded',!!d.expanded);log.innerHTML=(d.lines||[]).map(diagnosticLineHtml).join('')||'<div class="empty">Ingen diagnostik ännu.</div>';if(wasNearBottom||!d.expanded)log.scrollTop=log.scrollHeight}
...
async function copyResolverDiagnostics(){const d=resolverDiagnosticState(),run=resolverRunState(),text=[`Mästarklass OS ${VERSION} Resolverdiagnostik`,`Status: ${run.status} ${run.processed}/${run.total}`,`Instrument: ${d.currentInstrument||'–'}`,`Steg: ${d.currentStep||'–'}`,...(d.lines||[]).map((x,i)=>`${i+1}. ${x.time} ${x.step}${x.detail?' — '+x.detail:''}`)].join('\n');try{await navigator.clipboard.writeText(text);resolverDiag('Logg kopierad',`${d.lines?.length||0} rader`)}catch(error){resolverDiag('Kopiering misslyckades',String(error?.message||error),'error')}return false}
function toggleResolverDiagnosticLog(){const d=resolverDiagnosticState();d.expanded=!d.expanded;persistResolverDiagnosticMeta(d);updateResolverDiagnosticUI({...d,lines:resolverDiagnosticLines()});return false}
function clearResolverDiagnosticLog(){persistResolverDiagnosticLines([]);const d=resolverDiagnosticState();d.currentStep='Logg rensad';persistResolverDiagnosticMeta(d);updateResolverDiagnosticUI({...d,lines:[]});return false}
...
function resolverRunState(){return merge({version:'11.15.12',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],message:''},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.12';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
async function resolveOneInstrument(item){
 ...
 const d=resolverDiagnosticState();d.currentInstrument=item.name||item.key;persistResolverDiagnostic(d);updateResolverDiagnosticUI(d);
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.12',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
function startResolverBatchFromUI(){
 if(resolverWorkerActive||resolverStartLocked){resolverDiag('Start blockerad',`worker=${resolverWorkerActive} lock=${resolverStartLocked}`,'warning');return false;}
 resetResolverDiagnostics();resolverDiag('Knapptryck mottaget','Startar diagnostisk batch');startResolverHeartbeat();
 ...
 try{
  ...
  persistResolverRun(run);syncResolverSummary(run);updateResolverProgressUI(run);setResolverStatusLabel(run.total?'Kör':'Klar');
  if(!run.total){run.status='complete';persistResolverRun(run);updateResolverProgressUI(run);setResolverButtonState('Inga instrument återstår',true);resolverStartLocked=false;return true}
  resolverWorkerActive=true;
  setResolverButtonState(`Verifierar ${run.processed+1}/${run.total}…`,true);
  // Starta arbetaren direkt. Ingen timer eller extra render får ligga mellan trycket och instrument 1.
  resolverDiag('Worker anropas',run.runId);
  void runGlobalResolver(run.runId).catch(error=>{
   console.error('Resolver worker failed',error);resolverDiag('Worker kraschade',String(error?.stack||error?.message||error),'error');
  ...
  }).finally(()=>{resolverWorkerActive=false;resolverStartLocked=false;stopResolverHeartbeat();resolverDiag('Worker avslutad',resolverRunState().status)});
  return true;
 }catch(error){
  console.error('Resolver start failed',error);stopResolverHeartbeat();resolverDiag('Start kraschade',String(error?.stack||error?.message||error),'error');
...
async function runGlobalResolver(runId){
 ...
 let run=resolverRunState();
 if(run.status!=='running'||run.runId!==runId){resolverDiag('Worker avbruten','Körstatus eller runId matchar inte','error');return false;}
...
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){const state=globalResolverState(),run=resolverRunState(),row=rowOverride||(run.suggestions||[]).find(x=>x.key===key)||(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.12'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.12'],source:'permanent-identity-registry-11.15.12',resolverVersion:'11.15.12',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.8',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
