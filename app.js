...
function marketRoutingState(){return merge({version:'11.15.48',lastRun:'',status:'not-run',total:0,routed:0,quoteReady:0,navReady:0,fxReady:0,needsPrice:0,needsNav:0,needsFx:0,routes:{},missingPrice:[],missingNav:[],missingFx:[],blockedQuoteRoutes:[]},parse(safeGet(MARKET_ROUTING_KEY))||{})}
function persistMarketRouting(x){x.version='11.15.48';x.lastRun=new Date().toISOString();safeSet(MARKET_ROUTING_KEY,JSON.stringify(x));return x}
...
function buildMarketRoutingBridge({log=true}={}){
 ...
 if(log)resolverDiag('ISIN Market Routing 11.15.48',`${routed}/${hs.length} rutter · ${quoteReady} kurser · ${navReady} NAV · ${needsFx} saknar FX · ${blockedQuoteRoutes.length} NAV-rutter skyddade`,routed===hs.length?'success':'warning');
 return result
}
...
function marketRoutingPanel(){const r=marketRoutingState(),built=r.lastRun?resolverInteractionTime(r.lastRun):'Inte byggd';return `<div class="portfolioBridgeBox marketRoutingBridge"><div><b>Smart Market Router & Valuation Bridge 11.15.48</b><span>${r.status==='not-run'?'Inte byggd':`Rutter klara ${r.routed}/${r.total} · ${r.quoteReady} kurser · ${r.navReady} NAV`}</span><small>Senast byggd: ${built} · ${r.needsPrice||0} saknar kurs · ${r.needsNav||0} saknar NAV · ${r.needsFx||0} saknar FX</small></div><div class="identityConflictActions"><button id="mk-market-routing-btn" class="primary" type="button">Bygg marknadsrutter</button><button class="secondary" type="button" onclick="return window.mkOpenMarketRoutingReport(event)">Öppna rutt- och felrapport</button><button class="secondary" type="button" onclick="return window.mkOpenSmartRouterReport(event)">Öppna datatillit och providerplan</button><button class="secondary" type="button" onclick="return window.mkOpenAdaptiveQuoteReport(event)">Öppna Adaptive Quote Engine</button></div></div>`}
function runMarketRoutingWithFeedback(event){event?.preventDefault?.();event?.stopPropagation?.();resolverButtonFeedback('mk-market-routing-btn','Bygger rutter…',true);try{const r=buildMarketRoutingBridge();buildSmartRouterState({log:true});canonicalCache=null;liveValuationSnapshot({persist:true});resolverButtonFeedback('mk-market-routing-btn','Rutter klara ✓',true);render();restoreResolverButton('mk-market-routing-btn',1500);return r}catch(error){const message=storageErrorText(error);resolverDiag('ISIN Market Routing 11.15.48 fel',message,'error');resolverButtonFeedback('mk-market-routing-btn','Kunde inte bygga',true);alert('Marknadsrutterna kunde inte byggas.\n\n'+message);restoreResolverButton('mk-market-routing-btn',2200);return false}}
window.mkRunMarketRouting=runMarketRoutingWithFeedback;

function renderMarketRoutingReport(){const r=marketRoutingState();const rows=(items,label)=>items?.length?items.map(x=>`<div class="missingIsinRow"><span><b>${escapeHtml(x.name||x.currency||'Okänd')}</b><small>${escapeHtml(x.account||x.currency||'')} ${x.ticker?`· ${escapeHtml(x.ticker)}`:''} ${x.isin?`· ${escapeHtml(x.isin)}`:''}</small></span><strong>${label}</strong></div>`).join(''):`<div class="notice okbox">Inga poster.</div>`;openModal(`<div class="sectionHead"><div><h2>Marknadsrutter 11.15.48</h2><p>Rutter klara ${r.routed}/${r.total} · senast byggd ${resolverInteractionTime(r.lastRun)}</p></div></div><div class="grid"><div class="card metric"><span>Kurser</span><b>${r.quoteReady}</b></div><div class="card metric"><span>NAV</span><b>${r.navReady}</b></div><div class="card metric"><span>FX klara</span><b>${r.fxReady}</b></div><div class="card metric"><span>NAV-rutter skyddade</span><b>${r.blockedQuoteRoutes?.length||0}</b></div></div><h3>Saknar kurs</h3>${rows(r.missingPrice,'Kurs saknas')}<h3 class="topGap">Saknar NAV</h3>${rows(r.missingNav,'NAV saknas')}<h3 class="topGap">Saknar FX</h3>${rows(r.missingFx,'FX saknas')}<p class="disclaimer">NAV-instrument, inklusive Montrose-fonder, blockeras från aktieproviders för att undvika HTTP 403 och andra felaktiga kursanrop. Senast kända värden används tills giltig kurs eller NAV finns.</p>`);return false}
window.mkOpenMarketRoutingReport=event=>{event?.preventDefault?.();return renderMarketRoutingReport()};



/* 11.15.48 — Smart Provider Router & Valuation Confidence */
function smartRouterState(){return merge({version:'11.15.48',lastRun:'',status:'not-run',total:0,quoteRoutes:0,navRoutes:0,cacheFresh:0,cacheDelayed:0,providerNeeded:0,highConfidence:0,mediumConfidence:0,lowConfidence:0,averageConfidence:0,items:[]},parse(safeGet(SMART_ROUTER_KEY))||{})}
function persistSmartRouter(x){x.version='11.15.48';x.lastRun=new Date().toISOString();safeSet(SMART_ROUTER_KEY,JSON.stringify(x));return x}
...
function buildSmartRouterState({log=true}={}){
 ...
 if(log)resolverDiag('Smart Provider Router 11.15.48',`${items.length} instrument · ${quoteRoutes} kursrutter · ${navRoutes} NAV-rutter · tillit ${result.averageConfidence}%`,lowConfidence?'warning':'success');
 return result
}
...
function renderSmartRouterReport(){const s=buildSmartRouterState({log:false});const rows=s.items.slice().sort((a,b)=>a.confidence-b.confidence).map(x=>`<div class="smartRouteRow ${x.confidenceLevel}"><div><b>${escapeHtml(x.name)}</b><small>${escapeHtml(x.account)} · ${escapeHtml(x.pricing.toUpperCase())} · ${escapeHtml(x.providerAction)}</small><small>${escapeHtml((x.reasons||[]).join(' · ')||'Saknar underlag')}</small></div><strong>${x.confidence}%</strong></div>`).join('');openModal(`<div class="sectionHead"><div><h2>Smart Provider Router 11.15.48</h2><p>${s.total} instrument · ${s.quoteRoutes} kursrutter · ${s.navRoutes} NAV-rutter</p></div></div>${smartRouterSummaryHtml()}<h3>Instrument sorterade efter lägst datatillit</h3><div class="smartRouteList">${rows}</div><p class="disclaimer">NAV-instrument skickas endast till NAV-spåret. Prisproviders används bara för kursinstrument som saknar giltig cache. Portföljdata, antal och GAV ändras inte.</p>`);return false}
...
function adaptiveQuoteEngineState(){return merge({version:'11.15.48',lastRun:'',status:'not-run',attempted:0,total:0,processed:0,batches:0,currentBatch:0,navHandled:0,freshCache:0,delayedCache:0,providerHits:0,fallbackHits:0,failed:0,providerPlan:{},items:[]},parse(safeGet(ADAPTIVE_QUOTE_KEY))||{})}
function persistAdaptiveQuoteEngine(x){x.version='11.15.48';x.lastRun=new Date().toISOString();safeSet(ADAPTIVE_QUOTE_KEY,JSON.stringify(x));return x}
...
async function fetchRoutedQuotes(){
 const c=providerConfig(),all=instrumentMappingCoverage().items.filter(x=>x.ticker||x.isin),batchSize=Math.max(1,Math.min(30,num(c.maxSymbolsPerSync)||30)),mappings=all,quotes=[],failures=[],state=adaptiveDataState(),engine={version:'11.15.48',status:'running',attempted:0,total:mappings.length,processed:0,batches:Math.ceil(mappings.length/batchSize),freshCache:0,delayedCache:0,providerHits:0,fallbackHits:0,failed:0,navHandled:0,providerPlan:{},items:[]};let recovered=0,navRequired=0;
 ...
 for(let start=0;start<mappings.length;start+=batchSize){
  ...
  if(q){quotes.push(q);engine.items.push({key:original.key,name:original.name,account:original.account,isin:original.isin,status:q===cached?'cache':'provider',statusLabel:q===cached?'Fördröjd cache':'Ny kurs hämtad',provider:q.provider,plan});}else{failures.push({name:original.name,ticker:original.ticker,error:last,key:original.key});engine.failed++;engine.items.push({key:original.key,name:original.name,account:original.account,isin:original.isin,status:'failed',statusLabel:'Ingen kurs',provider:'',plan,error:last});}
  }
