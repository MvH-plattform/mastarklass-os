...
const RESOLVER_RUN_KEY='mastarklass_os_11_15_6_resolver_run';
...
function permanentIdentityRegistry(){return merge({version:'11.15.6',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.6';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function savePermanentIdentityRoute(key,route,meta={}){
 ...
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.3',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.6',source:'permanent-identity-registry-11.15.6',updatedAt:new Date().toISOString()};restored++}
 }
 ...
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.3',`${restored} rutter återställda · ${migrated} migrerade`,{})}
...
function resolverRunState(){return merge({version:'11.15.6',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],suggestions:[],message:''},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.6';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function requestResolverStop(){const x=resolverRunState();if(x.status!=='running')return;x.stopRequested=true;x.message='Stoppar säkert efter aktuellt instrument…';persistResolverRun(x);updateResolverProgressUI(x)}
...
async function resolveOneInstrument(item){
 ...
 let candidates=[...localResolverCandidates(item)],errors=[];
 let unique=candidates.map(c=>({...c,score:resolverScore(item,c)})).sort((a,b)=>b.score-a.score);
 if(unique[0]&&unique[0].score>=98)return {key:item.key,name:item.name,account:item.account,assetType:item.assetType,currency:item.currency,best:unique[0],candidates:unique.slice(0,4),confidence:unique[0].score,margin:100,conflict:false,autoSafe:true,variants:tickerVariants(item),exchanges:exchangeVariants(item),errors:[]};
 const jobs=[];const cfg=providerConfig();if(cfg.twelveDataKey)jobs.push(withDeadline(searchTwelveDataResolver(item),6000,'Twelve Data'));if(cfg.alphaVantageKey)jobs.push(withDeadline(searchAlphaVantageResolver(item),6000,'Alpha Vantage'));if(cfg.finnhubKey)jobs.push(withDeadline(searchFinnhubInstrument(item),6000,'Finnhub'));jobs.push(withDeadline(searchOpenFigiPost(item),6000,'OpenFIGI'));
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.6',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
function updateResolverProgressUI(run=resolverRunState()){
 ...
 const fields={resolverRemaining:Math.max(0,(run.total||0)-(run.processed||0)),resolverPermanent:run.permanent||0,resolverSafe:run.newSafe||0,resolverReview:run.review||0,resolverSkipped:run.skipped||0,resolverFailed:run.failed||0};for(const [id,v] of Object.entries(fields)){const el=document.getElementById(id);if(el)el.textContent=v}
...
window.mkRunResolverBatch=function(event){if(event){event.preventDefault?.();event.stopPropagation?.()}queueMicrotask(()=>startResolverBatchFromUI());return false};
window.mkStopResolver=function(event){if(event){event.preventDefault?.();event.stopPropagation?.()}requestResolverStop();return false};
window.mkReviewResolver=function(event){if(event){event.preventDefault?.();event.stopPropagation?.()}openGlobalResolverReview();return false};
async function startResolverBatchFromUI(){
 const button=document.getElementById('runGlobalResolver');
 if(button){button.classList.add('isDisabled');button.setAttribute('aria-disabled','true');button.removeAttribute('href');button.textContent='Startar batch…'}
 try{
  await runGlobalResolver();
 }catch(error){
  console.error('Resolver batch failed',error);
  let run=resolverRunState();run.status='error';run.message=`Kunde inte starta batch: ${String(error?.message||error)}`;run.errors=num(run.errors)+1;persistResolverRun(run);syncResolverSummary(run);updateResolverProgressUI(run);alert(run.message);render();
 }finally{
  const next=document.getElementById('runGlobalResolver');if(next&&resolverRunState().status!=='running'){next.classList.remove('isDisabled');next.setAttribute('aria-disabled','false');next.href='#resolver-run'}
 }
}
async function runGlobalResolver(){
 if(resolverWorkerActive)return;resolverWorkerActive=true;
 try{
  let run=resolverRunState(),allItems=instrumentMappingCoverage().items,byKey=new Map(allItems.map(x=>[x.key,x])),freshTargets=resolverTargets();
  const canResume=run.status==='paused'&&run.targetKeys?.length&&run.cursor<run.targetKeys.length;
  if(!canResume){run={version:'11.15.6',status:'running',runId:crypto.randomUUID?.()||String(Date.now()),startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),completedAt:'',cursor:0,total:freshTargets.length,processed:0,skipped:Object.keys(permanentIdentityRegistry().routes||{}).length,permanent:Object.keys(permanentIdentityRegistry().routes||{}).length,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:freshTargets.map(x=>x.key),suggestions:[],message:'Startar stabil batch…'}}
  else{run.status='running';run.stopRequested=false;run.message='Fortsätter från senaste checkpoint…'}
  persistResolverRun(run);render();await new Promise(r=>setTimeout(r,80));
  const batchEnd=Math.min(run.targetKeys.length,run.cursor+8);
  for(let i=run.cursor;i<batchEnd;i++){
   run=resolverRunState();if(run.stopRequested){run.status='paused';run.message=`Pausad efter ${run.processed}/${run.total}.`;persistResolverRun(run);syncResolverSummary(run);updateResolverProgressUI(run);return}
   const item=byKey.get(run.targetKeys[i]);if(!item){run.cursor=i+1;run.processed++;run.failed++;run.message='Instrument saknas i aktuell portfölj och hoppades över.';persistResolverRun(run);continue}
   if(routeIsUsable(permanentRouteFor(item.key))){run.cursor=i+1;run.processed++;run.skipped++;run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length;persistResolverRun(run);updateResolverProgressUI(run);continue}
   run.message=`Verifierar ${run.processed+1}/${run.total}: ${item.name}`;persistResolverRun(run);updateResolverProgressUI(run);
   try{
    const row=await resolveOneInstrument(item);run=resolverRunState();run.cursor=i+1;run.processed++;run.errors+=row.errors.length;run.suggestions=[...(run.suggestions||[]).filter(x=>x.key!==row.key),compactResolverSuggestion(row)].slice(-80);
    if(row.autoSafe&&applyResolverSuggestion(row.key,0,{silent:true,rowOverride:row})){run.newSafe++;run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length;refreshValuationAfterResolver()}
   ...
   run.message=`Checkpoint sparad ${run.processed}/${run.total}`;persistResolverRun(run);syncResolverSummary(run);updateResolverProgressUI(run);await new Promise(r=>setTimeout(r,120))
  }
  run=resolverRunState();if(run.cursor>=run.targetKeys.length){run.status='complete';run.completedAt=new Date().toISOString();run.message=`Klar: ${run.newSafe} nya säkra · ${run.review} för granskning · ${run.failed} utan träff.`}else{run.status='paused';run.message=`Batch klar. ${run.total-run.processed} återstår — tryck Fortsätt.`}run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length;persistResolverRun(run);syncResolverSummary(run);refreshValuationAfterResolver();liveEvent(run.failed?'warning':'success','Global Identity Resolver 11.15.6',run.message,{skipped:run.skipped,permanent:run.permanent,errors:run.errors});render()
 }finally{resolverWorkerActive=false}
}
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){const state=globalResolverState(),run=resolverRunState(),row=rowOverride||(run.suggestions||[]).find(x=>x.key===key)||(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.6'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.6'],source:'permanent-identity-registry-11.15.6',resolverVersion:'11.15.6',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.3',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
function applySafeResolverSuggestions(){const run=resolverRunState();let count=0;for(const row of run.suggestions||[]){if(row.autoSafe&&!routeIsUsable(permanentRouteFor(row.key))&&applyResolverSuggestion(row.key,0))count++}return count}
function resolverReviewHtml(){const run=resolverRunState(),rows=(run.suggestions||[]).map(row=>{const best=row.best,applied=routeIsUsable(permanentRouteFor(row.key));return `<article class="resolverResult ${row.conflict?'conflict':row.autoSafe?'safe':best?'review':'missing'}"><div class="resolverTitle"><div><b>${esc(row.name)}</b><small>${esc(row.account)} · ${esc(row.assetType)}</small></div><span>${applied?'Permanent':best?`${num(row.confidence)}%`:'Ingen träff'}</span></div>${best?`<div class="resolverBest"><strong>${esc(best.ticker||best.isin)}</strong><p>${esc(best.name||'')} · ${esc(best.exchange||'Okänd börs')} · ${esc(best.currency||row.currency||'')}</p><small>${esc(best.provider)}${row.conflict?' · konflikt mellan källor':''}</small></div><div class="resolverActions"><button class="primary" data-resolver-apply="${esc(row.key)}" ${applied?'disabled':''}>${applied?'Sparad permanent':'Godkänn och spara permanent'}</button>${row.candidates.length>1?`<select data-resolver-choice="${esc(row.key)}">${row.candidates.map((c,i)=>`<option value="${i}">${esc(c.ticker||c.isin)} · ${esc(c.provider)} · ${num(c.score)}%</option>`).join('')}</select>`:''}</div>`:`<div class="notice">Ingen säker identitet hittades. ${esc((row.errors||[]).join(' · ')||'Ingen provider gav användbar träff.')}</div>`}</article>`}).join('');return `<div class="eyebrow">Global Identity Resolver 11.15.6</div><h2>Verifiera resultat</h2><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`}
function openGlobalResolverReview(){openModal(resolverReviewHtml());document.querySelectorAll('[data-resolver-apply]').forEach(btn=>btn.addEventListener('click',()=>{const key=btn.dataset.resolverApply,sel=document.querySelector(`[data-resolver-choice="${CSS.escape(key)}"]`),idx=num(sel?.value);if(applyResolverSuggestion(key,idx)){refreshValuationAfterResolver();openGlobalResolverReview()}}))}
function globalResolverPanel(){const st=globalResolverState(),run=resolverRunState(),remaining=run.total?Math.max(0,run.total-run.processed):resolverTargets().length,saved=Object.keys(permanentIdentityRegistry().routes||{}).length,running=run.status==='running',paused=run.status==='paused';return `<section class="section globalResolverPanel"><div class="sectionHead"><div><div class="eyebrow">Global Identity Resolver 11.15.6</div><h2>Stabil batchmotor för Android/PWA.</h2></div><span class="healthScore">${saved} låsta</span></div><p>Resolvern arbetar i batcher om högst åtta instrument, sparar checkpoint efter varje instrument och börjar alltid med lokal identitet innan externa providers anropas.</p><div class="grid"><div class="card metric"><span>Återstår</span><b id="resolverRemaining">${remaining}</b></div><div class="card metric"><span>Permanent sparade</span><b id="resolverPermanent">${saved}</b></div><div class="card metric"><span>Nya säkra</span><b id="resolverSafe">${run.newSafe||0}</b></div><div class="card metric"><span>För granskning</span><b id="resolverReview">${run.review||0}</b></div><div class="card metric"><span>Överhoppade</span><b id="resolverSkipped">${run.skipped||0}</b></div><div class="card metric"><span>Misslyckade</span><b id="resolverFailed">${run.failed||0}</b></div></div><div class="resolverProgress"><div><span>${running?'Kör':paused?'Pausad':'Redo'}</span><b><span id="resolverProcessed">${run.processed||0}</span>/<span id="resolverTotal">${run.total||0}</span></b></div><div class="bar"><i id="resolverProgressBar" style="width:${run.total?Math.round((run.processed||0)/run.total*100):0}%"></i></div><small id="resolverMessage">${esc(run.message||'Kör en liten stabil batch när du är redo.')}</small></div><div class="foundationActions"><a id="runGlobalResolver" class="primary resolverActionLink ${running?'isDisabled':''}" href="${running?'#market':'#resolver-run'}" role="button" aria-disabled="${running?'true':'false'}">${running?'Batch körs…':paused?'Fortsätt nästa batch (max 8)':'Kör nästa batch (max 8)'}</a><button id="stopGlobalResolver" class="danger" type="button" onclick="return window.mkStopResolver(event)" ${running?'':'disabled'}>Stoppa säkert</button><button id="reviewGlobalResolver" class="secondary" type="button" onclick="return window.mkReviewResolver(event)" ${(run.suggestions||[]).length?'':'disabled'}>Granska resultat</button></div><div class="resolverSources"><span>Lokal först</span><span>Permanent Registry</span><span>Global Registry</span><span>OpenFIGI</span><span>Twelve Data</span><span>Alpha Vantage</span><span>Finnhub</span></div><p class="disclaimer">Inga automatiska tunga resolveranrop görs vid appstart. Antal, GAV, kredit, transaktioner och Portfolio Ledger ändras aldrig.</p></section>`}
...
function handleResolverHashAction(){
 if(location.hash!=='#resolver-run')return false;
 history.replaceState(null,'','#market');
 screen='market';
 const action=document.getElementById('runGlobalResolver');
 if(action){action.classList.add('isDisabled');action.setAttribute('aria-disabled','true');action.removeAttribute('href');action.textContent='Startar batch…'}
 setTimeout(()=>startResolverBatchFromUI(),0);
 return true;
}
...
function installInteractionRouter(){
 ...
 document.addEventListener('click',event=>{
  ...
  if(!target)return;
  if(target.id==='runGlobalResolver'){event.preventDefault();event.stopImmediatePropagation();startResolverBatchFromUI();return}
  if(target.id==='stopGlobalResolver'){event.preventDefault();event.stopImmediatePropagation();requestResolverStop();return}
  if(target.id==='reviewGlobalResolver'){event.preventDefault();event.stopImmediatePropagation();openGlobalResolverReview();return}
...
function boot(){
 ...
 if(location.hash==='#resolver-run')setTimeout(handleResolverHashAction,0);
 
 window.addEventListener('hashchange',()=>{if(handleResolverHashAction())return;const next=location.hash.slice(1)||'home';if(NAV.some(x=>x[0]===next)){screen=next;render();requestAnimationFrame(()=>scrollTo({top:0,left:0,behavior:'auto'}))}});
