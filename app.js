...
async function runStorageCleanupMigration({silent=false}={}){
 const before=storageFootprint(),rows=localStorageEntries(),migrated=[],removed=[],kept=[],errors=[];
 for(const row of rows){
  if(protectedStorageKey(row.key)){kept.push(row.key);continue}
  if(disposableStorageKey(row.key)){
   try{localStorage.removeItem(row.key);removed.push(`${row.key} (${Math.round(row.size/1024)} KB)`)}catch(error){errors.push(`${row.key}: ${storageErrorText(error)}`)}
   continue
  }
  if(row.size>120000){
   const result=await migrateLargeValueToIndexedDb(row);
   if(result.ok)migrated.push(`${row.key} (${Math.round(row.size/1024)} KB)`);
   else errors.push(`${row.key}: ${result.error}`)
  }else kept.push(row.key)
 }
 const after=storageFootprint(),freedKb=Math.max(0,before.kb-after.kb);
 const result=persistStorageMigration({status:freedKb>=256?'success':(freedKb>0?'partial':'warning'),beforeKb:before.kb,afterKb:after.kb,freedKb,migrated,removed,kept:kept.slice(0,80),errors});
 resolverDiag('Storage Cleanup 11.15.37',`${before.kb} KB → ${after.kb} KB · frigjort ${freedKb} KB`,result.status==='success'?'success':'warning');
 if(!silent){
  alert(`Storage Cleanup klar.\n\nFöre: ${before.kb} KB\nEfter: ${after.kb} KB\nFrigjort: ${freedKb} KB\nMigrerade: ${migrated.length}\nRensade: ${removed.length}\nFel: ${errors.length}`);
 }
...
function resolverReviewHtml(){
 ...
 return `<div class="eyebrow">Global Identity Resolver 11.15.39</div><h2>ISIN-first Candidate Inspector</h2><p class="disclaimer">ISIN är primär identitetsnyckel. Ticker, börs, valuta och instrumenttyp används som sekundära kontroller. Kandidater utan ISIN kan granskas men räknas inte som verifierade identiteter.</p><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`
}
...
async function boot(){
 ...
 if(storageMigrationState().status==='not-run')await runStorageCleanupMigration({silent:true});
 refreshUnifiedIdentity();setTimeout(()=>{canonicalCache=null;refreshUnifiedIdentity({renderAfter:true})},350);
