...
function permanentIdentityRegistry(){return merge({version:'11.15.1',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.1';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function savePermanentIdentityRoute(key,route,meta={}){
 ...
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.1',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.1',source:'permanent-identity-registry-11.15.1',updatedAt:new Date().toISOString()};restored++}
 }
 ...
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.1',`${restored} rutter återställda · ${migrated} migrerade`,{})}
...
function resolverTargets(){return instrumentMappingCoverage().items.filter(item=>{const saved=permanentRouteFor(item.key);if(routeIsUsable(saved))return false;const m=liveFoundation.mappings[item.key]||{};const cls=classifyInstrument(canonicalHoldings().find(h=>h._key===item.key)||item).code;return !m.verified||num(m.identityConfidence||m.confidence)<92||(!m.ticker&&!m.isin)||(!m.isin&&cls!=='fund'&&!m.ticker)})}
...
async function runGlobalResolver(){const btn=document.getElementById('runGlobalResolver');if(btn){btn.disabled=true;btn.textContent='Verifierar 0/'+resolverTargets().length+'…'}const targets=resolverTargets(),suggestions=[];let errors=0;for(let i=0;i<targets.length;i+=4){const batch=await Promise.all(targets.slice(i,i+4).map(resolveOneInstrument));suggestions.push(...batch);errors+=batch.reduce((n,x)=>n+x.errors.length,0);if(btn)btn.textContent='Verifierar '+Math.min(i+4,targets.length)+'/'+targets.length+'…'}const resolved=suggestions.filter(x=>x.autoSafe).length,review=suggestions.filter(x=>x.best&&!x.autoSafe).length,unresolved=suggestions.filter(x=>!x.best).length;const previous=globalResolverState(),state={...previous,version:'11.15.1',status:errors&&resolved+review===0?'error':unresolved?'partial':'success',lastRun:new Date().toISOString(),scanned:targets.length,resolved,review,unresolved,errors,suggestions,audit:[{date:new Date().toISOString(),scanned:targets.length,resolved,review,unresolved,errors},...(previous.audit||[])].slice(0,100)};persistGlobalResolver(state);let autoSaved=0;for(const row of suggestions){if(row.autoSafe&&!row.applied&&applyResolverSuggestion(row.key,0,{silent:true}))autoSaved++}liveEvent(state.status==='success'?'success':'warning','Global Identity Resolver 11.15.1',`${autoSaved} säkra sparade permanent · ${review} för granskning · ${unresolved} utan träff`,{errors});render();openGlobalResolverReview()}
function applyResolverSuggestion(key,index=0,{silent=false}={}){const state=globalResolverState(),row=(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.1'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.1'],source:'permanent-identity-registry-11.15.1',resolverVersion:'11.15.1',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;row.applied=true;row.appliedAt=new Date().toISOString();row.appliedCandidate=index;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.1',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
...
function globalResolverPanel(){const st=globalResolverState(),coverage=instrumentMappingCoverage(),remaining=resolverTargets().length,saved=Object.keys(permanentIdentityRegistry().routes||{}).length;return `<section class="section globalResolverPanel"><div class="sectionHead"><div><div class="eyebrow">Global Identity Resolver 11.15.1</div><h2>Rätt instrument. Rätt börs. Permanent rutt.</h2></div><span class="healthScore">${coverage.complete}/${coverage.total}</span></div><p>Motorn verifierar identitet med ISIN, alternativa tickerformat, börsvarianter och flera providers. Den bästa fungerande rutten sparas permanent och återanvänds vid framtida live-synk.</p><div class="grid"><div class="card metric"><span>Återstår att lösa</span><b>${remaining}</b></div><div class="card metric"><span>Permanent sparade</span><b>${saved}</b></div><div class="card metric"><span>För granskning</span><b>${st.review}</b></div><div class="card metric"><span>Konflikter</span><b>${(st.suggestions||[]).filter(x=>x.conflict).length}</b></div></div><div class="foundationActions"><button id="runGlobalResolver" class="primary" type="button">Kör Global Identity Resolver 11.15.1</button><button id="reviewGlobalResolver" class="secondary" type="button" ${(st.suggestions||[]).length?'':'disabled'}>Granska och godkänn</button></div><div class="resolverSources"><span>Global Registry</span><span>OpenFIGI</span><span>Twelve Data</span><span>Alpha Vantage</span><span>Finnhub</span><span>ISIN + börsvarianter</span></div><p class="disclaimer">Antal, GAV, marknadsvärde, kredit, transaktioner och Portfolio Ledger ändras aldrig.</p></section>`}
