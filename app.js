...
async function fetchWithPolicy(url,{provider='unknown',timeoutMs=9000,retries=1,headers={}}={}){
 ...
 for(let attempt=0;attempt<=retries;attempt++){
  ...
  try{
   ...
   updateProviderTelemetry(provider,{...latest,successes:num(latest.successes)+1,lastSuccess:new Date().toISOString(),lastError:'',latencyMs:latency,status:'online'});
   return response;
  }catch(error){
   clearTimeout(timer);lastError=error;const latest=providerTelemetry()[provider]||{};
   updateProviderTelemetry(provider,{...latest,failures:num(latest.failures)+1,lastError:error?.name==='AbortError'?'timeout':String(error?.message||error),latencyMs:Math.round(performance.now()-started),status:attempt<retries?'retrying':'degraded'});
   if(attempt<retries)await new Promise(resolve=>setTimeout(resolve,450*(attempt+1)));
  }
 }
 throw lastError||new Error('Providerfel');
}
function providerHealthSummary(){const items=Object.values(providerTelemetry()).map(p=>({...p,reliability:providerReliability(p)}));return {items,active:items.filter(p=>p.status==='online').length,degraded:items.filter(p=>p.status==='degraded'||p.status==='retrying').length,score:items.length?Math.round(items.reduce((s,p)=>s+p.reliability,0)/items.length):0}}
...
const SAFE_INSTRUMENTS=[
 [/alibaba/,{ticker:'BABA',currency:'USD',exchange:'NYSE'}],[/amazon/,{ticker:'AMZN',currency:'USD',exchange:'NASDAQ'}],[/apple/,{ticker:'AAPL',currency:'USD',exchange:'NASDAQ'}],[/microsoft/,{ticker:'MSFT',currency:'USD',exchange:'NASDAQ'}],[/alphabet|google/,{ticker:'GOOGL',currency:'USD',exchange:'NASDAQ'}],[/nvidia/,{ticker:'NVDA',currency:'USD',exchange:'NASDAQ'}],[/berkshire.*b/,{ticker:'BRK.B',currency:'USD',exchange:'NYSE'}],[/pepsico|pepsi/,{ticker:'PEP',currency:'USD',exchange:'NASDAQ'}],[/coca.?cola/,{ticker:'KO',currency:'USD',exchange:'NYSE'}],[/realty income/,{ticker:'O',currency:'USD',exchange:'NYSE'}],[/stag industrial/,{ticker:'STAG',currency:'USD',exchange:'NYSE'}],[/united parcel|\bups\b/,{ticker:'UPS',currency:'USD',exchange:'NYSE'}],[/main street capital/,{ticker:'MAIN',currency:'USD',exchange:'NYSE'}],[/abbvie/,{ticker:'ABBV',currency:'USD',exchange:'NYSE'}],[/johnson.*johnson/,{ticker:'JNJ',currency:'USD',exchange:'NYSE'}],[/procter.*gamble/,{ticker:'PG',currency:'USD',exchange:'NYSE'}],[/texas instruments/,{ticker:'TXN',currency:'USD',exchange:'NASDAQ'}],[/broadcom/,{ticker:'AVGO',currency:'USD',exchange:'NASDAQ'}],[/visa/,{ticker:'V',currency:'USD',exchange:'NYSE'}],[/mastercard/,{ticker:'MA',currency:'USD',exchange:'NYSE'}],
 [/investor.*b/,{ticker:'INVE B',currency:'SEK',exchange:'OMX'}],[/industrivarden.*c/,{ticker:'INDU C',currency:'SEK',exchange:'OMX'}],[/volvo.*b/,{ticker:'VOLV B',currency:'SEK',exchange:'OMX'}],[/atlas copco.*b/,{ticker:'ATCO B',currency:'SEK',exchange:'OMX'}],[/essity.*b/,{ticker:'ESSITY B',currency:'SEK',exchange:'OMX'}],[/evolution/,{ticker:'EVO',currency:'SEK',exchange:'OMX'}],[/swedbank.*a/,{ticker:'SWED A',currency:'SEK',exchange:'OMX'}],[/handelsbanken.*a/,{ticker:'SHB A',currency:'SEK',exchange:'OMX'}],[/seb.*a/,{ticker:'SEB A',currency:'SEK',exchange:'OMX'}],[/nordea/,{ticker:'NDA SE',currency:'SEK',exchange:'OMX'}],[/telia/,{ticker:'TELIA',currency:'SEK',exchange:'OMX'}],[/tele2.*b/,{ticker:'TEL2 B',currency:'SEK',exchange:'OMX'}],[/castellum/,{ticker:'CAST',currency:'SEK',exchange:'OMX'}],[/xact.*norden.*hogutdel/,{ticker:'XACTHDIV',currency:'SEK',exchange:'OMX'}],[/novo nordisk/,{ticker:'NOVO B',currency:'DKK',exchange:'OMXCOP'}]
];
function mappingSuggestionFor(item){const n=normalizeInstrumentName(item.name),match=SAFE_INSTRUMENTS.find(([rx])=>rx.test(n));return match?{...match[1],confidence:99,reason:'Verifierad lokal namnmatchning'}:null}
...
function providerConfig(){return merge({twelveDataKey:'',alphaVantageKey:'',enableStooq:true,maxSymbolsPerSync:30},parse(safeGet(PROVIDER_CONFIG_KEY))||{})}
function persistProviderConfig(x){safeSet(PROVIDER_CONFIG_KEY,JSON.stringify(x))}
function providerRouteLabel(){const c=providerConfig(),a=[];if(c.twelveDataKey)a.push('Twelve Data');if(c.alphaVantageKey)a.push('Alpha Vantage');if(c.enableStooq)a.push('Stooq');a.push('Local Cache');return a.join(' → ')}
...
function market(){
 ...
 return `<section class="hero"><div class="eyebrow">Live Intelligence 11.3.0</div><h2>Marknad & live-data</h2><p>Read-only marknadsdata från flera utbytbara providers. Din lokala portföljdata ändras aldrig av live-lagret.</p><div class="liveHeroStats"><div><span>Synkstatus</span><b>${esc(sync.status)}</b></div><div><span>Senaste synk</span><b>${sync.lastSuccess?new Date(sync.lastSuccess).toLocaleString('sv-SE'):'Aldrig'}</b></div><div><span>Instrumentmappning</span><b>${mapping.complete}/${mapping.total}</b></div><div><span>Cache</span><b>${cache.items} kurser</b></div></div></section>
 <section class="section liveSyncPanel"><div class="sectionHead"><div><div class="eyebrow">Smart Provider Router</div><h2>Synkronisering</h2></div><span class="syncStatus ${esc(sync.status)}">${esc(sync.status)}</span></div><p>${esc(sync.message)}</p><div class="foundationActions"><button id="syncLive" class="primary" type="button">Synkronisera live-data</button><button id="providerSettings" class="secondary" type="button">Providerinställningar</button><button id="discoverMappings" class="primary" type="button">Identifiera instrument automatiskt</button><button id="reviewMappings" class="secondary" type="button">Granska instrumentmappning</button><button id="suggestMappings" class="secondary" type="button">Lägg till säkra tickerförslag</button><button id="validateLive" class="secondary" type="button">Validera cache</button></div><p class="routeLine"><b>Aktiv rutt:</b> ${esc(providerRouteLabel())}</p>${sync.errors?.length?`<div class="warning"><b>Senaste fel</b><p>${sync.errors.map(esc).join('<br>')}</p></div>`:''}</section>
