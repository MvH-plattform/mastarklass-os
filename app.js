...
function permanentIdentityRegistry(){return merge({version:'11.15.22',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.22';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function savePermanentIdentityRoute(key,route,meta={}){
 ...
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.22',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.22',source:'permanent-identity-registry-11.15.22',updatedAt:new Date().toISOString()};restored++}
 }
 ...
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.22',`${restored} rutter återställda · ${migrated} migrerade`,{})}
...
function resolverRunState(){return merge({version:'11.15.22',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],suggestions:[]},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.22';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function refreshValuationAfterResolver(){try{liveValuationSnapshot({persist:true});runDataConfidenceEngine({reason:'resolver-11.15.22'});runAutonomousPortfolioScan({reason:'resolver-11.15.22',force:true})}catch(error){console.warn('Efteranalys kunde inte köras',error)}}
...
async function runGlobalResolver(){
 ...
 liveEvent(run.failed?'warning':'success','Global Identity Resolver 11.15.22',run.message,{skipped:run.skipped,permanent:run.permanent,errors:run.errors});
 render();return true
}
function syncResolverResultsFromRun(run){
 const previous=globalResolverState();
 const suggestions=(run.suggestions||[]).filter(x=>!routeIsUsable(permanentRouteFor(x.key)));
 persistGlobalResolver({...previous,version:'11.15.22',status:run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:suggestions.filter(x=>x.best).length,unresolved:suggestions.filter(x=>!x.best).length,errors:run.errors,suggestions,routes:previous.routes||{},audit:[{date:new Date().toISOString(),status:run.status,scanned:run.processed,resolved:run.newSafe,review:suggestions.filter(x=>x.best).length,unresolved:suggestions.filter(x=>!x.best).length,errors:run.errors},...(previous.audit||[])].slice(0,100)});
}
...
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){const state=globalResolverState(),row=rowOverride||(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.22'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.22'],source:'permanent-identity-registry-11.15.22',resolverVersion:'11.15.22',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;row.applied=true;row.appliedAt=new Date().toISOString();row.appliedCandidate=index;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.22',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
...
/* 11.15.22 Permanent Registry Save Fix */
...
function verifiedPermanentWrite(key,route,meta={}){
 ...
 try{
  ...
  registry.routes=registry.routes||{};
  registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.22',savedAt:now,source:meta.source||route.source||'global-resolver-approved'};
  ...
  registry.version='11.15.22';registry.updatedAt=now;
...
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){
 ...
 const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:num(candidate.score),verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.22'};
 const written=verifiedPermanentWrite(key,route,{source:'global-resolver-approved-11.15.22'});
 if(!written.ok){if(!silent)alert(`Kunde inte spara permanent: ${written.error}`);return false}
 liveFoundation.mappings[key]={...current,...written.route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:num(candidate.score),identityReasons:[candidate.provider,'Verifierad i Permanent Registry 11.15.22'],source:'permanent-identity-registry-11.15.22',resolverVersion:'11.15.22',permanentRoute:written.route,updatedAt:new Date().toISOString()};
 persistLiveFoundation();
 ...
 liveEvent('success','Permanent Registry Save 11.15.22',run.message,written.route);
 return true;
}
...
function boot(){
 ...
 setTimeout(()=>runAutomaticIntelligenceCycle('app-open'),1500);
