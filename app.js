...
function runAutonomousPortfolioAnalysis({reason='automatic'}={}){
 ...
 const next={...state,lastRun:new Date().toISOString(),status:'success',message:`Datatillit ${confidence.portfolioScore}/100 · ${confidence.manualReview} kräver manuell kontroll · Portfolio Intelligence ${ip.grade} ${ip.score}`,runs:num(state.runs)+1,reason};
 persistAutonomousIntelligence(next);return {confidence,intelligence:ip,state:next};
}
