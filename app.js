...
const RESOLVER_RUN_KEY='mastarklass_os_11_15_5_resolver_run';
...
function permanentIdentityRegistry(){return merge({version:'11.15.5',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.5';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.5',source:'permanent-identity-registry-11.15.5',updatedAt:new Date().toISOString()};restored++}
 }
...
function resolverRunState(){return merge({version:'11.15.5',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],suggestions:[],message:''},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.5';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.5',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
async function startResolverBatchFromUI(){
 ...
 if(button){button.disabled=true;button.textContent='Startar batch…'}
 try{
  ...
  const next=document.getElementById('runGlobalResolver');if(next&&resolverRunState().status!=='running')next.disabled=false;
 }
}
...
function boot(){
 ...
 if('requestIdleCallback'in window)requestIdleCallback(()=>{try{runAutonomousPortfolioAnalysis({reason:'idle-open'})}catch(e){}},{timeout:2500});
