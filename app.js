...
function permanentIdentityRegistry(){
 const raw=parse(safeGet(PERMANENT_IDENTITY_KEY));
 const registry=merge({version:'11.15.25',updatedAt:'',routes:{},audit:[]},raw&&typeof raw==='object'?raw:{});
...
function persistPermanentIdentityRegistry(x,{allowRecovery=true}={}){
 x.version='11.15.25';x.updatedAt=new Date().toISOString();x.audit=(x.audit||[]).slice(0,120);
 const payload=JSON.stringify(x),before=storageFootprint();
 resolverDiag('Permanent write 04 Skrivning start',`nyckel=${PERMANENT_IDENTITY_KEY} · ${payload.length} tecken · localStorage≈${before.kb} KB`);
 let ok=safeSet(PERMANENT_IDENTITY_KEY,payload);
 if(!ok&&allowRecovery){
  resolverDiag('Permanent write 05 Lagringstryck',`setItem misslyckades · localStorage≈${before.kb} KB`,'warning');
  const actions=compactReadOnlyStorageForRegistry();resolverDiag('Permanent write 06 Säker komprimering',actions.join(' · ')||'inget att komprimera','warning');
  ok=safeSet(PERMANENT_IDENTITY_KEY,payload)
 }
 if(!ok){resolverDiag('Permanent write 07 Skrivning misslyckades',`nyckel=${PERMANENT_IDENTITY_KEY}`,'error');return false}
 const raw=safeGet(PERMANENT_IDENTITY_KEY),read=parse(raw);
 const verified=Boolean(read&&read.routes&&typeof read.routes==='object');
 resolverDiag(verified?'Permanent write 08 Direkt återläsning klar':'Permanent write 08 Direkt återläsning fel',`nyckel=${PERMANENT_IDENTITY_KEY} · routes=${Object.keys(read?.routes||{}).length} · rå=${raw?.length||0} tecken`,verified?'success':'error');
 return verified
}
...
function savePermanentIdentityRoute(key,route,meta={}){
 resolverDiag('Permanent write 01 Start',`holdingKey=${key||'–'} · ticker=${route?.ticker||'–'} · isin=${route?.isin||'–'}`);
 ...
 resolverDiag('Permanent write 03 Register läst',`${Object.keys(registry.routes||{}).length} poster före skrivning · nyckel=${PERMANENT_IDENTITY_KEY}`);
 registry.routes=registry.routes||{};
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.25',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
 ...
 if(!persistPermanentIdentityRegistry(registry))return false;
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.25',source:'permanent-identity-registry-11.15.25',updatedAt:new Date().toISOString()};restored++}
 }
...
function resolverDiagnosticState(){
 ...
 const meta=merge({version:'11.15.25',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false,storage:resolverTraceStorage},raw);
 meta.lines=resolverDiagnosticLines();meta.storage=resolverTraceStorage;return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.25',startedAt:x?.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x?.heartbeat),currentInstrument:x?.currentInstrument||'',currentStep:x?.currentStep||'',expanded:!!x?.expanded,storage:resolverTraceStorage};
...
function resolverRunDefaults(){return {version:'11.15.25',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,fundPending:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],message:''}}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.25',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,fundPending:run.fundPending||0,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){
 ...
 if(!savePermanentIdentityRoute(key,route,{source:'global-resolver-approved-11.15.25'})){if(!silent)alert('Permanent lagring misslyckades. Read-only cache och diagnostik har komprimerats säkert och ett nytt försök gjordes. Instrumentet ligger kvar. Kopiera Resolver Chain Trace för exakt fel.');return false}
 ...
 liveEvent('success','Permanent Identity Registry 11.15.25',`${row.name} sparad permanent som ${route.ticker||route.isin} · ${route.exchange||'okänd börs'} · ${route.currency}`,verified);
...
function openGlobalResolverReview(){
 ...
 document.querySelectorAll('[data-resolver-apply]').forEach(btn=>btn.addEventListener('click',event=>{
  ...
  const saved=applyResolverSuggestion(key,idx);
