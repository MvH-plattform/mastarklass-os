...
const RESOLVER_DIAG_KEY='mastarklass_os_11_15_11_resolver_diag';
...
function permanentIdentityRegistry(){return merge({version:'11.15.11',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.11';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.11',source:'permanent-identity-registry-11.15.11',updatedAt:new Date().toISOString()};restored++}
 }
...
function resolverDiagnosticState(){return merge({version:'11.15.11',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',lines:[]},parse(safeGet(RESOLVER_DIAG_KEY))||{})}
function persistResolverDiagnostic(x){x.version='11.15.11';x.updatedAt=new Date().toISOString();x.lines=(x.lines||[]).slice(-120);safeSet(RESOLVER_DIAG_KEY,JSON.stringify(x));return x}
function resolverDiag(step,detail='',level='info'){
 const d=resolverDiagnosticState(),time=new Date().toLocaleTimeString('sv-SE',{hour:'2-digit',minute:'2-digit',second:'2-digit'}),line={time,step:String(step||''),detail:String(detail||''),level};
 d.currentStep=line.step;d.lines=[...(d.lines||[]),line].slice(-120);persistResolverDiagnostic(d);updateResolverDiagnosticUI(d);console.log('[Resolver diagnostics]',line.step,line.detail);return line
}
function resetResolverDiagnostics(){const d={version:'11.15.11',startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),heartbeat:0,currentInstrument:'',currentStep:'Start',lines:[]};persistResolverDiagnostic(d);resolverDiag('Diagnostik startad','Väntar på batch');return d}
function updateResolverDiagnosticUI(d=resolverDiagnosticState()){
 const log=document.getElementById('resolverDiagnosticLog'),beat=document.getElementById('resolverHeartbeat'),step=document.getElementById('resolverCurrentStep'),instrument=document.getElementById('resolverCurrentInstrument');
 if(beat)beat.textContent=String(d.heartbeat||0);if(step)step.textContent=d.currentStep||'Redo';if(instrument)instrument.textContent=d.currentInstrument||'–';
 if(log){log.innerHTML=(d.lines||[]).slice(-40).map(x=>`<div class="diagLine ${esc(x.level||'info')}"><b>${esc(x.time)}</b> ${esc(x.step)}${x.detail?`<small>${esc(x.detail)}</small>`:''}</div>`).join('')||'<div class="empty">Ingen diagnostik ännu.</div>';log.scrollTop=log.scrollHeight}
}
function startResolverHeartbeat(){stopResolverHeartbeat();resolverHeartbeatTimer=setInterval(()=>{const d=resolverDiagnosticState();d.heartbeat=num(d.heartbeat)+1;persistResolverDiagnostic(d);updateResolverDiagnosticUI(d)},1000)}
...
async function copyResolverDiagnostics(){const d=resolverDiagnosticState(),run=resolverRunState(),text=[`Mästarklass OS ${VERSION} Resolverdiagnostik`,`Status: ${run.status} ${run.processed}/${run.total}`,`Instrument: ${d.currentInstrument||'–'}`,`Steg: ${d.currentStep||'–'}`,...(d.lines||[]).map(x=>`${x.time} ${x.step}${x.detail?' — '+x.detail:''}`)].join('\n');try{await navigator.clipboard.writeText(text);resolverDiag('Logg kopierad',`${d.lines?.length||0} rader`)}catch(error){resolverDiag('Kopiering misslyckades',String(error?.message||error),'error')}return false}
...
function resolverRunState(){return merge({version:'11.15.11',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],message:''},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.11';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.11',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){const state=globalResolverState(),run=resolverRunState(),row=rowOverride||(run.suggestions||[]).find(x=>x.key===key)||(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.11'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.11'],source:'permanent-identity-registry-11.15.11',resolverVersion:'11.15.11',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.8',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
