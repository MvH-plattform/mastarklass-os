...
function permanentIdentityRegistry(){return merge({version:'11.15.24',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.24';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.24',source:'permanent-identity-registry-11.15.24',updatedAt:new Date().toISOString()};restored++}
 }
...
function resolverDiagnosticState(){
 ...
 const meta=merge({version:'11.15.24',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false,storage:resolverTraceStorage},raw);
 meta.lines=resolverDiagnosticLines();meta.storage=resolverTraceStorage;return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.24',startedAt:x?.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x?.heartbeat),currentInstrument:x?.currentInstrument||'',currentStep:x?.currentStep||'',expanded:!!x?.expanded,storage:resolverTraceStorage};
...
function resolverRunDefaults(){return {version:'11.15.24',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,fundPending:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],message:''}}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.24',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,fundPending:run.fundPending||0,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){
 ...
 const route={provider:candidate.provider,ticker:String(candidate.ticker||'').trim(),isin:String(candidate.isin||'').trim(),exchange:String(candidate.exchange||'').trim(),currency:String(candidate.currency||row.currency||'SEK').trim(),figi:candidate.figi||'',confidence:num(candidate.score),verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.24'};
 if(route.ticker==='0'||!routeIsUsable(route)){if(!silent)alert('Kandidaten kunde inte sparas: ticker/ISIN, provider eller valuta saknas.');return false}
 const holding=canonicalHoldings().find(h=>h._key===key)||row;
 liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:route.ticker||route.isin},instrumentClass:classifyInstrument(holding).code,verified:true,identityStatus:'verified',identityConfidence:route.confidence,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.24'],source:'permanent-identity-registry-11.15.24',resolverVersion:'11.15.24',permanentRoute:route,updatedAt:new Date().toISOString()};
 persistLiveFoundation();
 if(!savePermanentIdentityRoute(key,route,{source:'global-resolver-approved-11.15.24'})){if(!silent)alert('Permanent lagring misslyckades. Instrumentet ligger kvar för granskning.');return false}
 const verified=permanentRouteFor(key);
 if(!routeIsUsable(verified)){if(!silent)alert('Lagringen kunde inte verifieras genom direkt återläsning. Instrumentet ligger kvar.');return false}
 ...
 liveEvent('success','Permanent Identity Registry 11.15.24',`${row.name} sparad permanent som ${route.ticker||route.isin} · ${route.exchange||'okänd börs'} · ${route.currency}`,verified);
