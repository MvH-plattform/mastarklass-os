...
function resolverTargets(){return instrumentMappingCoverage().items.filter(item=>{const m=liveFoundation.mappings[item.key]||{};return !m.verified||num(m.identityConfidence||m.confidence)<92||!m.ticker||(!m.isin&&classifyInstrument(canonicalHoldings().find(h=>h._key===item.key)||item).code!=='fund')})}
