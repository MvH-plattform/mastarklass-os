...
function resolverInteractionState(){return merge({version:'11.15.42',lastSync:'',lastConflictOpen:'',syncStatus:'idle',conflictStatus:'idle',lastError:''},parse(safeGet(RESOLVER_INTERACTION_KEY))||{})}
function persistResolverInteraction(x){x.version='11.15.42';safeSet(RESOLVER_INTERACTION_KEY,JSON.stringify(x));return x}
...
async function runPortfolioBridgeWithFeedback(event){
 ...
 resolverDiag('Portfolio Identity Bridge 11.15.42 knapptryck','Synk-knapp mottagen','info');
 ...
 try{
  ...
  resolverDiag('Portfolio Identity Bridge 11.15.42 klar',`${r.withIsin}/${r.total} med ISIN · ${r.withoutIsin} saknar · ${r.conflicts.length} konflikter`,'success');
  ...
  resolverDiag('Portfolio Identity Bridge 11.15.42 fel',message,'error');
...
function openConflictResolverWithFeedback(event){
 ...
 event?.preventDefault?.();event?.stopPropagation?.();
 resolverDiag('Conflict Resolver 11.15.42 knapptryck','Konfliktknapp mottagen','info');
 ...
 try{
  ...
  resolverDiag('Conflict Resolver 11.15.42 modal öppnad',`${report.withIsin}/${report.total} med ISIN · ${report.withoutIsin} saknar · ${report.conflicts.length} konflikter`,'success');
  ...
  resolverDiag('Conflict Resolver 11.15.42 fel',message,'error');
...
function buildUnifiedIdentityReport(){
 ...
 for(const [isin,rows] of byIsin){
  const names=new Set(rows.map(r=>normalizeInstrumentName(r.name)));
  if(names.size>1)conflicts.push({isin,rows})
 }
 const report={
 ...
 persistIdentityConflictState({status:report.status,withIsin:report.withIsin,withoutIsin:report.withoutIsin,conflicts:report.conflicts,missing:report.missing});
...
function identityConflictState(){return merge({version:'11.15.38',lastRun:'',status:'not-run',withIsin:0,withoutIsin:0,conflicts:[],missing:[]},parse(safeGet(IDENTITY_CONFLICT_KEY))||{})}
...
function identityConflictRendererSelfTest(){
 try{
  ...
  resolverDiag('Conflict Renderer 11.15.42 self-test',ok?'escapeHtml OK':'escapeHtml gav oväntat resultat',ok?'success':'error');
  return ok
 }catch(error){
  resolverDiag('Conflict Renderer 11.15.42 self-test fel',storageErrorText(error),'error');
  return false
 }
}
...
function renderIdentityConflictDetails(){
 ...
 const conflicts=report.conflicts||[],missing=report.missing||[];
 const conflictHtml=conflicts.length?conflicts.map(c=>`<div class="identityConflictCard"><div><b>${escapeHtml(c.isin)}</b><span>${c.rows.length} innehav använder samma ISIN</span></div>${c.rows.map(r=>`<button type="button" onclick="return window.mkJumpToHoldingEditor(event,'${escapeHtml(r.key)}')"><strong>${escapeHtml(r.name)}</strong><small>${escapeHtml(r.account||'Okänt konto')} · ${escapeHtml(r.ticker||'ingen ticker')}</small></button>`).join('')}</div>`).join(''):`<div class="notice okbox">Inga ISIN-konflikter hittades.</div>`;
 ...
 openModal(`<div class="sectionHead"><div><h2>Unified Identity Engine 11.15.42</h2><p>${report.withIsin}/${report.total} med ISIN · ${report.withoutIsin} saknar · ${conflicts.length} konflikter</p></div></div><h3>ISIN-konflikter</h3>${conflictHtml}<h3 class="topGap">Saknar ISIN</h3><div class="missingIsinList">${missingHtml||'<div class="notice okbox">Alla innehav har ISIN.</div>'}</div>`);
 return report
}
...
function resolverReviewHtml(){
 ...
 return `<div class="eyebrow">Global Identity Resolver 11.15.42</div><h2>ISIN-first Candidate Inspector</h2><p class="disclaimer">ISIN är primär identitetsnyckel. Ticker, börs, valuta och instrumenttyp används som sekundära kontroller. Kandidater utan ISIN kan granskas men räknas inte som verifierade identiteter.</p><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`
}
