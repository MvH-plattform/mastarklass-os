...
async function fetchRoutedQuotes(){const c=providerConfig(),mappings=instrumentMappingCoverage().items.filter(x=>x.ticker).slice(0,Math.max(1,num(c.maxSymbolsPerSync)||30)),quotes=[],failures=[];for(const m of mappings){const adapters=[];m.routeTicker=m.providerSymbols||{};if(c.twelveDataKey)adapters.push(fetchTwelveDataQuote);if(c.alphaVantageKey)adapters.push(fetchAlphaVantageQuote);if(c.finnhubKey)adapters.push(fetchFinnhubQuote);if(c.enableStooq)adapters.push(fetchStooqQuote);let q=null,last='ingen aktiv prisprovider';for(const adapter of adapters){try{q=await adapter(m);break}catch(e){last=String(e?.message||e)}}if(q)quotes.push(q);else failures.push({name:m.name,ticker:m.ticker,error:last})}return {quotes,errors:failures.length,failures,attempted:mappings.length}}
...
function wealthDashboard(){const ip=intelligencePortfolio(),health=wealthHealth(ip),plan=allocationPlan(ip),coach=wealthCoach(ip,health,plan),goals=goalIntelligence(),risks=riskRadar(ip,health),registry=providerRegistry();return `<section class="hero wealthHero"><div class="eyebrow">Mästarklass OS 11.10.0 · Global Identity Resolver</div><h2>Din förmögenhetsplan, förklarad.</h2><p>Lokalt, spårbart och read-only. Wealth Intelligence använder Portfolio Ledger, masterdata och skyddsregler utan att skapa order eller ändra dina innehav.</p><div class="wealthHeroGrid"><div><span>Portfolio Health</span><b>${health.score}/100</b></div><div><span>Intelligence</span><b>${ip.grade} · ${ip.score}</b></div><div><span>Nästa kapital</span><b>${kr(plan.monthly)}</b></div><div class="liveState ${registry.code}"><span>Live-lager</span><b><i></i>${registry.status}</b></div></div></section><section class="section coachStack"><div class="sectionHead"><div><div class="eyebrow">Wealth Coach 11.1.2.1</div><h2>Prioriterade råd</h2></div><small>${coach.length} råd</small></div><div class="coachAdviceList">${coach.map((x,i)=>`<article class="coachAdvice ${x.tone}"><span>${i+1}</span><div><h3>${esc(x.title)}</h3><p>${esc(x.text)}</p></div></article>`).join('')}</div></section><section class="section"><h2>Portföljhälsa</h2><div class="healthGrid">${[['Diversifiering',health.diversification],['Belåning',health.leverage],['Koncentration',health.concentration],['Datakvalitet',health.data],['Utdelningsstyrka',health.dividend],['Likviditet',health.liquidity]].map(([l,v])=>`<div class="healthItem"><div><span>${l}</span><b>${v}</b></div><div class="bar"><i style="width:${v}%"></i></div></div>`).join('')}</div></section><section class="section"><h2>Målprognos</h2><div class="goalTimeline">${goals.map(g=>`<div class="goalForecast"><div><b>${g.label}</b><small>${pct(g.progress)} uppnått</small></div><strong>${fmtYears(g.years)}</strong></div>`).join('')}</div><p class="disclaimer">Prognosen använder 7 % antagen årlig avkastning och nuvarande månadssparande. Det är en simulering, inte en garanti.</p></section><section class="section"><h2>Riskradar</h2><div class="riskList">${risks.map(r=>`<div class="riskItem ${r.level}"><b>${esc(r.title)}</b><p>${esc(r.text)}</p></div>`).join('')}</div></section><section class="section"><h2>Möjlighetsmotor</h2>${plan.rows.length?`<div class="allocationList">${plan.rows.map((x,i)=>allocationRow(x,i+1)).join('')}</div>`:`<div class="notice">Ingen investeringskandidat passerar nuvarande skyddsregler. Kreditreduktion, buffert eller bättre data prioriteras.</div>`}<p class="disclaimer">Förslagen är lokala beslutsunderlag. Ingen handel utförs.</p></section>`}
...
/* 11.10.0 Global Identity Resolver — integrated, no extra JS/CSS files */
function globalResolverState(){return merge({version:'11.10.0',status:'idle',lastRun:'',scanned:0,resolved:0,review:0,unresolved:0,errors:0,suggestions:[],audit:[]},parse(safeGet(GLOBAL_RESOLVER_KEY))||{})}
...
function resolverScore(item,c){
 let score=nameSimilarity(item.name,c.name||c.ticker||'');
 const wantedCur=String(item.currency||'').toUpperCase(),gotCur=String(c.currency||'').toUpperCase();
 if(c.isin&&/^[A-Z]{2}[A-Z0-9]{9}[0-9]$/.test(String(c.isin).toUpperCase()))score+=18;
 if(wantedCur&&gotCur&&wantedCur===gotCur)score+=8;
 if(item.assetType&&c.type&&normalizeInstrumentName(item.assetType)===normalizeInstrumentName(c.type))score+=7;
 if(/fond|fund/i.test(item.assetType||'')&&/fund|mutual/i.test(c.type||''))score+=8;
 if(/etf/i.test(item.assetType||'')&&/etf/i.test(c.type||''))score+=8;
 if(/aktie|stock|equity/i.test(item.assetType||'')&&/stock|equity|common/i.test(c.type||''))score+=6;
 if(c.provider==='Global Registry')score=Math.max(score,98);
 if(c.provider==='OpenFIGI'&&c.isin)score+=4;
 return Math.max(0,Math.min(100,Math.round(score)));
}
async function searchOpenFigiInstrument(item){
 const cfg=providerConfig(),headers={'Content-Type':'application/json'};if(cfg.openFigiKey)headers['X-OPENFIGI-APIKEY']=cfg.openFigiKey;
 const res=await fetchWithPolicy('https://api.openfigi.com/v3/search',{provider:'openfigi',timeoutMs:10000,retries:0,headers});
 // fetchWithPolicy uses GET, so OpenFIGI search needs a direct POST with the same safety policy.
 throw new Error('OpenFIGI POST route');
}
async function searchOpenFigiPost(item){
 const cfg=providerConfig(),ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),10000),headers={'Content-Type':'application/json'};if(cfg.openFigiKey)headers['X-OPENFIGI-APIKEY']=cfg.openFigiKey;
 try{
  const res=await fetch('https://api.openfigi.com/v3/search',{method:'POST',headers,body:JSON.stringify({query:item.name}),signal:ctrl.signal,cache:'no-store'});clearTimeout(timer);
  if(!res.ok)throw new Error(`HTTP ${res.status}`);const x=await res.json();
  return (x.data||[]).slice(0,8).map(r=>({ticker:r.ticker||'',name:r.name||r.securityDescription||'',exchange:r.exchCode||r.marketSector||'',currency:r.currency||item.currency||'',type:r.securityType2||r.securityType||'',isin:(r.securityIdentifiers||[]).find(z=>z.idType==='ID_ISIN')?.idValue||'',figi:r.figi||'',provider:'OpenFIGI'}));
 }finally{clearTimeout(timer)}
}
async function searchFinnhubInstrument(item){const key=providerConfig().finnhubKey;if(!key)return[];const url=`https://finnhub.io/api/v1/search?q=${encodeURIComponent(item.name)}&token=${encodeURIComponent(key)}`;const res=await fetchWithPolicy(url,{provider:'finnhub',timeoutMs:8000,retries:0}),x=await res.json();return (x.result||[]).slice(0,8).map(r=>({ticker:r.symbol||'',name:r.description||'',exchange:r.primaryExchange||'',currency:item.currency||'',type:r.type||'',provider:'Finnhub'}))}
...
function resolverTargets(){return instrumentMappingCoverage().items.filter(item=>{const m=liveFoundation.mappings[item.key]||{};return !m.verified||num(m.identityConfidence||m.confidence)<90||!m.ticker||(!m.isin&&classifyInstrument(canonicalHoldings().find(h=>h._key===item.key)||item).code!=='fund')})}
async function resolveOneInstrument(item){
 let candidates=[...localResolverCandidates(item)],errors=[];
 const jobs=[];if(providerConfig().twelveDataKey)jobs.push(searchTwelveDataInstrument(item));if(providerConfig().alphaVantageKey)jobs.push(searchAlphaVantageInstrument(item));if(providerConfig().finnhubKey)jobs.push(searchFinnhubInstrument(item));jobs.push(searchOpenFigiPost(item));
 for(const job of jobs){try{candidates.push(...await job)}catch(e){errors.push(String(e?.message||e))}}
 const unique=[...new Map(candidates.filter(c=>c.ticker||c.isin).map(c=>[resolverCandidateKey(c),c])).values()].map(c=>({...c,score:resolverScore(item,c)})).sort((a,b)=>b.score-a.score);
 const best=unique[0]||null,second=unique[1]||null,margin=best?best.score-num(second?.score):0;
 const conflict=Boolean(best&&second&&best.score>=85&&second.score>=85&&normalizeInstrumentName(best.ticker)!==normalizeInstrumentName(second.ticker));
 const autoSafe=Boolean(best&&best.score>=98&&margin>=5&&!conflict);
 return {key:item.key,name:item.name,account:item.account,assetType:item.assetType,currency:item.currency,best,candidates:unique.slice(0,5),confidence:best?.score||0,margin,conflict,autoSafe,errors};
}
async function runGlobalResolver(){
 const btn=document.getElementById('runGlobalResolver');if(btn){btn.disabled=true;btn.textContent='Söker i identitetskällor…'}
 const targets=resolverTargets().slice(0,30),suggestions=[];let errors=0;
 for(const item of targets){const result=await resolveOneInstrument(item);suggestions.push(result);errors+=result.errors.length}
 const resolved=suggestions.filter(x=>x.autoSafe).length,review=suggestions.filter(x=>x.best&&!x.autoSafe).length,unresolved=suggestions.filter(x=>!x.best).length;
 const previous=globalResolverState();const state={version:'11.10.0',status:errors&&resolved+review===0?'error':unresolved?'partial':'success',lastRun:new Date().toISOString(),scanned:targets.length,resolved,review,unresolved,errors,suggestions,audit:[{date:new Date().toISOString(),scanned:targets.length,resolved,review,unresolved,errors},...(previous.audit||[])].slice(0,100)};
 persistGlobalResolver(state);liveEvent(state.status==='success'?'success':'warning','Global Identity Resolver 11.10',`${resolved} säkra · ${review} för granskning · ${unresolved} utan träff`,{errors});render();openGlobalResolverReview();
}
function applyResolverSuggestion(key,index=0){
 const state=globalResolverState(),row=(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;
 const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;
 liveFoundation.mappings[key]={...current,ticker:candidate.ticker||current.ticker||'',isin:candidate.isin||current.isin||'',exchange:candidate.exchange||current.exchange||'',currency:candidate.currency||current.currency||row.currency||'SEK',providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{})},figi:candidate.figi||current.figi||'',instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Godkänt i Global Identity Resolver'],source:'global-resolver-approved-11.10',resolverVersion:'11.10.0',updatedAt:new Date().toISOString()};
 persistLiveFoundation();row.applied=true;row.appliedAt=new Date().toISOString();row.appliedCandidate=index;persistGlobalResolver(state);liveEvent('success','Global Identity Resolver 11.10',`${row.name} kopplades till ${candidate.ticker||candidate.isin}`,{provider:candidate.provider,score:candidate.score});return true;
}
...
function resolverReviewHtml(){
 const st=globalResolverState();const rows=(st.suggestions||[]).map(row=>{const best=row.best;return `<article class="resolverResult ${row.conflict?'conflict':row.autoSafe?'safe':best?'review':'missing'}"><div class="resolverTitle"><div><b>${esc(row.name)}</b><small>${esc(row.account)} · ${esc(row.assetType)}</small></div><span>${row.applied?'Godkänd':best?`${num(row.confidence)}%`:'Ingen träff'}</span></div>${best?`<div class="resolverBest"><strong>${esc(best.ticker||best.isin)}</strong><p>${esc(best.name||'')} · ${esc(best.exchange||'Okänd börs')} · ${esc(best.currency||row.currency||'')}</p><small>${esc(best.provider)}${row.conflict?' · konflikt mellan källor':''}</small></div><div class="resolverActions"><button class="primary" data-resolver-apply="${esc(row.key)}" ${row.applied?'disabled':''}>${row.applied?'Tillämpad':'Godkänn bästa förslag'}</button>${row.candidates.length>1?`<select data-resolver-choice="${esc(row.key)}">${row.candidates.map((c,i)=>`<option value="${i}">${esc(c.ticker||c.isin)} · ${esc(c.provider)} · ${num(c.score)}%</option>`).join('')}</select>`:''}</div>`:`<div class="notice">Ingen säker identitet hittades automatiskt. Behåll instrumentet för manuell granskning.</div>`}</article>`}).join('');
 return `<div class="eyebrow">Global Identity Resolver 11.10</div><h2>Resolverresultat</h2><p class="disclaimer">Förslag kombinerar lokalt register, OpenFIGI och aktiverade prisproviders. Ingen portföljdata ändras.</p><div class="grid"><div class="card metric"><span>Sökta</span><b>${st.scanned}</b></div><div class="card metric"><span>Säkra</span><b>${st.resolved}</b></div><div class="card metric"><span>Granskning</span><b>${st.review}</b></div><div class="card metric"><span>Utan träff</span><b>${st.unresolved}</b></div></div><button id="applySafeResolver" class="primary" ${st.resolved?'':'disabled'}>Godkänn alla säkra förslag</button><div class="resolverList">${rows||'<div class="empty">Kör resolvermotorn för att skapa förslag.</div>'}</div>`;
}
function openGlobalResolverReview(){openModal(resolverReviewHtml());document.getElementById('applySafeResolver')?.addEventListener('click',()=>{const n=applySafeResolverSuggestions();alert(`${n} säkra identiteter godkändes.`);closeModal();render()});document.querySelectorAll('[data-resolver-apply]').forEach(btn=>btn.addEventListener('click',()=>{const key=btn.dataset.resolverApply,sel=document.querySelector(`[data-resolver-choice="${CSS.escape(key)}"]`),idx=num(sel?.value);if(applyResolverSuggestion(key,idx)){openGlobalResolverReview();render()}}))}
function globalResolverPanel(){const st=globalResolverState(),coverage=instrumentMappingCoverage(),remaining=resolverTargets().length;return `<section class="section globalResolverPanel"><div class="sectionHead"><div><div class="eyebrow">Global Identity Resolver 11.10</div><h2>Flera källor. Ett verifierat instrument.</h2></div><span class="healthScore">${coverage.complete}/${coverage.total}</span></div><p>Resolvermotorn söker parallellt i det lokala registret, OpenFIGI och dina aktiverade provideranslutningar. Den jämför ISIN, namn, ticker, börs, valuta, instrumenttyp och andelsklass.</p><div class="grid"><div class="card metric"><span>Återstår att lösa</span><b>${remaining}</b></div><div class="card metric"><span>Senaste säkra</span><b>${st.resolved}</b></div><div class="card metric"><span>För granskning</span><b>${st.review}</b></div><div class="card metric"><span>Konflikter</span><b>${(st.suggestions||[]).filter(x=>x.conflict).length}</b></div></div><div class="foundationActions"><button id="runGlobalResolver" class="primary" type="button">Kör Global Identity Resolver</button><button id="reviewGlobalResolver" class="secondary" type="button" ${(st.suggestions||[]).length?'':'disabled'}>Granska och godkänn</button></div><div class="resolverSources"><span>Global Registry</span><span>OpenFIGI</span><span>Twelve Data</span><span>Alpha Vantage</span><span>Finnhub</span><span class="disabledSource">Morningstar · anslutning saknas</span></div><p class="disclaimer">Endast godkända identiteter skrivs till read-only live-lagret. Manuella kopplingar och portföljmasterdata skyddas.</p></section>`}
