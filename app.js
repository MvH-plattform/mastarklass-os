...
const RESOLVER_RUN_KEY='mastarklass_os_11_15_9_resolver_run';
...
let resolverWorkerActive=false,resolverStartLocked=false;
...
function permanentIdentityRegistry(){return merge({version:'11.15.9',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.9';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.9',source:'permanent-identity-registry-11.15.9',updatedAt:new Date().toISOString()};restored++}
 }
...
function resolverRunState(){return merge({version:'11.15.9',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],message:''},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.9';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.9',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
function setResolverButtonState(label,disabled=true){const b=document.getElementById('runGlobalResolver');if(b){b.textContent=label;b.disabled=disabled;b.setAttribute('aria-busy',disabled?'true':'false')}}
async function startResolverBatchFromUI(){
 if(resolverStartLocked||resolverWorkerActive)return false;
 resolverStartLocked=true;
 setResolverButtonState('Startar batch…',true);
 try{
  let run=resolverRunState();
  const fresh=resolverTargets();
  const canResume=run.status==='paused'&&Array.isArray(run.targetKeys)&&run.targetKeys.length&&run.cursor<run.targetKeys.length;
  if(!canResume){
   run={version:'11.15.9',status:'running',runId:crypto.randomUUID?.()||String(Date.now()),startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),completedAt:'',cursor:0,total:fresh.length,processed:0,skipped:Object.keys(permanentIdentityRegistry().routes||{}).length,permanent:Object.keys(permanentIdentityRegistry().routes||{}).length,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:fresh.map(x=>x.key),targetItems:fresh.map(x=>({key:x.key,name:x.name,account:x.account,assetType:x.assetType,currency:x.currency,ticker:x.ticker||'',isin:x.isin||'',exchange:x.exchange||''})),suggestions:[],message:`Batch startad. ${Math.min(8,fresh.length)} instrument förbereds…`};
  }else{
   run.status='running';run.stopRequested=false;run.message=`Fortsätter från ${run.processed}/${run.total}…`;
  }
  persistResolverRun(run);updateResolverProgressUI(run);
  await new Promise(r=>setTimeout(r,120));
  await runGlobalResolver();
  return true;
 }catch(error){
  console.error('Resolver batch failed',error);
  const run=resolverRunState();run.status='error';run.errors=num(run.errors)+1;run.message=`Batchfel: ${String(error?.message||error)}`;persistResolverRun(run);syncResolverSummary(run);updateResolverProgressUI(run);setResolverButtonState('Försök igen',false);return false;
 }finally{resolverStartLocked=false}
}
async function runGlobalResolver(){
 if(resolverWorkerActive)return false;
 resolverWorkerActive=true;
 try{
  let run=resolverRunState();
  if(run.status!=='running')return false;
  let targetItems=Array.isArray(run.targetItems)?run.targetItems:[];
  if(!targetItems.length){
   run.status='error';run.message='Batchen saknar förberedd instrumentlista. Tryck Försök igen.';persistResolverRun(run);syncResolverSummary(run);updateResolverProgressUI(run);setResolverButtonState('Försök igen',false);return false;
  }
  const byKey=new Map(targetItems.map(x=>[x.key,x]));
  run.message=`Resolverarbetaren är aktiv. Förbereder instrument ${run.processed+1}/${run.total}…`;persistResolverRun(run);updateResolverProgressUI(run);
  await new Promise(r=>setTimeout(r,40));
  const batchEnd=Math.min(run.targetKeys.length,run.cursor+8);
  for(let i=run.cursor;i<batchEnd;i++){
   run=resolverRunState();
   if(run.stopRequested){run.status='paused';run.message=`Pausad efter ${run.processed}/${run.total}.`;persistResolverRun(run);syncResolverSummary(run);updateResolverProgressUI(run);break}
   const item=byKey.get(run.targetKeys[i]);
   if(!item){run.cursor=i+1;run.processed++;run.failed++;run.message=`Hoppade över saknat instrument (${run.processed}/${run.total}).`;persistResolverRun(run);updateResolverProgressUI(run);continue}
   run.message=`Verifierar ${run.processed+1}/${run.total}: ${item.name}`;persistResolverRun(run);updateResolverProgressUI(run);setResolverButtonState(`Verifierar ${run.processed+1}/${run.total}…`,true);
   await new Promise(r=>setTimeout(r,60));
   try{
    if(routeIsUsable(permanentRouteFor(item.key))){run=resolverRunState();run.cursor=i+1;run.processed++;run.skipped++;}
    else{
     const row=await withDeadline(resolveOneInstrument(item),9000,'Instrumentresolver');
     run=resolverRunState();run.cursor=i+1;run.processed++;run.errors+=row.errors.length;run.suggestions=[...(run.suggestions||[]).filter(x=>x.key!==row.key),compactResolverSuggestion(row)].slice(-80);
  ...
  if(run.status==='running'){
   if(run.cursor>=run.targetKeys.length){run.status='complete';run.completedAt=new Date().toISOString();run.message=`Klar: ${run.newSafe} säkra · ${run.review} för granskning · ${run.failed} utan träff.`}
   else{run.status='paused';run.message=`Batch klar: ${run.processed}/${run.total}. ${run.total-run.processed} återstår.`}
  }
  run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length;persistResolverRun(run);syncResolverSummary(run);updateResolverProgressUI(run);
  refreshValuationAfterResolver();
  setResolverButtonState(run.status==='paused'?'Fortsätt nästa batch (max 8)':'Kör nästa batch (max 8)',false);
  render();
  return true;
 }finally{resolverWorkerActive=false}
}
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){const state=globalResolverState(),run=resolverRunState(),row=rowOverride||(run.suggestions||[]).find(x=>x.key===key)||(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.9'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.9'],source:'permanent-identity-registry-11.15.9',resolverVersion:'11.15.9',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.8',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
