...
function market(){
 ...
 return `<section class="hero"><div class="eyebrow">Live Intelligence 11.4.0</div><h2>Marknad & live-data</h2><p>Read-only marknadsdata från flera utbytbara providers. Din lokala portföljdata ändras aldrig av live-lagret.</p><div class="liveHeroStats"><div><span>Synkstatus</span><b>${esc(sync.status)}</b></div><div><span>Senaste synk</span><b>${sync.lastSuccess?new Date(sync.lastSuccess).toLocaleString('sv-SE'):'Aldrig'}</b></div><div><span>Instrumentmappning</span><b>${mapping.complete}/${mapping.total}</b></div><div><span>Cache</span><b>${cache.items} kurser</b></div></div></section>
 <section class="section liveSyncPanel"><div class="sectionHead"><div><div class="eyebrow">Smart Provider Router</div><h2>Synkronisering</h2></div><span class="syncStatus ${esc(sync.status)}">${esc(sync.status)}</span></div><p>${esc(sync.message)}</p><div class="foundationActions"><button id="syncLive" class="primary" type="button">Synkronisera live-data</button><button id="providerSettings" class="secondary" type="button">Providerinställningar</button><button id="discoverMappings" class="primary" type="button">Identifiera återstående instrument</button><button id="reviewMappings" class="secondary" type="button">Granska osäkra kopplingar</button></div><p class="routeLine"><b>Aktiv rutt:</b> ${esc(providerRouteLabel())}</p>${sync.errors?.length?`<div class="warning"><b>Senaste fel</b><p>${sync.errors.map(esc).join('<br>')}</p></div>`:''}</section>
