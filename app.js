...
function registryMatch(item){const n=normalizeInstrumentName(item.name);let best=null,bestScore=0;for(const e of GLOBAL_INSTRUMENT_REGISTRY){for(const a of registryAliases(e)){const score=n===a?100:(n.includes(a)||a.includes(n)?96:nameSimilarity(n,a));if(score>bestScore){best=e;bestScore=score}}}return bestScore>=88?{entry:best,score:bestScore}:null}
function instrumentRegistryState(){return merge({version:'11.6.0',updatedAt:'',recognized:0,unrecognized:0,lastRun:'',entries:GLOBAL_INSTRUMENT_REGISTRY.length},parse(safeGet(INSTRUMENT_REGISTRY_KEY))||{})}
function persistInstrumentRegistryState(x){safeSet(INSTRUMENT_REGISTRY_KEY,JSON.stringify(x))}
function enrichMappingsFromRegistry(){const items=instrumentMappingCoverage().items;let enriched=0,recognized=0;for(const item of items){const hit=registryMatch(item);if(!hit)continue;recognized++;const e=hit.entry,current=liveFoundation.mappings[item.key]||{};const same=!current.ticker||normalizeInstrumentName(current.ticker)===normalizeInstrumentName(e.ticker);if(!same&&current.source==='manual-live-mapping')continue;liveFoundation.mappings[item.key]={...current,ticker:e.ticker,isin:e.isin||current.isin||'',currency:e.currency,exchange:e.exchange,country:e.country,providerSymbols:e.providerSymbols,type:e.type,name:item.name,account:item.account,assetType:item.assetType,confidence:hit.score,verified:true,source:'global-instrument-registry',registryVersion:'11.6.0',updatedAt:new Date().toISOString()};enriched++}
 ...
 const state={version:'11.6.0',updatedAt:new Date().toISOString(),lastRun:new Date().toISOString(),recognized,unrecognized:Math.max(0,items.length-recognized),entries:GLOBAL_INSTRUMENT_REGISTRY.length};persistInstrumentRegistryState(state);return {...state,enriched}}
...
function providerConfig(){const c=merge({twelveDataKey:'',alphaVantageKey:'',finnhubKey:'',enableStooq:false,stooqExplicitOptIn:false,maxSymbolsPerSync:30},parse(safeGet(PROVIDER_CONFIG_KEY))||{});if(!c.stooqExplicitOptIn)c.enableStooq=false;return c}
