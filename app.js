...
const PERMANENT_IDB_NAME='mastarklass_os_permanent_registry';
const PERMANENT_IDB_STORE='registry';
const PERMANENT_IDB_RECORD='permanent_identity_registry';
const RESOLVER_IDB_RUN_RECORD='resolver_run_state';
const RESOLVER_IDB_TRACE_RECORD='resolver_trace_lines';
const RESOLVER_IDB_META_RECORD='resolver_trace_meta';
let permanentRegistryCache=null,permanentRegistryDb=null,permanentRegistryStorage='startup-cache';
...
const MANUAL_IDENTITY_KEY='mastarklass_os_11_15_34_manual_identity';
const IDENTITY_RECOVERY_KEY='mastarklass_os_11_15_34_identity_recovery';
const IDENTITY_VALIDATION_KEY='mastarklass_os_11_15_35_identity_validation';
const PERSISTENCE_REPAIR_KEY='mastarklass_os_11_15_36_persistence_repair';
const STORAGE_MIGRATION_KEY='mastarklass_os_11_15_37_storage_migration';
const IDENTITY_CONFLICT_KEY='mastarklass_os_11_15_38_identity_conflicts';
...
const SMART_ROUTER_KEY='mastarklass_os_11_15_47_smart_router';
const ADAPTIVE_QUOTE_KEY='mastarklass_os_11_15_47_adaptive_quote_engine';
...
let resolverStateWriteTimer=null,resolverTraceWriteTimer=null,resolverMetaWriteTimer=null;
...
function normalizeIsinValue(x){const v=String(x||'').trim().toUpperCase().replace(/\s+/g,'');return validIsin(v)?v:''}
function manualIdentityState(){const raw=parse(safeGet(MANUAL_IDENTITY_KEY));return raw&&typeof raw==='object'?merge({version:'11.15.34',updatedAt:'',byKey:{},byFingerprint:{}},raw):{version:'11.15.34',updatedAt:'',byKey:{},byFingerprint:{}}}
function identityFingerprintFor(item){return `${normalizeInstrumentName(item?.name||item?._name||'')}|${normalizeInstrumentName(item?.account||item?._account||'')}|${normalizeInstrumentName(item?.assetType||item?._type||'')}`}
function persistManualIdentity(state){state.version='11.15.34';state.updatedAt=new Date().toISOString();safeSet(MANUAL_IDENTITY_KEY,JSON.stringify(state));return state}
function saveManualIdentity(key,item,patch={}){const state=manualIdentityState(),fp=identityFingerprintFor(item),clean={ticker:String(patch.ticker||'').trim(),isin:normalizeIsinValue(patch.isin),exchange:String(patch.exchange||'').trim(),currency:String(patch.currency||'').trim().toUpperCase(),assetType:String(patch.assetType||item?.assetType||item?._type||'').trim(),name:String(item?.name||item?._name||'').trim(),account:String(item?.account||item?._account||'').trim(),source:'manual-identity-11.15.34',updatedAt:new Date().toISOString()};state.byKey[key]={...(state.byKey[key]||{}),...clean};if(fp)state.byFingerprint[fp]={...(state.byFingerprint[fp]||{}),...clean};persistManualIdentity(state);return clean}
function mergedIdentityForHolding(h,key){const state=manualIdentityState(),fp=identityFingerprintFor({name:h?._name||holdingName(h),account:h?._account||accountName(h),assetType:h?._type||assetType(h)}),mapping=liveFoundation.mappings?.[key]||{},route=permanentRouteFor?.(key)||null,manual={...(state.byFingerprint?.[fp]||{}),...(state.byKey?.[key]||{})},override=admin.overrides?.[key]||{};const sources=[h,override,mapping,route,manual];const first=(keys,normalizer=x=>String(x||'').trim())=>{for(let i=sources.length-1;i>=0;i--){for(const k of keys){const v=normalizer(sources[i]?.[k]);if(v)return v}}return''};return {ticker:first(['ticker','symbol','providerSymbol']),isin:first(['isin','ISIN'],normalizeIsinValue),exchange:first(['exchange','bors','börs','market']),currency:first(['currency','valuta'],x=>String(x||'').trim().toUpperCase())||'SEK',assetType:first(['assetType','type','category','tillgangsslag','tillgångsslag'])||assetType(h),source:manual.isin||manual.ticker?'manual-identity-11.15.34':route?'permanent-registry':mapping?'live-mapping':'holding'} }
function findPermanentRouteByIsin(isin){const wanted=normalizeIsinValue(isin);if(!wanted)return null;for(const [key,route] of Object.entries(permanentIdentityRegistry().routes||{})){if(normalizeIsinValue(route?.isin)===wanted&&routeIsUsable(route))return {key,route}}return null}
function migrateIdentityIntegrity(){const state=manualIdentityState();let recovered=0;for(const h of canonicalHoldings()){const key=h._key,fp=identityFingerprintFor(h),candidates=[h,admin.overrides?.[key],liveFoundation.mappings?.[key],permanentRouteFor(key),state.byKey?.[key],state.byFingerprint?.[fp]].filter(Boolean);let isin='',ticker='',exchange='',currency='';for(const c of candidates){isin=isin||normalizeIsinValue(c.isin||c.ISIN);ticker=ticker||String(c.ticker||c.symbol||'').trim();exchange=exchange||String(c.exchange||c.bors||c['börs']||'').trim();currency=currency||String(c.currency||c.valuta||'').trim().toUpperCase()}if(isin||ticker){saveManualIdentity(key,h,{isin,ticker,exchange,currency:currency||'SEK',assetType:h._type});const m=liveFoundation.mappings[key]||{};liveFoundation.mappings[key]={...m,isin:isin||m.isin||'',ticker:ticker||m.ticker||'',exchange:exchange||m.exchange||'',currency:currency||m.currency||'SEK',name:h._name,account:h._account,assetType:h._type,identitySource:'integrity-merge-11.15.34',updatedAt:new Date().toISOString()};recovered++}}
 if(recovered)persistLiveFoundation();return recovered}

function identityRecoveryState(){return merge({version:'11.15.34',lastRun:'',status:'not-run',scannedSources:0,candidates:0,recovered:0,conflicts:0,unmatched:0,items:[],errors:[]},parse(safeGet(IDENTITY_RECOVERY_KEY))||{})}
function persistIdentityRecovery(x){x.version='11.15.34';x.lastRun=new Date().toISOString();safeSet(IDENTITY_RECOVERY_KEY,JSON.stringify(x));return x}
function collectIdentityRecords(value,path='root',out=[],seen=new WeakSet()){
 if(!value||typeof value!=='object')return out;if(seen.has(value))return out;seen.add(value);
 if(Array.isArray(value)){value.forEach((v,i)=>collectIdentityRecords(v,`${path}[${i}]`,out,seen));return out}
 const isin=normalizeIsinValue(value.isin||value.ISIN||value.isinCode||value.instrumentIsin||value.securityIsin);
 if(isin){out.push({isin,name:text(value,['name','securityName','instrumentName','title','fundName','holdingName'],''),account:text(value,['account','accountName','konto','platform','broker'],''),ticker:text(value,['ticker','symbol','providerSymbol'],''),exchange:text(value,['exchange','bors','börs','market'],''),currency:text(value,['currency','valuta'],''),assetType:text(value,['assetType','type','category','tillgangsslag','tillgångsslag'],''),path})}
 for(const [k,v] of Object.entries(value))if(v&&typeof v==='object')collectIdentityRecords(v,`${path}.${k}`,out,seen);return out
}
async function scanIndexedDbIdentityRecords(){const out=[],errors=[];if(!indexedDB?.databases)return {out,errors};let dbs=[];try{dbs=await indexedDB.databases()}catch(e){errors.push(String(e?.message||e));return {out,errors}}
 for(const info of dbs){if(!info.name)continue;try{const db=await new Promise((resolve,reject)=>{const r=indexedDB.open(info.name);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)});for(const storeName of [...db.objectStoreNames]){try{const rows=await new Promise((resolve,reject)=>{const tx=db.transaction(storeName,'readonly'),req=tx.objectStore(storeName).getAll();req.onsuccess=()=>resolve(req.result||[]);req.onerror=()=>reject(req.error)});rows.forEach((row,i)=>collectIdentityRecords(row,`IndexedDB:${info.name}/${storeName}[${i}]`,out))}catch(e){errors.push(`${info.name}/${storeName}: ${String(e?.message||e)}`)}}db.close()}catch(e){errors.push(`${info.name}: ${String(e?.message||e)}`)}}return {out,errors}}
function recoveryMatchScore(h,r){const hn=normalizeInstrumentName(h._name),rn=normalizeInstrumentName(r.name);let score=rn?nameSimilarity(hn,rn):0;if(r.account&&normalizeInstrumentName(r.account)===normalizeInstrumentName(h._account))score+=8;if(r.assetType&&normalizeInstrumentName(r.assetType)===normalizeInstrumentName(h._type))score+=5;if(r.ticker&&String(r.ticker).toUpperCase()===String(h.ticker||h.symbol||'').toUpperCase())score+=8;return Math.min(100,score)}
async function runIdentityRecovery({silent=false}={}){const records=[],errors=[];let scannedSources=0;try{for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(!k)continue;const raw=localStorage.getItem(k);const obj=parse(raw);if(obj&&typeof obj==='object'){scannedSources++;collectIdentityRecords(obj,`localStorage:${k}`,records)}}}catch(e){errors.push(String(e?.message||e))}
 const idb=await scanIndexedDbIdentityRecords();records.push(...idb.out);errors.push(...idb.errors);scannedSources+=new Set(idb.out.map(x=>x.path.split('[')[0])).size;
 const unique=[...new Map(records.map(r=>[`${r.isin}|${normalizeInstrumentName(r.name)}|${r.path}`,r])).values()];let recovered=0,conflicts=0;const items=[];
 for(const h of canonicalHoldings()){const current=mergedIdentityForHolding(h,h._key);if(current.isin){items.push({key:h._key,name:h._name,status:'already',isin:current.isin,source:current.source});continue}const matches=unique.map(r=>({r,score:recoveryMatchScore(h,r)})).filter(x=>x.score>=88).sort((a,b)=>b.score-a.score);if(!matches.length){items.push({key:h._key,name:h._name,status:'unmatched'});continue}const top=matches[0],sameTop=matches.filter(x=>x.score===top.score&&x.r.isin!==top.r.isin);if(sameTop.length){conflicts++;items.push({key:h._key,name:h._name,status:'conflict',candidates:[top.r,...sameTop.map(x=>x.r)]});continue}saveManualIdentity(h._key,h,{isin:top.r.isin,ticker:top.r.ticker,exchange:top.r.exchange,currency:top.r.currency||h.currency||'SEK',assetType:h._type});admin.overrides[h._key]={...(admin.overrides[h._key]||{}),isin:top.r.isin};liveFoundation.mappings[h._key]={...(liveFoundation.mappings[h._key]||{}),isin:top.r.isin,ticker:top.r.ticker||liveFoundation.mappings[h._key]?.ticker||'',exchange:top.r.exchange||liveFoundation.mappings[h._key]?.exchange||'',currency:(top.r.currency||liveFoundation.mappings[h._key]?.currency||'SEK').toUpperCase(),identitySource:'recovered-11.15.34',identityRecoveryPath:top.r.path,updatedAt:new Date().toISOString()};recovered++;items.push({key:h._key,name:h._name,status:'recovered',isin:top.r.isin,source:top.r.path,score:top.score})}
 if(recovered){persistAdmin();persistLiveFoundation();canonicalCache=null}const result=persistIdentityRecovery({status:errors.length?'partial':'complete',scannedSources,candidates:unique.length,recovered,conflicts,unmatched:items.filter(x=>x.status==='unmatched').length,items,errors});resolverDiag('Identity Recovery 11.15.34',`${recovered} ISIN återställda · ${conflicts} konflikter · ${result.unmatched} utan träff`,recovered?'success':'warning');if(!silent)alert(`ISIN-återställning klar.\n${recovered} återställda\n${conflicts} konflikter\n${result.unmatched} utan träff`);render();return result}

window.mkRunIdentityRecovery=event=>{event?.preventDefault?.();void runIdentityRecovery();return false};
...
function resolverInteractionState(){return merge({version:'11.15.43',lastSync:'',lastConflictOpen:'',syncStatus:'idle',conflictStatus:'idle',lastError:''},parse(safeGet(RESOLVER_INTERACTION_KEY))||{})}
function persistResolverInteraction(x){x.version='11.15.43';safeSet(RESOLVER_INTERACTION_KEY,JSON.stringify(x));return x}
function resolverInteractionTime(iso){if(!iso)return 'Inte körd';try{return new Date(iso).toLocaleTimeString('sv-SE',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}catch{return iso}}
function resolverButtonFeedback(id,text,disabled=false){
 const el=document.getElementById(id);
 if(!el)return;
 if(!el.dataset.originalText)el.dataset.originalText=el.textContent.trim();
 el.textContent=text;el.disabled=disabled;el.setAttribute('aria-busy',disabled?'true':'false')
}
function restoreResolverButton(id,delay=1200){
 const el=document.getElementById(id);if(!el)return;
 const original=el.dataset.originalText;
 setTimeout(()=>{if(original){el.textContent=original;el.disabled=false;el.setAttribute('aria-busy','false')}},delay)
}
async function runPortfolioBridgeWithFeedback(event){
 event?.preventDefault?.();event?.stopPropagation?.();
 resolverDiag('Portfolio Identity Bridge 11.15.43 knapptryck','Synk-knapp mottagen','info');
 persistResolverInteraction({...resolverInteractionState(),syncStatus:'running',lastError:''});
 resolverButtonFeedback('mk-portfolio-bridge-btn','Synkar…',true);
 try{
  await Promise.resolve();
  const r=bridgePortfolioToIdentity();
  const now=new Date().toISOString();
  persistResolverInteraction({...resolverInteractionState(),syncStatus:'success',lastSync:now,lastError:''});
  resolverDiag('Portfolio Identity Bridge 11.15.43 klar',`${r.withIsin}/${r.total} med ISIN · ${r.withoutIsin} saknar · ${r.conflicts.length} konflikter`,'success');
  resolverButtonFeedback('mk-portfolio-bridge-btn','Synkning klar ✓',true);
  render();restoreResolverButton('mk-portfolio-bridge-btn',1500);
 }catch(error){
  const message=storageErrorText(error);
  persistResolverInteraction({...resolverInteractionState(),syncStatus:'error',lastError:message});
  resolverDiag('Portfolio Identity Bridge 11.15.43 fel',message,'error');
  resolverButtonFeedback('mk-portfolio-bridge-btn','Synkning misslyckades',true);
  alert('Synkningen kunde inte slutföras.\n\n'+message);
  restoreResolverButton('mk-portfolio-bridge-btn',2200)
 }
 return false
}
window.mkRunPortfolioIdentityBridge=runPortfolioBridgeWithFeedback;

function openConflictResolverWithFeedback(event){
 if(!identityConflictRendererSelfTest()){alert('Konfliktrapportens renderer kunde inte starta. Se loggen för detaljer.');return false}
 event?.preventDefault?.();event?.stopPropagation?.();
 resolverDiag('Conflict Resolver 11.15.43 knapptryck','Konfliktknapp mottagen','info');
 persistResolverInteraction({...resolverInteractionState(),conflictStatus:'opening',lastError:''});
 resolverButtonFeedback('mk-conflict-resolver-btn','Öppnar rapport…',true);
 try{
  const report=renderIdentityConflictDetails();
  const now=new Date().toISOString();
  persistResolverInteraction({...resolverInteractionState(),conflictStatus:'success',lastConflictOpen:now,lastError:''});
  resolverDiag('Conflict Resolver 11.15.43 modal öppnad',`${report.withIsin}/${report.total} med ISIN · ${report.withoutIsin} saknar · ${report.conflicts.length} konflikter`,'success');
  restoreResolverButton('mk-conflict-resolver-btn',900)
 }catch(error){
  const message=storageErrorText(error);
  persistResolverInteraction({...resolverInteractionState(),conflictStatus:'error',lastError:message});
  resolverDiag('Conflict Resolver 11.15.43 fel',message,'error');
  resolverButtonFeedback('mk-conflict-resolver-btn','Kunde inte öppna',true);
  alert('Konfliktrapporten kunde inte öppnas.\n\n'+message);
  restoreResolverButton('mk-conflict-resolver-btn',2200)
 }
 return false
}
window.mkOpenIdentityConflictResolver=openConflictResolverWithFeedback;

function bindResolverInteractionHandlers(){
 const bridge=document.getElementById('mk-portfolio-bridge-btn');
 if(bridge&&!bridge.dataset.bound41){bridge.dataset.bound41='true';bridge.addEventListener('click',runPortfolioBridgeWithFeedback,{passive:false})}
 const conflicts=document.getElementById('mk-conflict-resolver-btn');
 if(conflicts&&!conflicts.dataset.bound41){conflicts.dataset.bound41='true';conflicts.addEventListener('click',openConflictResolverWithFeedback,{passive:false})}
 const fallback=document.getElementById('mk-conflict-fallback-btn');
 if(fallback&&!fallback.dataset.bound41){fallback.dataset.bound41='true';fallback.addEventListener('click',openConflictResolverWithFeedback,{passive:false})}
 const routing=document.getElementById('mk-market-routing-btn');
 if(routing&&!routing.dataset.bound44){routing.dataset.bound44='true';routing.addEventListener('click',runMarketRoutingWithFeedback,{passive:false})}
}

function portfolioIdentityBridgeState(){return merge({version:'11.15.40',lastRun:'',status:'not-run',source:'mastarklass_os_10_data',total:0,bridged:0,withIsin:0,withoutIsin:0,conflicts:0},parse(safeGet(PORTFOLIO_IDENTITY_BRIDGE_KEY))||{})}
function persistPortfolioIdentityBridge(x){x.version='11.15.40';x.lastRun=new Date().toISOString();safeSet(PORTFOLIO_IDENTITY_BRIDGE_KEY,JSON.stringify(x));return x}
function cleanupSafetyLockState(){return merge({version:'11.15.40',lastRun:'',status:'active',automaticCleanup:false},parse(safeGet(CLEANUP_SAFETY_LOCK_KEY))||{})}
function persistCleanupSafetyLock(x){x.version='11.15.40';x.lastRun=new Date().toISOString();safeSet(CLEANUP_SAFETY_LOCK_KEY,JSON.stringify(x));return x}
function activePortfolioPayload(){try{const parsed=parse(safeGet('mastarklass_os_10_data'));return parsed&&typeof parsed==='object'?parsed:null}catch{return null}}
function activePortfolioHoldings(){
 const payload=activePortfolioPayload();
 const candidates=[payload?.holdings,payload?.portfolio?.holdings,payload?.data?.holdings,payload?.state?.holdings,payload?.assets,Array.isArray(payload?.portfolio)?payload.portfolio:null];
 for(const list of candidates)if(Array.isArray(list)&&list.length)return list;
 return rawHoldings()
}
function bridgePortfolioToIdentity(){
 const live=activePortfolioHoldings();
 canonicalCache=null;
 const report=buildUnifiedIdentityReport();
 const result=persistPortfolioIdentityBridge({status:live.length?'success':'warning',source:live.length?'mastarklass_os_10_data':'fallback',total:report.total,bridged:live.length,withIsin:report.withIsin,withoutIsin:report.withoutIsin,conflicts:report.conflicts.length});
 return {...report,bridge:result}
}


function marketRoutingState(){return merge({version:'11.15.49',lastRun:'',status:'not-run',total:0,routed:0,quoteReady:0,navReady:0,fxReady:0,needsPrice:0,needsNav:0,needsFx:0,routes:{},missingPrice:[],missingNav:[],missingFx:[],blockedQuoteRoutes:[]},parse(safeGet(MARKET_ROUTING_KEY))||{})}
function persistMarketRouting(x){x.version='11.15.49';x.lastRun=new Date().toISOString();safeSet(MARKET_ROUTING_KEY,JSON.stringify(x));return x}
...
function marketRouteForHolding(h){
 ...
 const ticker=identity.ticker||mapping.ticker||inferTickerForHolding(h),isin=identity.isin||mapping.isin||'';
 const providerSymbols={...(mapping.providerSymbols||{}),...(mapping.routeTicker||{})};
 const pricing=cls.pricing==='nav'?'nav':'quote';
 const route={key:h._key,name:h._name,account:h._account,assetType:h._type,pricing,ticker,isin,exchange:identity.exchange||mapping.exchange||'',currency,providerSymbols,identitySource:identity.source||'',identityReady:Boolean(isin),marketReady:Boolean(pricing==='nav'?isin:(ticker||isin)),updatedAt:new Date().toISOString()};
 return route
}
function buildMarketRoutingBridge({log=true}={}){
 canonicalCache=null;
 const routes={},hs=canonicalHoldings();let routed=0,quoteReady=0,navReady=0,fxReady=0,needsPrice=0,needsNav=0,needsFx=0;
 for(const h of hs){
  const r=marketRouteForHolding(h);routes[h._key]=r;if(r.marketReady)routed++;
  const q=liveQuoteForRoute(h,r),nav=navFor(h),fx=r.currency==='SEK'?1:fxForCurrency(r.currency);
 ...
 if(log)resolverDiag('ISIN Market Routing 11.15.49',`${routed}/${hs.length} rutter · ${quoteReady} kurser · ${navReady} NAV · ${needsFx} saknar FX · ${blockedQuoteRoutes.length} NAV-rutter skyddade`,routed===hs.length?'success':'warning');
 return result
}
...
function liveQuoteForRoute(h,route=marketRouteForHolding(h)){
...
function marketRoutingPanel(){const r=marketRoutingState(),built=r.lastRun?resolverInteractionTime(r.lastRun):'Inte byggd';return `<div class="portfolioBridgeBox marketRoutingBridge"><div><b>Smart Market Router & Valuation Bridge 11.15.49</b><span>${r.status==='not-run'?'Inte byggd':`Rutter klara ${r.routed}/${r.total} · ${r.quoteReady} kurser · ${r.navReady} NAV`}</span><small>Senast byggd: ${built} · ${r.needsPrice||0} saknar kurs · ${r.needsNav||0} saknar NAV · ${r.needsFx||0} saknar FX</small></div><div class="identityConflictActions"><button id="mk-market-routing-btn" class="primary" type="button">Bygg marknadsrutter</button><button class="secondary" type="button" onclick="return window.mkOpenMarketRoutingReport(event)">Öppna rutt- och felrapport</button><button class="secondary" type="button" onclick="return window.mkOpenSmartRouterReport(event)">Öppna datatillit och providerplan</button><button class="secondary" type="button" onclick="return window.mkOpenAdaptiveQuoteReport(event)">Öppna Full Portfolio Quote Orchestrator</button><button class="secondary" type="button" onclick="return window.mkOpenRateLimitSchedulerReport(event)">Öppna Rate-Limit Scheduler</button></div></div>`}
function runMarketRoutingWithFeedback(event){event?.preventDefault?.();event?.stopPropagation?.();resolverButtonFeedback('mk-market-routing-btn','Bygger rutter…',true);try{const r=buildMarketRoutingBridge();buildSmartRouterState({log:true});canonicalCache=null;liveValuationSnapshot({persist:true});resolverButtonFeedback('mk-market-routing-btn','Rutter klara ✓',true);render();restoreResolverButton('mk-market-routing-btn',1500);return r}catch(error){const message=storageErrorText(error);resolverDiag('ISIN Market Routing 11.15.49 fel',message,'error');resolverButtonFeedback('mk-market-routing-btn','Kunde inte bygga',true);alert('Marknadsrutterna kunde inte byggas.\n\n'+message);restoreResolverButton('mk-market-routing-btn',2200);return false}}
window.mkRunMarketRouting=runMarketRoutingWithFeedback;

function renderMarketRoutingReport(){const r=marketRoutingState();const rows=(items,label)=>items?.length?items.map(x=>`<div class="missingIsinRow"><span><b>${escapeHtml(x.name||x.currency||'Okänd')}</b><small>${escapeHtml(x.account||x.currency||'')} ${x.ticker?`· ${escapeHtml(x.ticker)}`:''} ${x.isin?`· ${escapeHtml(x.isin)}`:''}</small></span><strong>${label}</strong></div>`).join(''):`<div class="notice okbox">Inga poster.</div>`;openModal(`<div class="sectionHead"><div><h2>Marknadsrutter 11.15.49</h2><p>Rutter klara ${r.routed}/${r.total} · senast byggd ${resolverInteractionTime(r.lastRun)}</p></div></div><div class="grid"><div class="card metric"><span>Kurser</span><b>${r.quoteReady}</b></div><div class="card metric"><span>NAV</span><b>${r.navReady}</b></div><div class="card metric"><span>FX klara</span><b>${r.fxReady}</b></div><div class="card metric"><span>NAV-rutter skyddade</span><b>${r.blockedQuoteRoutes?.length||0}</b></div></div><h3>Saknar kurs</h3>${rows(r.missingPrice,'Kurs saknas')}<h3 class="topGap">Saknar NAV</h3>${rows(r.missingNav,'NAV saknas')}<h3 class="topGap">Saknar FX</h3>${rows(r.missingFx,'FX saknas')}<p class="disclaimer">NAV-instrument, inklusive Montrose-fonder, blockeras från aktieproviders för att undvika HTTP 403 och andra felaktiga kursanrop. Senast kända värden används tills giltig kurs eller NAV finns.</p>`);return false}
window.mkOpenMarketRoutingReport=event=>{event?.preventDefault?.();return renderMarketRoutingReport()};



/* 11.15.49 — Smart Provider Router & Valuation Confidence */
function smartRouterState(){return merge({version:'11.15.49',lastRun:'',status:'not-run',total:0,quoteRoutes:0,navRoutes:0,cacheFresh:0,cacheDelayed:0,providerNeeded:0,highConfidence:0,mediumConfidence:0,lowConfidence:0,averageConfidence:0,items:[]},parse(safeGet(SMART_ROUTER_KEY))||{})}
function persistSmartRouter(x){x.version='11.15.49';x.lastRun=new Date().toISOString();safeSet(SMART_ROUTER_KEY,JSON.stringify(x));return x}
...
function buildSmartRouterState({log=true}={}){
 ...
 if(log)resolverDiag('Smart Provider Router 11.15.49',`${items.length} instrument · ${quoteRoutes} kursrutter · ${navRoutes} NAV-rutter · tillit ${result.averageConfidence}%`,lowConfidence?'warning':'success');
 return result
}
...
function renderSmartRouterReport(){const s=buildSmartRouterState({log:false});const rows=s.items.slice().sort((a,b)=>a.confidence-b.confidence).map(x=>`<div class="smartRouteRow ${x.confidenceLevel}"><div><b>${escapeHtml(x.name)}</b><small>${escapeHtml(x.account)} · ${escapeHtml(x.pricing.toUpperCase())} · ${escapeHtml(x.providerAction)}</small><small>${escapeHtml((x.reasons||[]).join(' · ')||'Saknar underlag')}</small></div><strong>${x.confidence}%</strong></div>`).join('');openModal(`<div class="sectionHead"><div><h2>Smart Provider Router 11.15.49</h2><p>${s.total} instrument · ${s.quoteRoutes} kursrutter · ${s.navRoutes} NAV-rutter</p></div></div>${smartRouterSummaryHtml()}<h3>Instrument sorterade efter lägst datatillit</h3><div class="smartRouteList">${rows}</div><p class="disclaimer">NAV-instrument skickas endast till NAV-spåret. Prisproviders används bara för kursinstrument som saknar giltig cache. Portföljdata, antal och GAV ändras inte.</p>`);return false}
...
const CLEANUP_EXACT_ALLOWLIST=[
 ...
 'mastarklass_os_providertelemetry',
 'mastarklass_os_provider_telemetry',
 'mastarklass_os_resolver_trace',
 'mastarklass_os_liveevents',
 'mastarklass_os_live_events'
];
function cleanupKeyIsExplicitlyAllowed(key){
 const k=String(key||'');
 return CLEANUP_EXACT_ALLOWLIST.includes(k)||/^mastarklass_os_(temp|tmp|session|trace)_/i.test(k)
}
function cleanupKeyIsAlwaysProtected(key){
 const k=String(key||'').toLowerCase();
 return [
  'mastarklass_os_10_data','mastarklass_os_10_3_1_admin','mastarklass_os_10_3_transactions','mastarklass_os_10_4_ledger',
  'portfolio','account','holding','transaction','ledger','api','direct_correction','manual_identity','permanent_registry','live_mapping',
  'portfolio_identity_bridge','cleanup_safety_lock'
 ].some(token=>k.includes(token))
}
function cleanupPreview(){
 const rows=localStorageEntries(),deletable=[],protectedRows=[];
 for(const row of rows){
  if(cleanupKeyIsAlwaysProtected(row.key)||!cleanupKeyIsExplicitlyAllowed(row.key))protectedRows.push(row);
  else deletable.push(row)
 }
 return {deletable,protectedRows,totalKb:Math.round(deletable.reduce((s,r)=>s+r.size,0)/1024)}
}
function renderCleanupPreview(){
 const p=cleanupPreview();
 const items=p.deletable.length?p.deletable.map(r=>`<div class="cleanupPreviewRow"><b>${escapeHtml(r.key)}</b><span>${Math.round(r.size/1024)} KB</span></div>`).join(''):'<div class="notice okbox">Inga uttryckligen tillåtna loggposter finns att rensa.</div>';
 openModal(`<div class="sectionHead"><div><h2>Cleanup Safety Lock 11.15.40</h2><p>Allt skyddas som standard. Endast exakt tillåtna loggar och temporära poster kan rensas.</p></div></div><div class="notice">Förhandsgranskning: ${p.deletable.length} poster · cirka ${p.totalKb} KB</div><div class="cleanupPreviewList">${items}</div>${p.deletable.length?`<button class="danger topGap" type="button" onclick="return window.mkExecuteSafeCleanup(event)">Rensa endast dessa poster</button>`:''}`);
 return false
}
window.mkPreviewSafeCleanup=event=>{event?.preventDefault?.();return renderCleanupPreview()};
window.mkExecuteSafeCleanup=event=>{
 event?.preventDefault?.();const p=cleanupPreview();let removed=0;
 for(const row of p.deletable){try{localStorage.removeItem(row.key);removed++}catch{}}
 persistCleanupSafetyLock({...cleanupSafetyLockState(),status:'active',automaticCleanup:false,lastCleanupRemoved:removed});
 closeModal();alert(`Säker rensning klar.\n${removed} tillåtna poster rensades.\nPortfölj- och identitetsdata skyddades.`);render();return false
};

function unifiedIdentityState(){return merge({version:'11.15.39',lastRun:'',status:'not-run',total:0,withIsin:0,withoutIsin:0,conflicts:[],missing:[],records:[]},parse(safeGet(UNIFIED_IDENTITY_KEY))||{})}
function persistUnifiedIdentityState(x){x.version='11.15.39';x.lastRun=new Date().toISOString();safeSet(UNIFIED_IDENTITY_KEY,JSON.stringify(x));return x}
function unifiedIdentityHoldings(){
 canonicalCache=null;
 const hs=canonicalHoldings();
 return hs.map(h=>{
  const merged=mergedIdentityForHolding(h,h._key);
  return {
   key:h._key,
   name:h._name,
   account:h._account||'',
   ...
   isin:normalizeIsinValue(merged.isin),
...
function buildUnifiedIdentityReport(){
 const records=unifiedIdentityHoldings(),byIsin=new Map(),missing=[];
 for(const r of records){
  if(r.isin){
   if(!byIsin.has(r.isin))byIsin.set(r.isin,[]);
   byIsin.get(r.isin).push(r)
  }else missing.push(r)
 }
 const conflicts=[],sharedIsinGroups=[];
 for(const [isin,rows] of byIsin){
  if(rows.length>1)sharedIsinGroups.push({isin,rows,accounts:[...new Set(rows.map(r=>r.account).filter(Boolean))]})
 }
 const report={
  status:conflicts.length?'warning':'success',
  total:records.length,
  withIsin:records.length-missing.length,
  withoutIsin:missing.length,
  conflicts,
  sharedIsinGroups,
  missing,
  records
 };
 persistUnifiedIdentityState(report);
 persistIdentityConflictState({status:report.status,withIsin:report.withIsin,withoutIsin:report.withoutIsin,conflicts:report.conflicts,sharedIsinGroups:report.sharedIsinGroups,missing:report.missing});
 persistIdentityValidation({
  status:conflicts.length?'warning':'success',
  total:report.total,
  withIsin:report.withIsin,
  withoutIsin:report.withoutIsin,
  duplicateConflicts:conflicts.length,
  unsafeVerifiedRoutes:identityValidationState().unsafeVerifiedRoutes||0,
  persistenceOk:persistenceRepairState().repaired===true,
  issues:conflicts.map(c=>({type:'duplicate-isin',name:c.isin,message:`ISIN används av olika instrument: ${c.rows.map(r=>r.name).join(' · ')}`}))
 });
 return report
}
function refreshUnifiedIdentity({renderAfter=false}={}){
 const report=buildUnifiedIdentityReport();
 resolverDiag('Unified Identity Engine 11.15.39',`${report.withIsin}/${report.total} med ISIN · ${report.withoutIsin} saknar · ${report.conflicts.length} konflikter`,report.conflicts.length?'warning':'success');
 if(renderAfter)render();
 return report
}
window.mkRefreshUnifiedIdentity=event=>{event?.preventDefault?.();refreshUnifiedIdentity({renderAfter:true});return false};

function identityConflictState(){return merge({version:'11.15.43',lastRun:'',status:'not-run',withIsin:0,withoutIsin:0,conflicts:[],sharedIsinGroups:[],missing:[]},parse(safeGet(IDENTITY_CONFLICT_KEY))||{})}
function persistIdentityConflictState(x){x.version='11.15.38';x.lastRun=new Date().toISOString();safeSet(IDENTITY_CONFLICT_KEY,JSON.stringify(x));return x}
function buildIdentityConflictReport(){
 const report=buildUnifiedIdentityReport();
 return {
...
function jumpToHoldingEditor(holdingKey){
 try{
  const h=canonicalHoldings().find(x=>x._key===holdingKey);
  if(!h){alert('Innehavet kunde inte hittas.');return false}
  window.__mkPendingEditHoldingKey=holdingKey;
  if(typeof openHoldingEditor==='function'){openHoldingEditor(holdingKey);return false}
  alert(`Öppna Portfölj → Administrera → Redigera befintligt innehav och välj:\n\n${h._name}`);
  return false
 }catch(error){alert('Kunde inte öppna redigeringen: '+storageErrorText(error));return false}
}
window.mkJumpToHoldingEditor=(event,key)=>{event?.preventDefault?.();return jumpToHoldingEditor(key)};

function identityConflictRendererSelfTest(){
 try{
  const probe=escapeHtml('<test & "ok">');
  const ok=probe==='&lt;test &amp; &quot;ok&quot;&gt;';
  resolverDiag('Conflict Renderer 11.15.43 self-test',ok?'escapeHtml OK':'escapeHtml gav oväntat resultat',ok?'success':'error');
  return ok
 }catch(error){
  resolverDiag('Conflict Renderer 11.15.43 self-test fel',storageErrorText(error),'error');
  return false
 }
}

function renderIdentityConflictDetails(){
 const report=buildIdentityConflictReport();
 const conflicts=report.conflicts||[],shared=report.sharedIsinGroups||[],missing=report.missing||[];
 const conflictHtml=conflicts.length?conflicts.map(c=>`<div class="identityConflictCard"><div><b>${escapeHtml(c.isin)}</b><span>${c.rows.length} olika instrument kräver granskning</span></div>${c.rows.map(r=>`<button type="button" onclick="return window.mkJumpToHoldingEditor(event,'${escapeHtml(r.key)}')"><strong>${escapeHtml(r.name)}</strong><small>${escapeHtml(r.account||'Okänt konto')} · ${escapeHtml(r.ticker||'ingen ticker')}</small></button>`).join('')}</div>`).join(''):`<div class="notice okbox">Inga verkliga ISIN-konflikter hittades.</div>`;
 const sharedHtml=shared.length?shared.map(c=>`<div class="identityConflictCard"><div><b>${escapeHtml(c.isin)}</b><span>Samma värdepapper på ${c.rows.length} innehav · tillåtet</span></div>${c.rows.map(r=>`<div class="missingIsinRow"><span><b>${escapeHtml(r.name)}</b><small>${escapeHtml(r.account||'Okänt konto')} · ${escapeHtml(r.ticker||'ingen ticker')}</small></span><strong>Gemensam identitet</strong></div>`).join('')}</div>`).join(''):`<div class="notice okbox">Inga delade ISIN mellan konton.</div>`;
 const missingHtml=missing.slice(0,120).map(r=>`<button class="missingIsinRow" type="button" onclick="return window.mkJumpToHoldingEditor(event,'${escapeHtml(r.key)}')"><span><b>${escapeHtml(r.name)}</b><small>${escapeHtml(r.account||'Okänt konto')} · ${escapeHtml(r.type||'Okänd typ')} · ${escapeHtml(r.ticker||'ingen ticker')}</small></span><strong>Lägg till ISIN</strong></button>`).join('');
 openModal(`<div class="sectionHead"><div><h2>Unified Identity Engine 11.15.43</h2><p>${report.withIsin}/${report.total} med ISIN · ${report.withoutIsin} saknar · ${conflicts.length} verkliga konflikter</p></div></div><h3>Verkliga ISIN-konflikter</h3>${conflictHtml}<h3 class="topGap">Samma värdepapper på flera konton</h3>${sharedHtml}<h3 class="topGap">Saknar ISIN</h3><div class="missingIsinList">${missingHtml||'<div class="notice okbox">Alla innehav har ISIN.</div>'}</div>`);
 return report
}

function refreshIdentityConflictCounters(){
 const r=buildIdentityConflictReport();
 resolverDiag('Unified Identity Engine 11.15.39',`${r.withIsin} med ISIN · ${r.withoutIsin} saknar · ${r.conflicts.length} konflikter`,r.conflicts.length?'warning':'success');
 return r
}

function storageMigrationState(){return merge({version:'11.15.37',lastRun:'',status:'not-run',beforeKb:0,afterKb:0,freedKb:0,migrated:[],removed:[],kept:[],errors:[]},parse(safeGet(STORAGE_MIGRATION_KEY))||{})}
function persistStorageMigration(x){x.version='11.15.37';x.lastRun=new Date().toISOString();safeSet(STORAGE_MIGRATION_KEY,JSON.stringify(x));return x}
function localStorageEntries(){
 ...
 try{
  for(let i=0;i<localStorage.length;i++){
   ...
   rows.push({key,value,size:value.length});
  }
 }catch{}
 return rows.sort((a,b)=>b.size-a.size)
}
function protectedStorageKey(key){
 return [
  'mastarklass_os_portfolio',
  'mastarklass_os_accounts',
  'mastarklass_os_transactions',
  'mastarklass_os_ledger',
  'mastarklass_os_api',
  'mastarklass_os_manual_identity',
  'mastarklass_os_permanent_registry',
  'mastarklass_os_live_mapping',
  'mastarklass_os_direct_corrections',
  IDENTITY_RECOVERY_KEY,
  IDENTITY_VALIDATION_KEY,
  PERSISTENCE_REPAIR_KEY,
  STORAGE_MIGRATION_KEY
 ].some(token=>String(key).includes(token))
}
function disposableStorageKey(key){
 const k=String(key).toLowerCase();
 return (
  /resolver_diag|resolver_trace|providertelemetry|provider_telemetry|liveevents|live_events|debug|diagnostic|cache|temp|tmp|session|steward|confidence_report|recovery_report/.test(k)
  || /mastarklass_os_11_(0[0-9]|1[0-4])_/.test(k)
  || /mastarklass_os_11_15_(0[0-9]|1[0-5])_/.test(k)
 )
}
async function migrateLargeValueToIndexedDb(row){
 const id='localStorage-migration:'+row.key;
 try{
  await idbWriteResolverRecord(id,{version:'11.15.37',sourceKey:row.key,migratedAt:new Date().toISOString(),value:row.value});
  const read=await idbReadResolverRecord(id);
  if(!read||read.sourceKey!==row.key||read.value!==row.value)throw new Error('Migrerad post kunde inte verifieras');
  localStorage.removeItem(row.key);
  return {ok:true,id}
 }catch(error){return {ok:false,error:storageErrorText(error)}}
}
async function runStorageCleanupMigration({silent=false}={}){
 const before=storageFootprint(),preview=cleanupPreview(),removed=[];
 for(const row of preview.deletable){try{localStorage.removeItem(row.key);removed.push(row.key)}catch{}}
 const after=storageFootprint(),freedKb=Math.max(0,before.kb-after.kb);
 const result=persistStorageMigration({status:'safe',beforeKb:before.kb,afterKb:after.kb,freedKb,migrated:[],removed,kept:preview.protectedRows.slice(0,80).map(r=>r.key),errors:[]});
 persistCleanupSafetyLock({...cleanupSafetyLockState(),status:'active',automaticCleanup:false,lastCleanupRemoved:removed.length});
 resolverDiag('Cleanup Safety Lock 11.15.40',`${removed.length} tillåtna poster rensade · portföljdata skyddad`,'success');
 if(!silent)alert(`Säker rensning klar.\n${removed.length} tillåtna poster rensades.\nPortföljdata och identitetsregister skyddades.`);
 render();return result
}
window.mkRunStorageCleanup=event=>{event?.preventDefault?.();void runStorageCleanupMigration();return false};

function persistenceRepairState(){return merge({version:'11.15.36',lastRun:'',status:'not-run',localStorage:{ok:false,error:'',beforeKb:0,afterKb:0},indexedDB:{ok:false,error:''},registry:{ok:false,error:'',routes:0},compaction:[],repaired:false},parse(safeGet(PERSISTENCE_REPAIR_KEY))||{})}
function persistPersistenceRepair(x){x.version='11.15.36';x.lastRun=new Date().toISOString();safeSet(PERSISTENCE_REPAIR_KEY,JSON.stringify(x));return x}
...
function storageErrorText(error){const name=String(error?.name||''),message=String(error?.message||error||'Okänt fel');if(/quota/i.test(name+' '+message))return 'Lagringsutrymmet är fullt (QuotaExceededError).';if(/security/i.test(name+' '+message))return 'Webbläsaren blockerar lokal lagring.';if(/invalidstate|closed/i.test(name+' '+message))return 'Databasen är stängd eller i ogiltigt tillstånd.';if(/blocked/i.test(name+' '+message))return 'IndexedDB blockeras av en annan flik eller äldre appinstans.';return message}
function testLocalStoragePersistence(){
 const key='__mk_persistence_probe_11_15_36__',value=JSON.stringify({version:'11.15.36',stamp:Date.now(),payload:'x'.repeat(2048)});
 try{
  const wrote=safeSet(key,value);
  if(!wrote)throw new Error('localStorage.setItem returnerade false');
  const read=safeGet(key);
  if(read!==value)throw new Error('Återläst värde skiljer sig från det skrivna');
  localStorage.removeItem(key);
  return {ok:true,error:''}
 }catch(error){try{localStorage.removeItem(key)}catch{}return {ok:false,error:storageErrorText(error)}}
}
async function testIndexedDbPersistence(){
 const key='__mk_persistence_probe_11_15_36__',value={version:'11.15.36',stamp:Date.now(),payload:'x'.repeat(2048)};
 try{
  await idbWriteResolverRecord(key,value);
  const read=await idbReadResolverRecord(key);
  if(!read||read.version!=='11.15.36'||read.payload!==value.payload)throw new Error('IndexedDB återläsning matchade inte skrivningen');
  const db=await openPermanentRegistryDb();
  await new Promise((resolve,reject)=>{const tx=db.transaction(PERMANENT_IDB_STORE,'readwrite');tx.objectStore(PERMANENT_IDB_STORE).delete(key);tx.oncomplete=()=>resolve(true);tx.onerror=()=>reject(tx.error||new Error('Probe kunde inte raderas'))});
  return {ok:true,error:''}
 }catch(error){return {ok:false,error:storageErrorText(error)}}
}
async function testPermanentRegistryPersistence(){
 try{
  const current=normalizePermanentRegistry(structuredClone(permanentIdentityRegistry()));
  await idbWritePermanentRegistry(current);
  const read=normalizePermanentRegistry(await idbReadPermanentRegistry());
  if(!read||typeof read.routes!=='object')throw new Error('Permanent Registry kunde inte läsas tillbaka');
  permanentRegistryCache=read;permanentRegistryStorage='IndexedDB';
  return {ok:true,error:'',routes:Object.keys(read.routes||{}).length}
 }catch(error){return {ok:false,error:storageErrorText(error),routes:Object.keys(permanentIdentityRegistry().routes||{}).length}}
}
async function runPersistenceRepair({silent=false}={}){
 const before=storageFootprint(),compaction=[];
 let local=testLocalStoragePersistence();
 if(!local.ok){
  await runStorageCleanupMigration({silent:true});
  compaction.push(...compactReadOnlyStorageForRegistry());
  try{
   const recovery=parse(safeGet(IDENTITY_RECOVERY_KEY));if(recovery?.items?.length>120){recovery.items=recovery.items.slice(0,120);safeSet(IDENTITY_RECOVERY_KEY,JSON.stringify(recovery));compaction.push('komprimerade återställningsrapport')}
   const validation=parse(safeGet(IDENTITY_VALIDATION_KEY));if(validation?.issues?.length>60){validation.issues=validation.issues.slice(0,60);safeSet(IDENTITY_VALIDATION_KEY,JSON.stringify(validation));compaction.push('komprimerade valideringsrapport')}
  }catch(error){compaction.push('extra komprimering misslyckades: '+storageErrorText(error))}
  local=testLocalStoragePersistence();
 }
 const indexedDB=await testIndexedDbPersistence();
 const registry=await testPermanentRegistryPersistence();
 const after=storageFootprint();
 const repaired=local.ok&&indexedDB.ok&&registry.ok;
 const result=persistPersistenceRepair({status:repaired?'success':'error',localStorage:{...local,beforeKb:before.kb,afterKb:after.kb},indexedDB,registry,compaction,repaired});
 resolverDiag('Persistence Repair 11.15.36',`localStorage=${local.ok?'OK':'FEL'} · IndexedDB=${indexedDB.ok?'OK':'FEL'} · registry=${registry.ok?'OK':'FEL'}`,repaired?'success':'error');
 if(!silent){
  const details=[
   `localStorage: ${local.ok?'OK':'FEL'+(local.error?' — '+local.error:'')}`,
   `IndexedDB: ${indexedDB.ok?'OK':'FEL'+(indexedDB.error?' — '+indexedDB.error:'')}`,
   `Permanent Registry: ${registry.ok?'OK':'FEL'+(registry.error?' — '+registry.error:'')}`,
   `Lagring före/efter: ${before.kb} KB → ${after.kb} KB`,
   compaction.length?`Åtgärder: ${compaction.join(' · ')}`:'Inga rensningar behövdes.'
  ];
  alert(`Persistence Repair klar.\n\n${details.join('\n')}`);
 }
 render();return result
}
window.mkRunPersistenceRepair=event=>{event?.preventDefault?.();void runPersistenceRepair();return false};

function identityValidationState(){return merge({version:'11.15.36',lastRun:'',status:'not-run',total:0,withIsin:0,withoutIsin:0,duplicateConflicts:0,unsafeVerifiedRoutes:0,persistenceOk:false,issues:[]},parse(safeGet(IDENTITY_VALIDATION_KEY))||{})}
function persistIdentityValidation(x){x.version='11.15.36';x.lastRun=new Date().toISOString();safeSet(IDENTITY_VALIDATION_KEY,JSON.stringify(x));return x}
function runIdentityIntegrityValidation({silent=false}={}){
 const hs=canonicalHoldings(),owners=new Map(),issues=[];let withIsin=0,unsafeVerifiedRoutes=0;
 for(const h of hs){
  const id=mergedIdentityForHolding(h,h._key),isin=normalizeIsinValue(id.isin);
  if(isin){withIsin++;if(!owners.has(isin))owners.set(isin,[]);owners.get(isin).push(h)}
  const route=permanentRouteFor(h._key);
  if(route&&route.identityVerified===true&&!validIsin(route.isin)&&!String(route.source||'').includes('Global Registry')){
   unsafeVerifiedRoutes++;issues.push({key:h._key,name:h._name,type:'unsafe-route',message:'Marknadsrutt utan giltigt ISIN var felaktigt märkt som verifierad.'});
  }
 }
 let duplicateConflicts=0;
 for(const [isin,rows] of owners){
  const names=new Set(rows.map(h=>normalizeInstrumentName(h._name)));
  if(names.size>1){duplicateConflicts++;issues.push({type:'duplicate-isin',name:isin,message:`ISIN används av olika instrument: ${rows.map(h=>h._name).join(' · ')}`})}
 }
 const localProbe=testLocalStoragePersistence();
 const repair=persistenceRepairState();
 const persistenceOk=localProbe.ok&&repair.indexedDB?.ok!==false&&repair.registry?.ok!==false;
 const result=persistIdentityValidation({status:(duplicateConflicts||unsafeVerifiedRoutes||!persistenceOk)?'warning':'success',total:hs.length,withIsin,withoutIsin:Math.max(0,hs.length-withIsin),duplicateConflicts,unsafeVerifiedRoutes,persistenceOk,issues:issues.slice(0,60)});
 resolverDiag('Identity Validation 11.15.36',`${withIsin}/${hs.length} med ISIN · ${duplicateConflicts} dubblettkonflikter · ${unsafeVerifiedRoutes} osäkra verifieringar`,result.status==='success'?'success':'warning');
 if(!silent)alert(`Identitetskontroll klar.\n${withIsin}/${hs.length} innehav har ISIN\n${duplicateConflicts} ISIN-konflikter\n${unsafeVerifiedRoutes} osäkra verifieringar\nBeständig lagring: ${persistenceOk?'OK':'FEL'}`);
 render();return result
}
window.mkRunIdentityValidation=event=>{event?.preventDefault?.();runIdentityIntegrityValidation();return false};

function instrumentMappingCoverage(){
 const hs=canonicalHoldings();
 const mapped=hs.map(h=>{
  const key=h._key;
  const existing=liveFoundation.mappings[key]||{},merged=mergedIdentityForHolding(h,key);
  const ticker=merged.ticker||inferTickerForHolding(h);
  const isin=merged.isin;
  const currency=merged.currency||'SEK';
  const exchange=merged.exchange;
  const asset=merged.assetType||h._type||assetType(h);
  const complete=Boolean((ticker||isin)&&currency&&asset);
  return {key,name:h._name,account:h._account,ticker:existing.ticker||ticker,isin:normalizeIsinValue(existing.isin)||isin,currency:existing.currency||currency,exchange:existing.exchange||exchange,assetType:existing.assetType||asset,identitySource:merged.source,complete};
 });
...
function fxFor(h){const direct=nval(h,['fxToSEK','fxRate','exchangeRate','currencyRate','sekRate']);if(direct)return direct;const identity=typeof mergedIdentityForHolding==='function'?mergedIdentityForHolding(h,h._key||identity(h,0)):{};return fxForCurrency(identity.currency||text(h,['currency','valuta'],'SEK'))}
...
function canonicalHoldings(){const raw=rawHoldings();if(canonicalCache&&canonicalCache.length===raw.length)return canonicalCache;canonicalCache=raw.map((base,i)=>{const h={...base,...(admin.overrides[base._key]||{})};const r=holdingValue(h);return {...h,_i:i,_value:r.value,_method:r.method,_valuation:r.valuation,_name:holdingName(h),_account:accountName(h),_type:assetType(h),_qty:quantity(h),_gav:gav(h),_fx:fxFor(h),_quote:quoteFor(h)}});return canonicalCache}
...
function saveMappingsFromEditor(){
 document.querySelectorAll('[data-map-key]').forEach(row=>{
  ...
  const item=canonicalHoldings().find(h=>h._key===key)||{};const patch={ticker:row.querySelector('[data-map-ticker]')?.value.trim()||'',isin:normalizeIsinValue(row.querySelector('[data-map-isin]')?.value),currency:(row.querySelector('[data-map-currency]')?.value.trim()||'SEK').toUpperCase(),exchange:prev.exchange||'',assetType:item._type||prev.assetType||''};saveManualIdentity(key,item,patch);liveFoundation.mappings[key]={...prev,...patch,updatedAt:new Date().toISOString(),source:'manual-live-mapping',identitySource:'manual-identity-11.15.34'};
 });
...
/* 11.15.49 — Rate-Limit Scheduler & Exchange Symbol Resolver */
function rateLimitSchedulerState(){return merge({version:'11.15.49',lastRun:'',status:'idle',runStartedAt:'',runFinishedAt:'',providers:{},queue:[],resolvedThisRun:0,deferred:0,rateLimited:0,invalidSymbol:0,noData:0,providerErrors:0,lastMessage:''},parse(safeGet(RATE_LIMIT_SCHEDULER_KEY))||{})}
function persistRateLimitScheduler(x){x.version='11.15.49';x.lastRun=new Date().toISOString();safeSet(RATE_LIMIT_SCHEDULER_KEY,JSON.stringify(x));return x}
const PROVIDER_LIMITS={
 'Twelve Data':{minGapMs:900,maxPerRun:8,cooldownMs:15*60*1000},
 'Alpha Vantage':{minGapMs:1200,maxPerRun:24,cooldownMs:15*60*1000},
 'Finnhub':{minGapMs:1050,maxPerRun:24,cooldownMs:15*60*1000},
 'Stooq Legacy':{minGapMs:500,maxPerRun:40,cooldownMs:5*60*1000}
};
function schedulerProviderState(name,state=rateLimitSchedulerState()){const old=state.providers?.[name]||{};return {attempts:num(old.attempts),successes:num(old.successes),failures:num(old.failures),runAttempts:num(old.runAttempts),lastAttempt:num(old.lastAttempt),cooldownUntil:num(old.cooldownUntil),lastError:String(old.lastError||''),status:String(old.status||'ready')}}
function schedulerErrorKind(error){const t=String(error?.message||error||'').toLowerCase();if(/429|rate limit|frequency|too many|limit reached|cooldown/.test(t))return'rate-limit';if(/ogiltig kurs|invalid symbol|symbol.*invalid|no data|not found|unknown symbol/.test(t))return'invalid-symbol';if(/saknar|empty|ingen kurs|no quote/.test(t))return'no-data';return'provider-error'}
async function schedulerBeforeProvider(name,state){const limits=PROVIDER_LIMITS[name]||{minGapMs:500,maxPerRun:20,cooldownMs:15*60*1000},ps=schedulerProviderState(name,state),now=Date.now();if(ps.cooldownUntil>now)throw new Error(`cooldown till ${new Date(ps.cooldownUntil).toLocaleTimeString('sv-SE')}`);if(ps.runAttempts>=limits.maxPerRun)throw new Error('providerbudget för denna synk är förbrukad');const wait=Math.max(0,ps.lastAttempt+limits.minGapMs-now);if(wait)await new Promise(r=>setTimeout(r,wait));ps.lastAttempt=Date.now();ps.attempts++;ps.runAttempts++;ps.status='running';state.providers[name]=ps;persistRateLimitScheduler(state)}
function schedulerAfterProvider(name,state,{ok,error}={}){const limits=PROVIDER_LIMITS[name]||{cooldownMs:15*60*1000},ps=schedulerProviderState(name,state);if(ok){ps.successes++;ps.status='ready';ps.lastError=''}else{ps.failures++;const kind=schedulerErrorKind(error);ps.lastError=String(error?.message||error||'');ps.status=kind==='rate-limit'?'cooling':'ready';if(kind==='rate-limit')ps.cooldownUntil=Date.now()+limits.cooldownMs;state[kind==='rate-limit'?'rateLimited':kind==='invalid-symbol'?'invalidSymbol':kind==='no-data'?'noData':'providerErrors']=(state[kind==='rate-limit'?'rateLimited':kind==='invalid-symbol'?'invalidSymbol':kind==='no-data'?'noData':'providerErrors']||0)+1}state.providers[name]=ps;persistRateLimitScheduler(state)}
async function scheduledProviderCall(name,adapter,mapping,state){await schedulerBeforeProvider(name,state);try{const q=await adapter(mapping);schedulerAfterProvider(name,state,{ok:true});return q}catch(error){schedulerAfterProvider(name,state,{ok:false,error});throw error}}
function exchangeSymbolCandidates(m){const base=String(m.ticker||m.symbol||'').trim().toUpperCase(),exchange=String(m.exchange||'').trim().toUpperCase(),isin=String(m.isin||'').trim().toUpperCase();if(!base)return[];const compact=base.replace(/\s+/g,''),dash=base.replace(/\s+/g,'-'),dot=base.replace(/\s+/g,'.'),raw=[base,compact,dash,dot];const add=(suffix)=>[base,compact,dash,dot].forEach(x=>raw.push(x.endsWith(suffix)?x:x+suffix));if(/STOCKHOLM|NASDAQ STOCKHOLM|OMX|SWEDEN|STO/.test(exchange)||isin.startsWith('SE'))add('.ST');if(/OSLO|NORWAY|OSL/.test(exchange)||isin.startsWith('NO'))add('.OL');if(/COPENHAGEN|DENMARK|CPH/.test(exchange)||isin.startsWith('DK'))add('.CO');if(/HELSINKI|FINLAND|HEL/.test(exchange)||isin.startsWith('FI'))add('.HE');if(/LONDON|LSE|UNITED KINGDOM/.test(exchange)||isin.startsWith('GB'))add('.L');if(/XETRA|FRANKFURT|GERMANY|FRA/.test(exchange)||isin.startsWith('DE')){add('.DE');add('.F')}if(/TORONTO|TSX|CANADA/.test(exchange)||isin.startsWith('CA'))add('.TO'); return [...new Set(raw.filter(Boolean))].slice(0,18)
}
...
function adaptiveTickerCandidates(m){
 const raw=[m.ticker,m.providerSymbol,m.routeTicker?.twelvedata,m.routeTicker?.alphavantage,m.routeTicker?.finnhub,m.providerSymbols?.twelveData,m.providerSymbols?.alphaVantage,m.providerSymbols?.finnhub,...exchangeSymbolCandidates(m)].filter(Boolean).map(x=>String(x).trim());
 return [...new Set(raw.filter(Boolean))].slice(0,18);
}
...
function adaptiveQuoteEngineState(){return merge({version:'11.15.49',lastRun:'',status:'not-run',attempted:0,total:0,processed:0,batches:0,currentBatch:0,navHandled:0,freshCache:0,delayedCache:0,providerHits:0,fallbackHits:0,failed:0,providerPlan:{},items:[]},parse(safeGet(ADAPTIVE_QUOTE_KEY))||{})}
function persistAdaptiveQuoteEngine(x){x.version='11.15.49';x.lastRun=new Date().toISOString();safeSet(ADAPTIVE_QUOTE_KEY,JSON.stringify(x));return x}
function quoteCacheForMapping(m){const quotes=liveState().quotes||[],keys=[m.key,m.ticker,m.providerSymbol,m.isin].filter(Boolean).map(x=>String(x).toUpperCase());return quotes.find(q=>keys.includes(String(q.instrumentKey||'').toUpperCase())||keys.includes(String(q.symbol||q.ticker||q.providerSymbol||q.isin||'').toUpperCase()))||null}
function quoteFreshness(q){if(!q)return {ageHours:Infinity,fresh:false,delayed:false};const t=new Date(q.timestamp||q.updatedAt||q.date||0).getTime(),ageHours=t?Math.max(0,(Date.now()-t)/36e5):Infinity;return {ageHours,fresh:ageHours<=18,delayed:ageHours>18&&ageHours<=168}}
function providerPlanFor(m){const c=providerConfig(),remembered=adaptiveDataState().successfulRoutes?.[m.key],plan=[];if(remembered?.provider)plan.push(remembered.provider);if(c.twelveDataKey)plan.push('Twelve Data');if(c.alphaVantageKey)plan.push('Alpha Vantage');if(c.finnhubKey)plan.push('Finnhub');if(c.enableStooq)plan.push('Stooq Legacy');plan.push('Local Cache','Senast känt värde');return [...new Set(plan)]}
function renderAdaptiveQuoteEngineReport(){const s=adaptiveQuoteEngineState(),groups=new Map();for(const x of (s.items||[])){const g=x.isin||x.key||x.name;if(!groups.has(g))groups.set(g,{...x,accounts:[]});const row=groups.get(g);if(x.account&&!row.accounts.includes(x.account))row.accounts.push(x.account)}const rows=[...groups.values()].sort((a,b)=>(a.status==='failed'?-1:1)-(b.status==='failed'?-1:1)||a.name.localeCompare(b.name,'sv')).map(x=>`<div class="smartRouteRow ${x.status==='failed'?'low':x.status==='cache'?'medium':'high'}"><div><b>${escapeHtml(x.name)}</b><small>${escapeHtml((x.accounts||[]).join(' · ')||x.account||'')} · ${escapeHtml(x.statusLabel||x.status)}</small><small>${escapeHtml((x.plan||[]).join(' → '))}</small>${x.error?`<small class="negative">${escapeHtml(x.error)}</small>`:''}</div><strong>${x.provider?escapeHtml(x.provider):'—'}</strong></div>`).join('');openModal(`<div class="sectionHead"><div><h2>Full Portfolio Quote Orchestrator 11.15.49</h2><p>${s.processed||s.attempted||0}/${s.total||s.attempted||0} behandlade · ${s.providerHits} providerträffar · ${s.freshCache+s.delayedCache} cacheträffar · ${s.navHandled||0} NAV · ${s.failed} utan kurs</p></div></div><div class="grid"><div class="card metric"><span>Färsk cache</span><b>${s.freshCache}</b></div><div class="card metric"><span>Fördröjd cache</span><b>${s.delayedCache}</b></div><div class="card metric"><span>Nya providerträffar</span><b>${s.providerHits}</b></div><div class="card metric"><span>Verkligt olösta</span><b>${s.failed}</b></div></div><h3>Providerplan och utfall</h3><div class="smartRouteList">${rows||'<div class="empty">Ingen synk har körts ännu.</div>'}</div><p class="disclaimer">Alla kursrutter körs automatiskt i säkra batcher. NAV-instrument, inklusive Montrose-fonder, skickas endast till NAV Center. Samma ISIN grupperas per värdepapper med kontona samlade under samma rad. Antal, GAV, transaktioner och Ledger ändras aldrig.</p>`);return false}
window.mkOpenAdaptiveQuoteReport=function(event){event?.preventDefault?.();event?.stopPropagation?.();return renderAdaptiveQuoteEngineReport()};
async function fetchRoutedQuotes(){
 const c=providerConfig(),all=instrumentMappingCoverage().items.filter(x=>x.ticker||x.isin),previous=adaptiveQuoteEngineState(),failedKeys=new Set((previous.items||[]).filter(x=>x.status==='failed').map(x=>x.key)),batchSize=Math.max(1,Math.min(12,num(c.maxSymbolsPerSync)||12)),mappings=all.slice().sort((a,b)=>(failedKeys.has(b.key)?1:0)-(failedKeys.has(a.key)?1:0)),quotes=[],failures=[],state=adaptiveDataState(),scheduler=persistRateLimitScheduler({...rateLimitSchedulerState(),status:'running',runStartedAt:new Date().toISOString(),runFinishedAt:'',resolvedThisRun:0,deferred:0,rateLimited:0,invalidSymbol:0,noData:0,providerErrors:0,lastMessage:'Synkronisering pågår',queue:[],providers:Object.fromEntries(Object.entries(rateLimitSchedulerState().providers||{}).map(([k,v])=>[k,{...v,runAttempts:0}]))}),engine={version:'11.15.49',status:'running',attempted:0,total:mappings.length,processed:0,batches:Math.ceil(mappings.length/batchSize),freshCache:0,delayedCache:0,providerHits:0,fallbackHits:0,failed:0,navHandled:0,providerPlan:{},items:[]};let recovered=0,navRequired=0;
 const holdingsByKey=new Map(canonicalHoldings().map(h=>[h._key,h]));
 const hardNav=(holding,original)=>{const name=String(original?.name||holding?.name||'').toLowerCase();const account=String(original?.account||holding?.account||'').toLowerCase();const type=String(original?.type||holding?.type||holding?.assetType||'').toLowerCase();return type.includes('fond')||type.includes('fund')||name.includes('montrose global monthly dividend')||name.includes('montrose global leverage 125')||name.includes('aktiespararna ')||name.includes('indexnära')||name.includes('aktiefond')||name.includes('europafond')||name.includes('ålandsbanken norden dividend')||account==='savr'&&(!original?.exchange||String(original.exchange).toUpperCase()==='NAV')};
 for(let start=0;start<mappings.length;start+=batchSize){
  const batch=mappings.slice(start,start+batchSize);
  for(const original of batch){
  engine.attempted++;engine.processed=engine.attempted;
  const holding=holdingsByKey.get(original.key),cls=classifyInstrument(holding||original),pricing=hardNav(holding,original)?'nav':cls.pricing,plan=pricing==='nav'?['NAV Center']:providerPlanFor(original);engine.providerPlan[original.key]=plan;
  if(pricing==='nav'){navRequired++;engine.navHandled++;state.diagnostics[original.key]={lastTried:new Date().toISOString(),tested:['NAV Center'],lastError:navFor(holding||original)?'':'NAV saknas',successfulRoute:navFor(holding||original)?'Lokalt NAV':''};engine.items.push({key:original.key,name:original.name,account:original.account,isin:original.isin,status:'nav',statusLabel:navFor(holding||original)?'NAV-rutt klar':'NAV saknas',provider:'NAV Center',plan:['NAV Center']});continue}
  const cached=quoteCacheForMapping(original),fresh=quoteFreshness(cached);
  if(cached&&fresh.fresh){quotes.push({...cached,instrumentKey:original.key,canonicalTicker:original.ticker,provider:cached.provider||'Local Cache'});engine.freshCache++;engine.fallbackHits++;engine.items.push({key:original.key,name:original.name,account:original.account,status:'cache',statusLabel:`Färsk cache ${Math.round(fresh.ageHours)} h`,provider:cached.provider||'Local Cache',plan});continue}
  const remembered=state.successfulRoutes[original.key],candidates=[remembered?.symbol,...adaptiveTickerCandidates(original)].filter(Boolean);let q=null,last='ingen aktiv prisprovider',tested=[];
  for(const symbol of [...new Set(candidates)]){
   const m={...original,ticker:symbol,providerSymbol:symbol,routeTicker:original.providerSymbols||{}},adapters=[];
   if(remembered?.provider==='Twelve Data'&&c.twelveDataKey)adapters.push(['Twelve Data',fetchTwelveDataQuote]);
   if(remembered?.provider==='Alpha Vantage'&&c.alphaVantageKey)adapters.push(['Alpha Vantage',fetchAlphaVantageQuote]);
   if(remembered?.provider==='Finnhub'&&c.finnhubKey)adapters.push(['Finnhub',fetchFinnhubQuote]);
   if(c.twelveDataKey)adapters.push(['Twelve Data',fetchTwelveDataQuote]);if(c.alphaVantageKey)adapters.push(['Alpha Vantage',fetchAlphaVantageQuote]);if(c.finnhubKey)adapters.push(['Finnhub',fetchFinnhubQuote]);if(c.enableStooq)adapters.push(['Stooq Legacy',fetchStooqQuote]);
   const seen=new Set();
   for(const [name,adapter] of adapters){if(seen.has(name))continue;seen.add(name);try{tested.push(`${name}: ${symbol}`);q=await scheduledProviderCall(name,adapter,m,scheduler);q.instrumentKey=original.key;q.canonicalTicker=original.ticker;if(symbol!==original.ticker)recovered++;state.successfulRoutes[original.key]={provider:name,symbol,updatedAt:new Date().toISOString()};engine.providerHits++;break}catch(e){last=String(e?.message||e)}}
   if(q)break;
  }
  if(!q&&cached){q={...cached,instrumentKey:original.key,canonicalTicker:original.ticker,provider:cached.provider||'Local Cache'};engine.delayedCache++;engine.fallbackHits++;}
  ...
  if(q){quotes.push(q);engine.items.push({key:original.key,name:original.name,account:original.account,isin:original.isin,status:q===cached?'cache':'provider',statusLabel:q===cached?'Fördröjd cache':'Ny kurs hämtad',provider:q.provider,plan});}else{const errorKind=schedulerErrorKind(last);failures.push({name:original.name,ticker:original.ticker,error:last,key:original.key,kind:errorKind});scheduler.queue.push({key:original.key,name:original.name,ticker:original.ticker,isin:original.isin,error:last,kind:errorKind,retryAfter:Math.max(...Object.values(scheduler.providers||{}).map(x=>num(x.cooldownUntil)),0)});scheduler.deferred++;engine.failed++;engine.items.push({key:original.key,name:original.name,account:original.account,isin:original.isin,status:'failed',statusLabel:'Ingen kurs',provider:'',plan,error:last});}
  }
  engine.currentBatch=Math.floor(start/batchSize)+1;persistAdaptiveQuoteEngine(engine);await new Promise(resolve=>setTimeout(resolve,0));
 }
 state.lastRun=new Date().toISOString();state.summary={tested:mappings.length,updated:quotes.length,recovered,navRequired,failed:failures.length};persistAdaptiveData(state);engine.status=failures.length?'partial':'success';persistAdaptiveQuoteEngine(engine);scheduler.status=failures.length?'partial':'success';scheduler.runFinishedAt=new Date().toISOString();scheduler.resolvedThisRun=engine.providerHits+engine.freshCache+engine.delayedCache;scheduler.lastMessage=`${scheduler.resolvedThisRun} lösta · ${scheduler.deferred} uppskjutna · ${scheduler.rateLimited} rate-limit`;persistRateLimitScheduler(scheduler);resolverDiag('Full Portfolio Quote Orchestrator 11.15.49',`${engine.processed}/${mappings.length} behandlade · ${quotes.length} kursutfall · ${engine.navHandled} NAV · ${engine.providerHits} provider · ${engine.freshCache} färsk cache · ${engine.delayedCache} reservcache · ${failures.length} utan kurs`,failures.length?'warning':'success');
 return {quotes,errors:failures.length,failures,attempted:mappings.length,recovered,navRequired,engine};
}

function renderRateLimitSchedulerReport(){const s=rateLimitSchedulerState(),providers=Object.entries(s.providers||{}).map(([name,p])=>`<div class="smartRouteRow ${p.status==='cooling'?'low':p.successes?'high':'medium'}"><div><b>${escapeHtml(name)}</b><small>${p.runAttempts||0} försök denna synk · ${p.successes||0} lyckade totalt · ${p.failures||0} fel</small><small>${p.cooldownUntil>Date.now()?`Pausad till ${new Date(p.cooldownUntil).toLocaleTimeString('sv-SE')}`:'Redo'}${p.lastError?` · ${escapeHtml(p.lastError)}`:''}</small></div><strong>${escapeHtml(String(p.status||'ready').toUpperCase())}</strong></div>`).join('');const queue=(s.queue||[]).map(x=>`<div class="smartRouteRow low"><div><b>${escapeHtml(x.name)}</b><small>${escapeHtml(x.kind||'provider-error')} · ${escapeHtml(x.ticker||x.isin||'')}</small><small>${escapeHtml(x.error||'')}</small></div><strong>Väntar</strong></div>`).join('');openModal(`<div class="sectionHead"><div><h2>Rate-Limit Scheduler 11.15.49</h2><p>${s.resolvedThisRun||0} lösta · ${s.deferred||0} uppskjutna · ${s.rateLimited||0} rate-limit</p></div></div><div class="grid"><div class="card metric"><span>Rate-limit</span><b>${s.rateLimited||0}</b></div><div class="card metric"><span>Ogiltig symbol</span><b>${s.invalidSymbol||0}</b></div><div class="card metric"><span>Ingen data</span><b>${s.noData||0}</b></div><div class="card metric"><span>Providerfel</span><b>${s.providerErrors||0}</b></div></div><h3>Providerbudget och cooldown</h3><div class="smartRouteList">${providers||'<div class="empty">Ingen synk har körts ännu.</div>'}</div><h3 class="topGap">Återstående kö</h3><div class="smartRouteList">${queue||'<div class="notice okbox">Kön är tom.</div>'}</div><p class="disclaimer">Motorn väntar mellan anrop, hoppar över providers i cooldown och prioriterar tidigare olösta instrument. Endast kursdata och providerdiagnostik uppdateras.</p>`);return false}
window.mkOpenRateLimitSchedulerReport=function(event){event?.preventDefault?.();event?.stopPropagation?.();return renderRateLimitSchedulerReport()};
...
function liveQuoteFor(h){return liveQuoteForRoute(h,marketRouteForHolding(h))}
...
async function syncLiveData(){const button=document.getElementById('syncLive');if(button){button.disabled=true;button.textContent='Synkroniserar…'}const sync={...liveSyncState(),lastAttempt:new Date().toISOString(),status:'syncing',message:'Adaptive Quote Engine hämtar FX och priser',errors:[]};persistLiveSync(sync);render();try{buildInstrumentMappings();applySafeMappingSuggestions();const [fxr,qr]=await Promise.all([fetchEcbFx(),fetchRoutedQuotes()]),old=liveState(),bySymbol=new Map((old.quotes||[]).map(q=>[String(q.symbol||q.ticker||'').toUpperCase(),q]));qr.quotes.forEach(q=>bySymbol.set(String(q.symbol).toUpperCase(),q));const cacheFallbacks=Math.max(0,(qr.attempted-(qr.navRequired||0))-qr.quotes.length);if(cacheFallbacks&&(old.quotes||[]).length){const t=providerTelemetry()['local-cache']||{};updateProviderTelemetry('local-cache',{...t,attempts:num(t.attempts)+1,successes:num(t.successes)+1,lastAttempt:new Date().toISOString(),lastSuccess:new Date().toISOString(),status:'standby'})}const next={...old,providers:liveFoundation.registry,fx:{...(old.fx||{}),...fxr.rates},quotes:[...bySymbol.values()],lastSync:new Date().toISOString(),updatedAt:new Date().toISOString(),foundationVersion:VERSION};safeSet(LIVE_KEY,JSON.stringify(next));validateLiveCache();const parts=[`${qr.quotes.length}/${Math.max(0,qr.attempted-(qr.navRequired||0))} kurser`,`${fxr.count} FX-kurser`];if(cacheFallbacks)parts.push(`${cacheFallbacks} utan ny kurs`);Object.assign(sync,{lastSuccess:new Date().toISOString(),status:(qr.quotes.length||fxr.count)?(qr.errors?'partial':'success'):'partial',message:parts.join(' · '),fxUpdated:fxr.count,quotesUpdated:qr.quotes.length,errors:qr.failures.slice(0,8).map(x=>`${x.name}: ${x.error}`)});persistLiveSync(sync);liveEvent(sync.status==='success'?'success':'warning','Adaptive Quote Engine 11.15.49',sync.message,{quoteErrors:qr.errors,fxErrors:fxr.errors,route:providerRouteLabel()});render()}catch(e){Object.assign(sync,{status:'error',message:String(e?.message||e),errors:[String(e?.message||e)]});persistLiveSync(sync);liveEvent('error','Adaptive Provider Network',`Synkronisering misslyckades: ${sync.message}`);render()}}
...
function emptyPermanentRegistry(){return {version:'11.15.34',updatedAt:'',storage:'IndexedDB',routes:{},audit:[]}}
function normalizePermanentRegistry(raw){
 const registry=merge(emptyPermanentRegistry(),raw&&typeof raw==='object'?raw:{});
 if(!registry.routes||typeof registry.routes!=='object'||Array.isArray(registry.routes))registry.routes={};
 if(!Array.isArray(registry.audit))registry.audit=[];
 registry.version='11.15.34';registry.storage='IndexedDB';
 return registry
}
function permanentIdentityRegistry(){
 if(permanentRegistryCache)return permanentRegistryCache;
 permanentRegistryCache=normalizePermanentRegistry(parse(safeGet(PERMANENT_IDENTITY_KEY)));
 return permanentRegistryCache
}
function openPermanentRegistryDb(){
 if(permanentRegistryDb)return Promise.resolve(permanentRegistryDb);
 return new Promise((resolve,reject)=>{
  if(!('indexedDB'in window))return reject(new Error('IndexedDB stöds inte'));
  const req=indexedDB.open(PERMANENT_IDB_NAME,1);
  req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains(PERMANENT_IDB_STORE))db.createObjectStore(PERMANENT_IDB_STORE)};
  req.onsuccess=()=>{permanentRegistryDb=req.result;permanentRegistryDb.onversionchange=()=>permanentRegistryDb.close();resolve(permanentRegistryDb)};
  req.onerror=()=>reject(req.error||new Error('IndexedDB kunde inte öppnas'));
  req.onblocked=()=>reject(new Error('IndexedDB är blockerad av en annan flik'))
 })
}
async function idbReadPermanentRegistry(){
 const db=await openPermanentRegistryDb();
 return new Promise((resolve,reject)=>{const tx=db.transaction(PERMANENT_IDB_STORE,'readonly'),req=tx.objectStore(PERMANENT_IDB_STORE).get(PERMANENT_IDB_RECORD);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error||new Error('IndexedDB-läsning misslyckades'))})
}
async function idbWritePermanentRegistry(registry){
 const db=await openPermanentRegistryDb();
 return new Promise((resolve,reject)=>{const tx=db.transaction(PERMANENT_IDB_STORE,'readwrite');tx.objectStore(PERMANENT_IDB_STORE).put(registry,PERMANENT_IDB_RECORD);tx.oncomplete=()=>resolve(true);tx.onerror=()=>reject(tx.error||new Error('IndexedDB-skrivning misslyckades'));tx.onabort=()=>reject(tx.error||new Error('IndexedDB-skrivning avbröts'))})
}
async function idbReadResolverRecord(recordKey){
 const db=await openPermanentRegistryDb();
 return new Promise((resolve,reject)=>{const tx=db.transaction(PERMANENT_IDB_STORE,'readonly'),req=tx.objectStore(PERMANENT_IDB_STORE).get(recordKey);req.onsuccess=()=>resolve(req.result??null);req.onerror=()=>reject(req.error||new Error('IndexedDB-läsning av resolverstatus misslyckades'))})
}
async function idbWriteResolverRecord(recordKey,value){
 const db=await openPermanentRegistryDb();
 return new Promise((resolve,reject)=>{const tx=db.transaction(PERMANENT_IDB_STORE,'readwrite');tx.objectStore(PERMANENT_IDB_STORE).put(value,recordKey);tx.oncomplete=()=>resolve(true);tx.onerror=()=>reject(tx.error||new Error('IndexedDB-skrivning av resolverstatus misslyckades'));tx.onabort=()=>reject(tx.error||new Error('IndexedDB-skrivning av resolverstatus avbröts'))})
}
function scheduleResolverIdbWrite(recordKey,value,kind='state'){
 const timerName=kind==='trace'?'resolverTraceWriteTimer':kind==='meta'?'resolverMetaWriteTimer':'resolverStateWriteTimer';
 const old=kind==='trace'?resolverTraceWriteTimer:kind==='meta'?resolverMetaWriteTimer:resolverStateWriteTimer;if(old)clearTimeout(old);
 const timer=setTimeout(()=>{void idbWriteResolverRecord(recordKey,structuredClone(value)).catch(error=>{try{console.warn('Resolver IndexedDB mirror failed',recordKey,error)}catch{}})},kind==='trace'?180:80);
 if(kind==='trace')resolverTraceWriteTimer=timer;else if(kind==='meta')resolverMetaWriteTimer=timer;else resolverStateWriteTimer=timer;
}
async function flushResolverIdbState(){
 try{if(resolverRunMemory)await idbWriteResolverRecord(RESOLVER_IDB_RUN_RECORD,structuredClone(resolverRunMemory));await idbWriteResolverRecord(RESOLVER_IDB_TRACE_RECORD,structuredClone(resolverTraceBuffer.slice(-400)));const meta=parse(safeGet(RESOLVER_DIAG_KEY))||{};await idbWriteResolverRecord(RESOLVER_IDB_META_RECORD,meta);return true}catch(error){try{console.warn('Resolver IndexedDB flush failed',error)}catch{}return false}
}
function resolverRunQuality(run){
 if(!run||typeof run!=='object')return -1;
 const statusScore={running:500000,paused:400000,complete:300000,error:100000,idle:0}[run.status]||0;
 return statusScore+Math.max(num(run.processed),num(run.cursor))*1000+num(run.total)+(Array.isArray(run.targetItems)&&run.targetItems.length?500:0);
}
function chooseBestResolverRun(...runs){
 const valid=runs.filter(x=>x&&typeof x==='object');
 if(!valid.length)return null;
 return valid.sort((a,b)=>resolverRunQuality(b)-resolverRunQuality(a)||((Date.parse(b.updatedAt||'')||0)-(Date.parse(a.updatedAt||'')||0)))[0];
}
async function hydrateResolverPersistentState(){
 try{
  const [run,lines,meta]=await Promise.all([idbReadResolverRecord(RESOLVER_IDB_RUN_RECORD),idbReadResolverRecord(RESOLVER_IDB_TRACE_RECORD),idbReadResolverRecord(RESOLVER_IDB_META_RECORD)]);
  let sessionRun=null,legacyRun=null;
  try{sessionRun=parse(sessionStorage.getItem(RESOLVER_RUN_SESSION_KEY))||null}catch{}
  try{legacyRun=parse(safeGet(RESOLVER_RUN_KEY))||null}catch{}
  const restoredRun=chooseBestResolverRun(run,sessionRun,legacyRun);
  if(restoredRun&&/^11\.15\.(?:2[4-9]|30)$/.test(String(restoredRun.version||''))){
   resolverRunMemory={...merge(resolverRunDefaults(),restoredRun),version:VERSION};
   resolverRunStorage='IndexedDB';
   await idbWriteResolverRecord(RESOLVER_IDB_RUN_RECORD,structuredClone(resolverRunMemory));
  }
  let restoredLines=lines;
  if(!Array.isArray(restoredLines)){try{restoredLines=parse(sessionStorage.getItem(RESOLVER_DIAG_LINES_KEY))||parse(safeGet(RESOLVER_DIAG_LINES_KEY))||[]}catch{restoredLines=[]}}
  if(Array.isArray(restoredLines)){resolverTraceBuffer=restoredLines.slice(-400);resolverTraceStorage='IndexedDB';if(resolverTraceBuffer.length)await idbWriteResolverRecord(RESOLVER_IDB_TRACE_RECORD,structuredClone(resolverTraceBuffer))}
  if(meta&&typeof meta==='object'){try{safeSet(RESOLVER_DIAG_KEY,JSON.stringify({...meta,version:VERSION,storage:'IndexedDB'}))}catch{}}
  resolverDiag('Resolver state 01 IndexedDB återställd',`run=${resolverRunMemory?.status||'idle'} ${resolverRunMemory?.processed||0}/${resolverRunMemory?.total||0} · logg=${resolverTraceBuffer.length}`,'success');
  return true
 }catch(error){resolverDiag('Resolver state 02 IndexedDB återställning misslyckades',String(error?.message||error),'error');return false}
}
async function initializePermanentRegistryStorage(){
 const legacy=normalizePermanentRegistry(parse(safeGet(PERMANENT_IDENTITY_KEY)));
 try{
  const stored=normalizePermanentRegistry(await idbReadPermanentRegistry());
  const merged=normalizePermanentRegistry({...legacy,...stored,routes:{...(legacy.routes||{}),...(stored.routes||{})},audit:[...(stored.audit||[]),...(legacy.audit||[])].slice(0,120)});
  await idbWritePermanentRegistry(merged);
  permanentRegistryCache=normalizePermanentRegistry(await idbReadPermanentRegistry());
  permanentRegistryStorage='IndexedDB';
  try{localStorage.removeItem(PERMANENT_IDENTITY_KEY)}catch{}
  resolverDiag('Permanent storage 01 IndexedDB klar',`${Object.keys(permanentRegistryCache.routes||{}).length} poster · legacy migrerad och frigjord`,'success');
  return true
 }catch(error){
  permanentRegistryCache=legacy;permanentRegistryStorage='localStorage fallback';
  resolverDiag('Permanent storage 02 IndexedDB fel',String(error?.message||error),'error');
  return false
 }
}
function storageFootprint(){
 let chars=0,keys=0;
 try{for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||'',v=localStorage.getItem(k)||'';chars+=k.length+v.length;keys++}}catch{}
 return {keys,bytes:chars*2,kb:Math.round(chars*2/1024)}
}
function compactReadOnlyStorageForRegistry(){
 const actions=[];
 try{
  [RESOLVER_DIAG_KEY,RESOLVER_DIAG_LINES_KEY].forEach(k=>{if(localStorage.getItem(k)!==null){localStorage.removeItem(k);actions.push(`rensade ${k}`)}});
  const events=parse(localStorage.getItem(LIVE_EVENT_KEY));if(Array.isArray(events)&&events.length>80){localStorage.setItem(LIVE_EVENT_KEY,JSON.stringify(events.slice(0,80)));actions.push(`komprimerade livehändelser ${events.length}→80`)}
  const telemetry=parse(localStorage.getItem(PROVIDER_TELEMETRY_KEY));if(telemetry&&typeof telemetry==='object'){
   const slim={};for(const [k,v] of Object.entries(telemetry))slim[k]={attempts:num(v.attempts),successes:num(v.successes),failures:num(v.failures),avgLatency:num(v.avgLatency),lastAttempt:v.lastAttempt||'',lastSuccess:v.lastSuccess||'',lastError:v.lastError||'',status:v.status||''};
   localStorage.setItem(PROVIDER_TELEMETRY_KEY,JSON.stringify(slim));actions.push('komprimerade providertelemetri')
  }
 }catch(e){actions.push(`komprimering fel: ${String(e?.message||e)}`)}
 return actions
}
async function persistPermanentIdentityRegistry(x){
 x=normalizePermanentRegistry(x);x.updatedAt=new Date().toISOString();x.audit=(x.audit||[]).slice(0,120);
 const payload=JSON.stringify(x),before=storageFootprint();
 resolverDiag('Permanent write 04 IndexedDB-skrivning start',`${payload.length} tecken · localStorage≈${before.kb} KB · mål=${PERMANENT_IDB_NAME}`);
 try{
  await idbWritePermanentRegistry(x);
  const read=normalizePermanentRegistry(await idbReadPermanentRegistry());
  const verified=Boolean(read&&read.routes&&typeof read.routes==='object');
  if(!verified)throw new Error('Direkt återläsning saknar routes');
  permanentRegistryCache=read;permanentRegistryStorage='IndexedDB';
  try{localStorage.removeItem(PERMANENT_IDENTITY_KEY)}catch{}
  resolverDiag('Permanent write 08 IndexedDB återläsning klar',`routes=${Object.keys(read.routes||{}).length} · localStorage påverkas inte`,'success');
  return true
 }catch(error){
  resolverDiag('Permanent write 07 IndexedDB-skrivning misslyckades',String(error?.message||error),'error');
  return false
 }
}
...
function samePermanentRoute(saved,route){
 if(!routeIsUsable(saved))return false;
 const norm=x=>String(x||'').trim().toUpperCase();
 return norm(saved.ticker)===norm(route.ticker)&&norm(saved.isin)===norm(route.isin)&&norm(saved.provider)===norm(route.provider)&&norm(saved.currency)===norm(route.currency)&&norm(saved.exchange)===norm(route.exchange)
}
async function savePermanentIdentityRoute(key,route,meta={}){
 resolverDiag('Permanent write 01 Start',`holdingKey=${key||'–'} · ticker=${route?.ticker||'–'} · isin=${route?.isin||'–'} · storage=IndexedDB`);
 if(!key||!routeIsUsable(route)){resolverDiag('Permanent write 02 Validering stopp',`ogiltig nyckel eller route`,'error');return false}
 const registry=normalizePermanentRegistry(structuredClone(permanentIdentityRegistry())),previous=registry.routes?.[key]||null;
 resolverDiag('Permanent write 03 Register läst',`${Object.keys(registry.routes||{}).length} poster före skrivning · ${permanentRegistryStorage}`);
 const identityVerified=validIsin(route.isin)||String(route.source||meta.source||'').includes('Global Registry');registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.34',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver',identityStatus:identityVerified?'verified':'manual-market-route',identityVerified,marketRouteApproved:!identityVerified};
 registry.audit=[{date:new Date().toISOString(),key,provider:route.provider,ticker:route.ticker||'',isin:route.isin||'',action:previous?'updated':'created'},...(registry.audit||[])].slice(0,120);
 if(!await persistPermanentIdentityRegistry(registry))return false;
 const verified=permanentRouteFor(key),exact=samePermanentRoute(verified,registry.routes[key]);
 resolverDiag(exact?'Permanent write 09 Post verifierad':'Permanent write 09 Post saknas/fel',`holdingKey=${key} · routes=${Object.keys(permanentIdentityRegistry().routes||{}).length} · ticker=${verified?.ticker||'–'} · börs=${verified?.exchange||'–'} · valuta=${verified?.currency||'–'}`,exact?'success':'error');
 return exact
}
async function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:route.identityVerified===true,identityStatus:route.identityStatus||(validIsin(route.isin)?'verified':'manual-market-route'),marketRouteApproved:route.marketRouteApproved===true,identityConfidence:route.identityVerified===true?(num(route.confidence)||96):Math.min(89,num(route.confidence)||89),resolverVersion:'11.15.34',source:'permanent-identity-registry-11.15.34',updatedAt:new Date().toISOString()};restored++}
 }
 const state=globalResolverState();
 for(const [key,route] of Object.entries(state.routes||{})){if(routeIsUsable(route)&&!registry.routes?.[key]){if(await savePermanentIdentityRoute(key,route,{source:'migration-global-resolver'}))migrated++}}
 for(const [key,m] of Object.entries(liveFoundation.mappings||{})){const route=m.permanentRoute||(m.verified&&num(m.identityConfidence||m.confidence)>=92?{provider:m.provider||'Verified mapping',ticker:m.ticker||'',isin:m.isin||'',exchange:m.exchange||'',currency:m.currency||'SEK',figi:m.figi||'',confidence:num(m.identityConfidence||m.confidence)||92,verifiedAt:m.verifiedAt||m.updatedAt||new Date().toISOString()}:null);if(routeIsUsable(route)&&!permanentRouteFor(key)){if(await savePermanentIdentityRoute(key,route,{source:'migration-live-foundation'}))migrated++}}
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.34',`${restored} rutter återställda · ${migrated} migrerade till IndexedDB`,{})}
...
function resolverScore(item,c){
 ...
 if(!validIsin(candIsin)&&c.provider!=='Global Registry')score=Math.min(score,89);
...
function resolverDiagnosticState(){
 ...
 const meta=merge({version:'11.15.34',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false,storage:resolverTraceStorage},raw);
 meta.lines=resolverDiagnosticLines();meta.storage=resolverTraceStorage;return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.34',startedAt:x?.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x?.heartbeat),currentInstrument:x?.currentInstrument||'',currentStep:x?.currentStep||'',expanded:!!x?.expanded,storage:'IndexedDB'};
 try{safeSet(RESOLVER_DIAG_KEY,JSON.stringify(meta))}catch{}
 scheduleResolverIdbWrite(RESOLVER_IDB_META_RECORD,meta,'meta');return meta
}
function persistResolverDiagnosticLines(lines){
 ...
 try{sessionStorage.setItem(RESOLVER_DIAG_LINES_KEY,payload)}catch{}
 resolverTraceStorage='IndexedDB';
 scheduleResolverIdbWrite(RESOLVER_IDB_TRACE_RECORD,resolverTraceBuffer,'trace');
 return resolverTraceBuffer
}
...
function resolverRunDefaults(){return {version:'11.15.34',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,fundPending:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],providerCooldowns:{},message:''}}
function resolverRunState(){
 ...
 if(run.version!==VERSION){
  if(raw&&/^11\.15\.(?:2[4-9]|30)$/.test(String(raw.version||''))){run={...run,version:VERSION,message:run.message||'Körstatus migrerad till IndexedDB Unified State Recovery.'}}
  else{run=resolverRunDefaults();resolverRunStorage='memory'}
 }
 resolverRunMemory=run;
 return run
}
function persistResolverRun(x){
 let run=merge(resolverRunDefaults(),x||{});run.version=VERSION;run.updatedAt=new Date().toISOString();
 const current=resolverRunMemory;
 // 11.15.34: cooldowns are monotonic state. A stale loop snapshot must never erase
 // a newer provider failure/cooldown written while the resolver was awaiting a provider.
 const mergedCooldowns={};
 for(const source of [current?.providerCooldowns,run.providerCooldowns]){
  if(!source||typeof source!=='object')continue;
  for(const [name,value] of Object.entries(source)){
   if(!value||typeof value!=='object')continue;
   const prev=mergedCooldowns[name]||{};
   mergedCooldowns[name]={...prev,...value,failures:Math.max(num(prev.failures),num(value.failures)),until:Math.max(num(prev.until),num(value.until))};
  }
 }
 run.providerCooldowns=mergedCooldowns;
 if(run.status==='idle'&&num(run.total)===0&&current&&resolverRunQuality(current)>resolverRunQuality(run)){
  resolverDiag('Resolver state 02 Tom idle-skrivning blockerad',`bevarar ${current.status} ${current.processed||0}/${current.total||0}`,'warning');
  run={...current,version:VERSION,updatedAt:new Date().toISOString()};
 }
 resolverRunMemory=run;resolverRunStorage='IndexedDB';
 const payload=JSON.stringify(run);
 try{sessionStorage.setItem(RESOLVER_RUN_SESSION_KEY,payload)}catch{}
 scheduleResolverIdbWrite(RESOLVER_IDB_RUN_RECORD,run,'state');
 return run
}
...
const RESOLVER_PROVIDER_COOLDOWN_KEY='mastarklass_os_11_15_30_provider_cooldowns';
function resolverProviderCooldowns(){const run=resolverRunState();return run.providerCooldowns&&typeof run.providerCooldowns==='object'?run.providerCooldowns:{}}
function persistResolverProviderCooldowns(x){const run=resolverRunState();run.providerCooldowns=x&&typeof x==='object'?x:{};persistResolverRun(run);return run.providerCooldowns}
function providerCooldownInfo(name){const all={...resolverProviderCooldowns()},x=all[name];if(!x)return null;if(num(x.until)<=Date.now()){delete all[name];persistResolverProviderCooldowns(all);return null}return x}
function registerProviderFailure(name,message){const all={...resolverProviderCooldowns()},prev=all[name]||{failures:0,until:0};const failures=num(prev.failures)+1,lower=String(message||'').toLowerCase(),rateLimited=/429|rate limit|too many/.test(lower),network=/failed to fetch|network|cors/.test(lower),threshold=name==='OpenFIGI'?1:rateLimited?1:4;let until=num(prev.until);if(rateLimited||(network&&failures>=threshold))until=Date.now()+15*60*1000;all[name]={failures,until,lastError:String(message||''),updatedAt:new Date().toISOString()};persistResolverProviderCooldowns(all);return all[name]}
function registerProviderSuccess(name){
 const all={...resolverProviderCooldowns()},current=all[name];
 if(!current)return;
 // A successful request clears historical failures only when no active cooldown exists.
 // Active cooldowns are never shortened by a late/stale success callback.
 if(num(current.until)>Date.now())return;
 delete all[name];persistResolverProviderCooldowns(all)
}
function providerCooldownLabel(name){const x=providerCooldownInfo(name);if(!x)return '';return `${name} pausad ${Math.max(1,Math.ceil((x.until-Date.now())/60000))} min`}
async function resolveOneInstrument(item){
 const holding=canonicalHoldings().find(h=>h._key===item.key)||item,merged=mergedIdentityForHolding(holding,item.key),m=liveFoundation.mappings[item.key]||{};
 item={...item,ticker:merged.ticker||m.ticker||item.ticker||'',isin:merged.isin||normalizeIsinValue(m.isin)||normalizeIsinValue(item.isin),exchange:merged.exchange||m.exchange||item.exchange||'',currency:merged.currency||m.currency||item.currency||'SEK',assetType:merged.assetType||item.assetType};
 resolverDiag('Instrument laddat',`${item.name} · ${item.ticker||'ingen ticker'} · ${item.isin||'inget ISIN'} · källa=${merged.source||'okänd'}`);
 ...
 resolverDiag('Identity Integrity merge',`${item.ticker||'–'} · ${item.isin||'–'} · ${item.exchange||'–'} · ${item.currency||'–'}`,'success');
 const instrumentClass=classifyInstrument(holding).code;
 ...
 if(instrumentClass==='fund'&&validIsin(item.isin)){
  const exact={ticker:item.ticker||'',isin:item.isin,name:item.name,exchange:item.exchange||'',currency:item.currency||'SEK',type:item.assetType||'Fond',provider:'Manual ISIN',score:100};
  resolverDiag('Fond/ISIN säker identitet',`${item.isin} · aktieproviders hoppas över`,'success');
  return {key:item.key,name:item.name,account:item.account,assetType:item.assetType,currency:item.currency,best:exact,candidates:[exact],confidence:100,margin:100,conflict:false,autoSafe:true,fundPending:false,variants:tickerVariants(item),exchanges:exchangeVariants(item),errors:[]};
 }
 let candidates=[],errors=[];
 if(validIsin(item.isin))candidates.push({ticker:item.ticker||'',isin:item.isin,name:item.name,exchange:item.exchange||'',currency:item.currency||'SEK',type:item.assetType||'',provider:'Manual ISIN'});
 ...
 if(unique[0]&&unique[0].score>=98&&(unique[0].ticker||instrumentClass==='fund'||unique[0].provider==='Global Registry')){resolverDiag('Lokal säker träff','Externa providers hoppas över','success');return {key:item.key,name:item.name,account:item.account,assetType:item.assetType,currency:item.currency,best:unique[0],candidates:unique.slice(0,4),confidence:unique[0].score,margin:100,conflict:false,autoSafe:true,variants:tickerVariants(item),exchanges:exchangeVariants(item),errors}}
 ...
 for(const [name,fn] of providers){
  ...
  const cooldown=providerCooldownInfo(name);if(cooldown){const label=providerCooldownLabel(name);resolverDiag(`${name} cooldown`,`${label} · senaste fel: ${cooldown.lastError||'nätverksfel'}`,'warning');errors.push(`${name}: ${label}`);continue}
  resolverDiag(`${name} start`,`${item.ticker||item.isin||item.name}`);
  const began=Date.now();
  try{const rows=await withDeadline(Promise.resolve().then(fn),6500,name);registerProviderSuccess(name);candidates.push(...(rows||[]));resolverDiag(`${name} klar`,`${rows?.length||0} kandidater · ${Date.now()-began} ms`,'success')}
  catch(error){const msg=String(error?.message||error);const state=registerProviderFailure(name,msg);errors.push(`${name}: ${msg}`);resolverDiag(`${name} fel`,`${msg} · ${Date.now()-began} ms${state.until>Date.now()?' · provider pausad 15 min':''}`,'error')}
 }
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.34',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,fundPending:run.fundPending||0,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
function updateResolverProgressUI(run=resolverRunState()){
 ...
 const fields={resolverRemaining:Math.max(0,(run.total||0)-(run.processed||0)),resolverPermanent:Object.keys(permanentIdentityRegistry().routes||{}).length,resolverPortfolioRemaining:Math.max(0,canonicalHoldings().length-Object.keys(permanentIdentityRegistry().routes||{}).length),resolverSafe:run.newSafe||0,resolverReview:run.review||0,resolverSkipped:run.skipped||0,resolverFundPending:run.fundPending||0,resolverFailed:run.failed||0};for(const [id,v] of Object.entries(fields)){const el=document.getElementById(id);if(el)el.textContent=v}
...
async function runGlobalResolver(runId){
 ...
 for(let i=run.cursor;i<batchEnd;i++){
  ...
  try{
   ...
   if(routeIsUsable(permanentRouteFor(item.key))){resolverDiag('Permanent träff','Instrument hoppas över','success');run=resolverRunState();run.cursor=i+1;run.processed++;run.skipped++;}
   else if(validIsin(item.isin)&&findPermanentRouteByIsin(item.isin)){
    const reused=findPermanentRouteByIsin(item.isin);resolverDiag('Permanent ISIN-återanvändning',`${item.isin} från ${reused.key}`,'success');await savePermanentIdentityRoute(item.key,{...reused.route,source:'isin-deduplicated-11.15.34'},{source:'isin-deduplicated-11.15.34'});run=resolverRunState();run.cursor=i+1;run.processed++;run.skipped++;run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length;
   }
   else{
    ...
    if(row.autoSafe&&await applyResolverSuggestion(row.key,0,{silent:true,rowOverride:row})){run.newSafe++;run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length}
...
async function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){
 ...
 resolverDiag('Permanent approve 01 Knapptryck',`holdingKey=${key} · kandidatindex=${index}`);
 if(!row||!candidate){resolverDiag('Permanent approve 02 Kandidat saknas',`row=${Boolean(row)} · candidate=${Boolean(candidate)}`,'error');return false}
 const current=liveFoundation.mappings[key]||{};
 if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;
 const route={provider:candidate.provider,ticker:String(candidate.ticker||'').trim(),isin:String(candidate.isin||'').trim(),exchange:String(candidate.exchange||'').trim(),currency:String(candidate.currency||row.currency||'SEK').trim(),figi:candidate.figi||'',confidence:num(candidate.score),verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.34'};
 if(route.ticker==='0'||!routeIsUsable(route)){resolverDiag('Permanent approve 03 Validering stopp',`ticker=${route.ticker||'–'} · provider=${route.provider||'–'} · valuta=${route.currency||'–'}`,'error');if(!silent)alert('Kandidaten kunde inte sparas: ticker/ISIN, provider eller valuta saknas.');return false}
 resolverDiag('Permanent approve 04 Kandidat godkänd',`${row.name} · ${route.ticker||route.isin} · ${route.exchange||'okänd börs'} · ${route.currency}`,'success');
 if(!await savePermanentIdentityRoute(key,route,{source:'global-resolver-approved-11.15.34'})){if(!silent)alert('Permanent lagring i IndexedDB misslyckades. Instrumentet ligger kvar och ingen portföljdata har ändrats. Kopiera Resolver Chain Trace för exakt fel.');return false}
 const verified=permanentRouteFor(key);
 if(!samePermanentRoute(verified,route)){resolverDiag('Permanent approve 05 Direktverifiering fel',`holdingKey=${key}`,'error');if(!silent)alert('Lagringen kunde inte verifieras genom exakt återläsning. Instrumentet ligger kvar. Kopiera Resolver Chain Trace.');return false}
 const holding=canonicalHoldings().find(h=>h._key===key)||row;
 liveFoundation.mappings[key]={...current,...verified,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:verified.ticker||verified.isin},instrumentClass:classifyInstrument(holding).code,verified:true,identityStatus:'verified',identityConfidence:num(verified.confidence)||route.confidence,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.43'],source:'permanent-identity-registry-11.15.34',resolverVersion:'11.15.34',permanentRoute:verified,updatedAt:new Date().toISOString()};
 persistLiveFoundation();
 state.routes=state.routes||{};state.routes[key]=verified;state.suggestions=(state.suggestions||[]).filter(x=>x.key!==key);persistGlobalResolver(state);
 run.suggestions=(run.suggestions||[]).filter(x=>x.key!==key);run.review=Math.max(0,num(run.review)-1);run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length;persistResolverRun(run);syncResolverSummary(run);
 resolverDiag('Permanent approve 06 Klar',`${row.name} borttagen från granskning · permanent=${run.permanent} · kvar=${run.review}`,'success');
 liveEvent('success','Permanent Identity Registry 11.15.34',`${row.name} sparad permanent som ${route.ticker||route.isin} · ${route.exchange||'okänd börs'} · ${route.currency}`,verified);
 return {name:row.name,ticker:route.ticker||route.isin,exchange:route.exchange||'okänd börs',currency:route.currency};
}
async function applySafeResolverSuggestions(){const run=resolverRunState();let count=0;for(const row of run.suggestions||[]){if(row.autoSafe&&!routeIsUsable(permanentRouteFor(row.key))&&await applyResolverSuggestion(row.key,0))count++}return count}
function resolverCandidateIdentity(item,c,row){
 const itemIsin=String(item?.isin||'').trim().toUpperCase(),candIsin=String(c?.isin||'').trim().toUpperCase();
 const hasIsin=validIsin(candIsin),exactIsin=Boolean(hasIsin&&itemIsin&&candIsin===itemIsin);
 const exchange=String(c?.exchange||'').trim(),currency=String(c?.currency||row?.currency||'').trim().toUpperCase(),type=String(c?.type||'').trim();
 const exchangeKnown=Boolean(exchange),currencyKnown=Boolean(currency),typeKnown=Boolean(type);
 const raw=num(c?.score),displayScore=exactIsin?raw:hasIsin?Math.min(raw,97):Math.min(raw,89);
 let level='ticker',label='Tickerförslag – ISIN saknas',tone='warning';
 if(exactIsin){level='verified';label='Verifierad – exakt ISIN';tone='success'}
 else if(hasIsin){level='isin';label='Stark kandidat – ISIN finns';tone='success'}
 else if(row?.conflict){level='conflict';label='Konflikt – kontrollera börs och ISIN';tone='danger'}
 const matches=[];
 if(exactIsin)matches.push('ISIN exakt');else if(hasIsin)matches.push('ISIN tillgängligt');else matches.push('ISIN saknas');
 matches.push(exchangeKnown?`Börs ${exchange}`:'Börs saknas');
 matches.push(currencyKnown?`Valuta ${currency}`:'Valuta saknas');
 matches.push(typeKnown?`Typ ${type}`:'Typ saknas');
 return {hasIsin,exactIsin,displayScore,level,label,tone,matches,exchange,currency,type,candIsin};
}
function resolverCandidateInspector(row,c,index,item){
 const id=resolverCandidateIdentity(item,c,row),checked=index===0?'checked':'';
 return `<label class="resolverCandidateCard ${esc(id.level)}"><input type="radio" name="resolver-choice-${esc(row.key)}" data-resolver-choice="${esc(row.key)}" value="${index}" ${checked}><span class="candidateRadio"></span><span class="candidateBody"><span class="candidateHead"><b>${esc(c.ticker||c.isin||'Okänd symbol')}</b><em>${id.displayScore}%</em></span><strong>${esc(c.name||row.name||'Okänt instrument')}</strong><span class="candidateIdentity ${esc(id.tone)}">${esc(id.label)}</span><dl><div><dt>ISIN</dt><dd>${id.hasIsin?esc(id.candIsin):'<span class="missingIdentity">Saknas</span>'}</dd></div><div><dt>Börs</dt><dd>${esc(id.exchange||'Okänd')}</dd></div><div><dt>Valuta</dt><dd>${esc(id.currency||'Okänd')}</dd></div><div><dt>Typ</dt><dd>${esc(id.type||'Okänd')}</dd></div><div><dt>Provider</dt><dd>${esc(c.provider||'Okänd')}</dd></div></dl><small>${id.matches.map(esc).join(' · ')}</small></span></label>`;
}
function resolverReviewHtml(){
 const run=resolverRunState(),holdings=new Map(instrumentMappingCoverage().items.map(x=>[x.key,x]));
 const rows=(run.suggestions||[]).map(row=>{const best=row.best,applied=routeIsUsable(permanentRouteFor(row.key)),item=holdings.get(row.key)||row,bestIdentity=best?resolverCandidateIdentity(item,best,row):null,candidates=(row.candidates||[]).slice(0,4);
 return `<article data-resolver-row="${esc(row.key)}" class="resolverResult ${row.conflict?'conflict':row.autoSafe?'safe':best?'review':'missing'}"><div class="resolverTitle"><div><b>${esc(row.name)}</b><small>${esc(row.account)} · ${esc(row.assetType)}</small></div><span>${applied?'Permanent':bestIdentity?bestIdentity.label:'Ingen träff'}</span></div>${best?`<div class="resolverBest identityFirst"><strong>${esc(best.ticker||best.isin)}</strong><p>${esc(best.name||'')} · ${esc(best.exchange||'Okänd börs')} · ${esc(best.currency||row.currency||'')}</p><div class="primaryIsin"><span>ISIN</span><b>${bestIdentity.hasIsin?esc(bestIdentity.candIsin):'SAKNAS'}</b></div><small>${esc(best.provider)}${row.conflict?' · konflikt mellan källor':''}</small></div><div class="candidateGuidance"><b>Välj efter ISIN först</b><span>Kontrollera därefter börs, valuta och instrumenttyp. En kandidat utan ISIN är endast ett tickerförslag och visas aldrig som 100 % verifierad.</span></div><div class="resolverCandidateList">${candidates.map((c,i)=>resolverCandidateInspector(row,c,i,item)).join('')}</div><div class="resolverActions"><button class="primary" data-resolver-apply="${esc(row.key)}" ${applied?'disabled':''}>${applied?'Sparad permanent':bestIdentity?.hasIsin?'Godkänn verifierad identitet':'Godkänn tickerförslag manuellt'}</button></div>`:row.fundPending?`<div class="notice"><b>Behöver fondidentitet</b><br>Traditionell fond utan ISIN. Aktieproviders hoppades över. Lägg till ISIN eller NAV-identitet i ett senare steg.</div>`:`<div class="notice">Ingen säker identitet hittades. ${esc((row.errors||[]).join(' · ')||'Ingen provider gav användbar träff.')}</div>`}</article>`}).join('');
 const categorized=Math.min(num(run.processed),num(run.newSafe)+num(run.review)+num(run.fundPending)+num(run.failed)+num(run.skipped));
 return `<div class="eyebrow">Global Identity Resolver 11.15.43</div><h2>ISIN-first Candidate Inspector</h2><p class="disclaimer">ISIN är primär identitetsnyckel. Ticker, börs, valuta och instrumenttyp används som sekundära kontroller. Kandidater utan ISIN kan granskas men räknas inte som verifierade identiteter.</p><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`
}
function openGlobalResolverReview(){
 const oldScroll=document.querySelector('.modalCard')?.scrollTop||0;
 openModal(resolverReviewHtml());
 const card=document.querySelector('.modalCard');if(card)card.scrollTop=oldScroll;
 document.querySelectorAll('[data-resolver-apply]').forEach(btn=>btn.addEventListener('click',async event=>{
  event.preventDefault();event.stopPropagation();
  const modalCard=document.querySelector('.modalCard'),scroll=modalCard?.scrollTop||0,key=btn.dataset.resolverApply,rowEl=btn.closest('[data-resolver-row]'),rowTop=rowEl?rowEl.getBoundingClientRect().top:0,sel=document.querySelector(`[data-resolver-choice="${CSS.escape(key)}"]:checked`)||document.querySelector(`[data-resolver-choice="${CSS.escape(key)}"]`),idx=num(sel?.value);
  btn.disabled=true;btn.textContent='Sparar…';
  const saved=await applyResolverSuggestion(key,idx);
  if(saved){refreshValuationAfterResolver();openGlobalResolverReview();const next=document.querySelector('.modalCard');if(next){next.scrollTop=scroll;requestAnimationFrame(()=>{const first=next.querySelector('[data-resolver-row]');if(first&&rowTop){const delta=first.getBoundingClientRect().top-rowTop;next.scrollTop=Math.max(0,next.scrollTop+delta)}})}const toast=document.createElement('div');toast.className='saveToast';toast.setAttribute('role','status');toast.textContent=`${saved.name} sparad permanent som ${saved.ticker} · ${saved.exchange} · ${saved.currency}`;document.body.appendChild(toast);setTimeout(()=>toast.remove(),3500)}
  else{btn.disabled=false;btn.textContent='Godkänn och spara permanent'}
 }))
}
function globalResolverPanel(){const st=globalResolverState(),run=resolverRunState(),remaining=run.total?Math.max(0,run.total-run.processed):resolverTargets().length,saved=Object.keys(permanentIdentityRegistry().routes||{}).length,running=run.status==='running',paused=run.status==='paused';return `<section class="section globalResolverPanel"><div class="sectionHead"><div><div class="eyebrow">Global Identity Resolver 11.15.43</div><h2>Stabil resolver med tydlig resultatbalans.</h2></div><span class="healthScore">${saved} låsta</span></div><p>Resolvern arbetar i batcher om högst åtta instrument, bevarar checkpoint och granskningsresultat i IndexedDB och pausar tillfälligt providers som upprepade gånger misslyckas. Fonder utan ISIN skickas inte till aktieproviders.</p>${(()=>{const b=portfolioIdentityBridgeState(),ui=resolverInteractionState();return `<div class="portfolioBridgeBox"><div><b>Portfolio Identity Bridge 11.15.43</b><span>${b.status==='not-run'?'Inte synkad':`${b.withIsin}/${b.total} med ISIN · källa ${b.source}`}</span><small>Senaste synkning: ${resolverInteractionTime(ui.lastSync)}</small></div><button id="mk-portfolio-bridge-btn" class="primary" type="button">Synka portfölj till identitetsmotorn</button></div>`})()}${(()=>{const c=buildUnifiedIdentityReport(),ui=resolverInteractionState();return `<div class="identityConflictBox"><div><b>Unified Identity Engine 11.15.43</b><span>${c.withIsin}/${c.total} med ISIN · ${c.withoutIsin} saknar · ${c.conflicts.length} konflikter</span><small>Senast öppnad rapport: ${resolverInteractionTime(ui.lastConflictOpen)}</small></div><div class="identityConflictActions"><button id="mk-conflict-resolver-btn" class="primary" type="button">Visa konflikter och saknade ISIN</button><button id="mk-conflict-fallback-btn" class="secondary" type="button">Öppna rapport direkt</button><button class="secondary" type="button" onclick="return window.mkRefreshUnifiedIdentity(event)">Läs om alla identiteter</button></div></div>`})()}${marketRoutingPanel()}${(()=>{const s=storageMigrationState();return `<div class="storageMigrationBox"><div><b>Cleanup Safety Lock 11.15.40</b><span>${s.status==='not-run'?'Inte körd':`${s.beforeKb} KB → ${s.afterKb} KB · säkerhetslås aktivt · ${s.removed?.length||0} tillåtna poster senast`}</span></div><button class="primary" type="button" onclick="return window.mkPreviewSafeCleanup(event)">Förhandsgranska säker rensning</button></div>`})()}${(()=>{const p=persistenceRepairState();return `<div class="persistenceRepairBox"><div><b>Persistence Repair 11.15.36</b><span>${p.status==='not-run'?'Inte körd':`localStorage ${p.localStorage?.ok?'OK':'fel'} · IndexedDB ${p.indexedDB?.ok?'OK':'fel'} · Registry ${p.registry?.ok?'OK':'fel'}`}</span></div><button class="primary" type="button" onclick="return window.mkRunPersistenceRepair(event)">Reparera och testa lagring</button></div>`})()}<div class="isinStageBox"><b>När ska ISIN läggas in?</b><span><strong>Steg 1:</strong> Cleanup körs aldrig automatiskt. Använd endast förhandsgranskad säker rensning vid behov.</span><span><strong>Steg 2:</strong> Kontrollera att Persistence Repair visar OK för alla tre lagren.</span><span><strong>Steg 3:</strong> Kör Sök gamla ISIN och Kontrollera identiteter.</span><span><strong>Steg 4:</strong> Kör Portfolio Identity Bridge och öppna därefter Unified Identity Engine.</span><span><strong>Steg 5:</strong> Spara ISIN via Portfölj → Administrera → Redigera befintligt innehav → ISIN → Spara direktkorrigering.</span><span><strong>Steg 6:</strong> Kontrollera att räknaren ökar och kör därefter resolverbatcher samt livekurser/NAV.</span></div>${(()=>{const r=identityRecoveryState();return `<div class="identityRecoveryBox"><div><b>ISIN Recovery 11.15.34</b><span>${r.status==='not-run'?'Inte körd':`${r.recovered} återställda · ${r.conflicts} konflikter · ${r.unmatched} utan träff`}</span></div><button class="secondary" type="button" onclick="return window.mkRunIdentityRecovery(event)">Sök gamla ISIN</button></div>`})()}${(()=>{const v=identityValidationState();return `<div class="identityValidationBox"><div><b>Recovery Validation 11.15.36</b><span>${v.status==='not-run'?'Inte körd':`${v.withIsin}/${v.total} med ISIN · ${v.duplicateConflicts} konflikter · lagring ${v.persistenceOk?'OK':'fel'}`}</span></div><button class="secondary" type="button" onclick="return window.mkRunIdentityValidation(event)">Kontrollera identiteter</button></div>`})()}<div class="grid"><div class="card metric"><span>Batch återstår</span><b id="resolverRemaining">${remaining}</b></div><div class="card metric"><span>Permanent sparade</span><b id="resolverPermanent">${saved}</b></div><div class="card metric"><span>Portfölj återstår</span><b id="resolverPortfolioRemaining">${Math.max(0,canonicalHoldings().length-saved)}</b></div><div class="card metric"><span>Nya säkra</span><b id="resolverSafe">${run.newSafe||0}</b></div><div class="card metric"><span>För granskning</span><b id="resolverReview">${run.review||0}</b></div><div class="card metric"><span>Redan permanent</span><b id="resolverSkipped">${run.skipped||0}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b id="resolverFundPending">${run.fundPending||0}</b></div><div class="card metric"><span>Misslyckade</span><b id="resolverFailed">${run.failed||0}</b></div></div><div class="resolverProgress"><div><span id="resolverStatusLabel">${running?'Kör':paused?'Pausad':'Redo'}</span><b><span id="resolverProcessed">${run.processed||0}</span>/<span id="resolverTotal">${run.total||0}</span></b></div><div class="bar"><i id="resolverProgressBar" style="width:${run.total?Math.round((run.processed||0)/run.total*100):0}%"></i></div><small id="resolverMessage">${esc(run.message||'Kör en liten stabil batch när du är redo.')}</small></div><div class="foundationActions"><button id="runGlobalResolver" class="primary resolverActionButton" type="button" onclick="return window.mkRunResolverBatch(event)" ${running?'disabled':''}>${running?'Batch körs…':paused?'Fortsätt nästa batch (max 8)':'Kör nästa batch (max 8)'}</button><button id="stopGlobalResolver" class="danger" type="button" onclick="return window.mkStopResolver(event)" ${running?'':'disabled'}>Stoppa säkert</button><button id="reviewGlobalResolver" class="secondary" type="button" onclick="return window.mkReviewResolver(event)" ${(run.suggestions||[]).length?'':'disabled'}>Granska resultat</button></div><div class="resolverSources"><span>Lokal först</span><span>Permanent Registry</span><span>Global Registry</span><span>OpenFIGI · automatisk cooldown</span><span>Twelve Data</span><span>Alpha Vantage</span><span>Finnhub</span><span>Fond/ISIN & NAV</span></div><section class="resolverDiagnostics"><div class="sectionHead"><div><div class="eyebrow">Resolver State Recovery 11.15.43</div><h3>Enhetlig körkedja och full logg</h3><small><b id="resolverDiagnosticCount">${resolverDiagnosticState().lines?.length||0}</b> rader · <b id="resolverDiagnosticStorage">${esc(resolverTraceStorage)}</b></small></div><span class="healthScore">Puls <b id="resolverHeartbeat">${resolverDiagnosticState().heartbeat||0}</b></span></div><div class="diagMeta"><span>Instrument <b id="resolverCurrentInstrument">${esc(resolverDiagnosticState().currentInstrument||'–')}</b></span><span>Senaste steg <b id="resolverCurrentStep">${esc(resolverDiagnosticState().currentStep||'Redo')}</b></span></div><div id="resolverDiagnosticLog" class="diagnosticConsole ${resolverDiagnosticState().expanded?'expanded':''}">${(resolverDiagnosticState().lines||[]).map(diagnosticLineHtml).join('')||'<div class="empty">Ingen diagnostik ännu.</div>'}</div><div class="diagnosticActions"><button id="toggleResolverDiagnosticLog" class="secondary" type="button" onclick="return window.mkToggleResolverDiagnosticLog(event)">${resolverDiagnosticState().expanded?'Komprimera loggen':'Visa hela loggen'}</button><button class="secondary" type="button" onclick="return window.mkCopyResolverDiagnostics(event)">Kopiera hela loggen</button><button class="secondary" type="button" onclick="return window.mkClearResolverDiagnosticLog(event)">Rensa loggen</button></div></section><p class="disclaimer">Körstatus, checkpoint, granskningsresultat och full logg speglas beständigt i IndexedDB. De återställs efter appbyte, omstart och versionsuppdatering. Diagnostiken ändrar aldrig antal, GAV, kredit, transaktioner eller Portfolio Ledger. Loggen visar exakt om körningen stannar före Promise-callback, i workerfunktionen eller i instrumentresolving.</p></section>`}
...
async function boot(){
 ...
 await initializePermanentRegistryStorage();
 await hydrateResolverPersistentState();
 ...
 document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')void flushResolverIdbState()});
 window.addEventListener('pagehide',()=>{void flushResolverIdbState()});
...
if(!window.__mkResolverInteractionObserver){
 window.__mkResolverInteractionObserver=new MutationObserver(()=>bindResolverInteractionHandlers());
 window.__mkResolverInteractionObserver.observe(document.documentElement,{childList:true,subtree:true});
 setTimeout(bindResolverInteractionHandlers,0)
}

boot().catch(error=>{console.error(error);const app=document.getElementById('app');if(app)app.innerHTML='<div class="fatal"><b>Appen kunde inte starta.</b><p>Lokal data har inte raderats. Stäng appen och öppna den igen.</p></div>';document.body.classList.add('appReady')})
})();
