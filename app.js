...
const RESOLVER_RUN_KEY='mastarklass_os_11_15_15_resolver_run';
const RESOLVER_DIAG_KEY='mastarklass_os_11_15_15_resolver_diag';
const RESOLVER_DIAG_LINES_KEY='mastarklass_os_11_15_15_resolver_diag_lines';
...
function permanentIdentityRegistry(){return merge({version:'11.15.15',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.15';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.15',source:'permanent-identity-registry-11.15.15',updatedAt:new Date().toISOString()};restored++}
 }
...
function resolverDiagnosticLines(){
 try{const parsed=parse(safeGet(RESOLVER_DIAG_LINES_KEY));return Array.isArray(parsed)?parsed.slice(-300):[]}catch{return[]}
}
function resolverDiagnosticState(){
 let raw={};try{raw=parse(safeGet(RESOLVER_DIAG_KEY))||{}}catch{}
 const meta=merge({version:'11.15.15',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false},raw);
 meta.lines=resolverDiagnosticLines();return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.15',startedAt:x?.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x?.heartbeat),currentInstrument:x?.currentInstrument||'',currentStep:x?.currentStep||'',expanded:!!x?.expanded};
 try{safeSet(RESOLVER_DIAG_KEY,JSON.stringify(meta))}catch{}return meta
}
function persistResolverDiagnosticLines(lines){const clean=(Array.isArray(lines)?lines:[]).slice(-300);try{safeSet(RESOLVER_DIAG_LINES_KEY,JSON.stringify(clean))}catch{}return clean}
function resolverDiag(step,detail='',level='info'){
 try{
  const now=new Date(),lines=resolverDiagnosticLines();
  ...
  const d=resolverDiagnosticState();d.currentStep=line.step;d.updatedAt=now.toISOString();persistResolverDiagnosticMeta(d);d.lines=saved;updateResolverDiagnosticUI(d);
  try{console.log('[Resolver diagnostics]',line.seq,line.step,line.detail)}catch{}return line
 }catch(error){try{console.error('[Resolver diagnostic failed]',error)}catch{}return null}
}
...
function armResolverBootstrapWatchdogs(runId){
 ...
 [[250,'Bootstrap watchdog 250 ms'],[2000,'Bootstrap watchdog 2 s'],[8000,'Bootstrap watchdog 8 s']].forEach(([ms,label])=>{const id=setTimeout(()=>{try{const run=resolverRunState(),d=resolverDiagnosticState();resolverBootstrapDiag(label,`run=${run.status} ${run.processed}/${run.total} · steg=${d.currentStep||'–'} · id=${run.runId===runId?'match':'avvikelse'}`,ms>=8000?'warning':'info')}catch(error){resolverBootstrapDiag(label,String(error?.message||error),'error')}},ms);resolverBootstrapWatchdogs.push(id)})
}
...
function clearResolverDiagnosticLog(){persistResolverDiagnosticLines([]);const d=resolverDiagnosticState();d.heartbeat=0;d.currentInstrument='';d.currentStep='Logg rensad';persistResolverDiagnosticMeta(d);updateResolverDiagnosticUI({...d,lines:[]});return false}
...
function resolverRunState(){return merge({version:'11.15.15',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],message:''},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.15';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.15',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){const state=globalResolverState(),run=resolverRunState(),row=rowOverride||(run.suggestions||[]).find(x=>x.key===key)||(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.15'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.15'],source:'permanent-identity-registry-11.15.15',resolverVersion:'11.15.15',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.8',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
