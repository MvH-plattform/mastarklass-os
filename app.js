...
function permanentIdentityRegistry(){return merge({version:'11.15.17',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.17';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function resolverDiagnosticState(){
 ...
 const meta=merge({version:'11.15.17',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false,storage:resolverTraceStorage},raw);
 meta.lines=resolverDiagnosticLines();meta.storage=resolverTraceStorage;return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.17',startedAt:x?.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x?.heartbeat),currentInstrument:x?.currentInstrument||'',currentStep:x?.currentStep||'',expanded:!!x?.expanded,storage:resolverTraceStorage};
...
function resolverRunDefaults(){return {version:'11.15.17',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],message:''}}
function resolverRunState(){
 ...
 const run=merge(resolverRunDefaults(),raw||{});
 resolverRunMemory=run;
 return run
}
function persistResolverRun(x){
 const run=merge(resolverRunDefaults(),x||{});run.version='11.15.17';run.updatedAt=new Date().toISOString();
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.17',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
