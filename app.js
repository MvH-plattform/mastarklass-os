...
function permanentIdentityRegistry(){return merge({version:'11.15.18',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.18';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function savePermanentIdentityRoute(key,route,meta={}){
 ...
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.2',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.2',source:'permanent-identity-registry-11.15.18',updatedAt:new Date().toISOString()};restored++}
 }
 ...
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.18',`${restored} rutter återställda · ${migrated} migrerade`,{})}
...
function resolverRunState(){return merge({version:'11.15.18',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,fundPending:0,errors:0,stopRequested:false,targetKeys:[],suggestions:[]},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.18';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function refreshValuationAfterResolver(){try{liveValuationSnapshot({persist:true});runDataConfidenceEngine({reason:'resolver-11.15.18'});runAutonomousPortfolioScan({reason:'resolver-11.15.18',force:true})}catch(error){console.warn('Efteranalys kunde inte köras',error)}}
...
function duplicateFundIdentityCandidate(item){
 const wanted=normalizeInstrumentName(item.name||'');if(!wanted)return null;
 for(const h of canonicalHoldings()){
  if(h._key===item.key||classifyInstrument(h).code!=='fund'||normalizeInstrumentName(h._name)!==wanted)continue;
  const m=liveFoundation.mappings[h._key]||{},route=permanentRouteFor(h._key)||m.permanentRoute||m;
  if(validIsin(route.isin))return {ticker:'',isin:String(route.isin).toUpperCase(),name:h._name,exchange:'FUND',currency:route.currency||item.currency||'SEK',type:'Mutual Fund',provider:'Fund Identity Registry',providerSymbols:route.providerSymbols||{}};
 }
 return null
}
function markFundIdentityPending(item){
 const current=liveFoundation.mappings[item.key]||{};
 liveFoundation.mappings[item.key]={...current,instrumentClass:'fund',pricingModel:'nav',identityStatus:'fund-identity-required',identityConfidence:num(current.identityConfidence||current.confidence),fundIdentityRequired:true,fundIdentityReason:'ISIN saknas för fond. Aktieproviders har hoppats över.',resolverVersion:'11.15.18',updatedAt:new Date().toISOString()};
 persistLiveFoundation()
}
async function resolveOneInstrument(item){
 ...
 const instrumentClass=classifyInstrument(canonicalHoldings().find(h=>h._key===item.key)||item).code,isFund=instrumentClass==='fund';
 let candidates=[...localResolverCandidates(item)],errors=[],routeType=isFund?'fund':'exchange';
 const duplicate=isFund?duplicateFundIdentityCandidate(item):null;if(duplicate)candidates.push(duplicate);
 const jobs=[];
 if(isFund){
  if(validIsin(item.isin))jobs.push(searchOpenFigiPost(item));
 }else{
  if(providerConfig().twelveDataKey)jobs.push(searchTwelveDataResolver(item));
  if(providerConfig().alphaVantageKey)jobs.push(searchAlphaVantageResolver(item));
  if(providerConfig().finnhubKey)jobs.push(searchFinnhubInstrument(item));
  jobs.push(searchOpenFigiPost(item));
 }
 ...
 const needsFundIdentity=Boolean(isFund&&!best&&!validIsin(item.isin));if(needsFundIdentity)markFundIdentityPending(item);
 return {key:item.key,name:item.name,account:item.account,assetType:item.assetType,currency:item.currency,best,candidates:unique.slice(0,8),confidence:best?.score||0,margin,conflict,autoSafe,variants:isFund?[]:tickerVariants(item),exchanges:isFund?['FUND/NAV']:exchangeVariants(item),errors,routeType,needsFundIdentity,statusLabel:needsFundIdentity?'Behöver fondidentitet':''}
}
async function runGlobalResolver(){
 ...
 if(!resume){run={version:'11.15.18',status:'running',runId:crypto.randomUUID?.()||String(Date.now()),startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),completedAt:'',cursor:0,total:targets.length,processed:0,skipped:Object.keys(permanentIdentityRegistry().routes||{}).length,permanent:Object.keys(permanentIdentityRegistry().routes||{}).length,newSafe:0,review:0,failed:0,fundPending:0,errors:0,stopRequested:false,targetKeys:targets.map(x=>x.key),suggestions:[],message:'Resolvern startar…'};persistResolverRun(run)}
 ...
 for(let i=run.cursor;i<targets.length;i++){
  ...
  try{
   ...
   else if(row.best)run.review++;else if(row.needsFundIdentity)run.fundPending++;else run.failed++;
 ...
 run=resolverRunState();run.status='complete';run.completedAt=new Date().toISOString();run.permanent=Object.keys(permanentIdentityRegistry().routes||{}).length;run.message=`Klar: ${run.newSafe} nya säkra · ${run.review} för granskning · ${run.fundPending||0} behöver fondidentitet · ${run.failed} utan träff.`;persistResolverRun(run);
 const previous=globalResolverState(),suggestions=run.suggestions||[];persistGlobalResolver({...previous,version:'11.15.18',status:run.failed?'partial':'success',lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,fundPending:run.fundPending||0,errors:run.errors,suggestions,audit:[{date:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,fundPending:run.fundPending||0,errors:run.errors},...(previous.audit||[])].slice(0,100)});
 refreshValuationAfterResolver();liveEvent(run.failed?'warning':'success','Global Identity Resolver 11.15.18',run.message,{skipped:run.skipped,permanent:run.permanent,errors:run.errors});render();if(suggestions.length)openGlobalResolverReview()
}
function applyResolverSuggestion(key,index=0,{silent=false,rowOverride=null}={}){const state=globalResolverState(),row=rowOverride||(state.suggestions||[]).find(x=>x.key===key),candidate=row?.candidates?.[index];if(!row||!candidate)return false;const current=liveFoundation.mappings[key]||{};if(isManualMapping(current)&&!silent&&!confirm('Detta instrument har en manuell koppling. Ersätta den med valt resolverförslag?'))return false;const route={provider:candidate.provider,ticker:candidate.ticker||'',isin:candidate.isin||'',exchange:candidate.exchange||'',currency:candidate.currency||row.currency||'SEK',figi:candidate.figi||'',confidence:candidate.score,verifiedAt:new Date().toISOString(),source:'global-resolver-11.15.18'};if(!routeIsUsable(route))return false;liveFoundation.mappings[key]={...current,...route,providerSymbols:{...(current.providerSymbols||{}),...(candidate.providerSymbols||{}),[String(candidate.provider||'provider').toLowerCase().replace(/\s+/g,'_')]:candidate.ticker||candidate.isin},instrumentClass:classifyInstrument(canonicalHoldings().find(h=>h._key===key)||row).code,verified:true,identityStatus:'verified',identityConfidence:candidate.score,identityReasons:[candidate.provider,'Verifierad i Global Identity Resolver 11.15.18'],source:'permanent-identity-registry-11.15.18',resolverVersion:'11.15.2',permanentRoute:route,updatedAt:new Date().toISOString()};persistLiveFoundation();savePermanentIdentityRoute(key,route,{source:'global-resolver-approved'});state.routes=state.routes||{};state.routes[key]=route;row.applied=true;row.appliedAt=new Date().toISOString();row.appliedCandidate=index;persistGlobalResolver(state);liveEvent('success','Permanent Identity Registry 11.15.18',`${row.name} låstes permanent till ${candidate.ticker||candidate.isin}`,route);return true}
...
function globalResolverPanel(){const st=globalResolverState(),run=recoverResolverRun(),coverage=instrumentMappingCoverage(),remaining=resolverTargets().length,saved=Object.keys(permanentIdentityRegistry().routes||{}).length,running=run.status==='running',paused=run.status==='paused';return `<section class="section globalResolverPanel"><div class="sectionHead"><div><div class="eyebrow">Global Identity Resolver 11.15.18</div><h2>Snabbare, återupptagbar och permanent.</h2></div><span class="healthScore">${saved} låsta</span></div><p>Fonder och börshandlade instrument får nu olika resolvervägar. Vanliga fonder utan ISIN skickas inte längre till aktieproviders, utan markeras tydligt för fondidentitet och NAV.</p><div class="grid"><div class="card metric"><span>Återstår</span><b>${remaining}</b></div><div class="card metric"><span>Permanent sparade</span><b>${saved}</b></div><div class="card metric"><span>Nya säkra</span><b>${run.newSafe||0}</b></div><div class="card metric"><span>För granskning</span><b>${run.review||st.review||0}</b></div><div class="card metric"><span>Överhoppade</span><b>${run.skipped||0}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Misslyckade</span><b>${run.failed||0}</b></div></div>${running||paused?`<div class="resolverProgress"><div><span>${paused?'Pausad':'Kör'}</span><b>${run.processed||0}/${run.total||0}</b></div><div class="bar"><i style="width:${run.total?Math.round((run.processed||0)/run.total*100):0}%"></i></div><small>${esc(run.message||'')}</small></div>`:''}<div class="foundationActions"><button id="runGlobalResolver" class="primary" type="button" ${running?'disabled':''}>${paused?'Fortsätt Global Identity Resolver 11.15.18':'Kör Global Identity Resolver 11.15.18'}</button><button id="stopGlobalResolver" class="danger" type="button" ${running?'':'disabled'}>Stoppa säkert</button><button id="reviewGlobalResolver" class="secondary" type="button" ${(st.suggestions||[]).length||(run.suggestions||[]).length?'':'disabled'}>Granska och godkänn</button></div><div class="resolverSources"><span>Permanent Registry</span><span>Global Registry</span><span>OpenFIGI</span><span>Twelve Data</span><span>Alpha Vantage</span><span>Finnhub</span><span>Fond/ISIN & NAV</span></div><p class="disclaimer">Identitets- och providerdata sparas i read-only live-lagret. Antal, GAV, kredit, transaktioner och Portfolio Ledger ändras aldrig.</p></section>`}
