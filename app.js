...
function mappingEditor(){
 const c=instrumentMappingCoverage();
 const rows=c.items.map(x=>`<div class="mappingRow" data-map-key="${esc(x.key)}"><div><b>${esc(x.name)}</b><small>${esc(x.account)} · ${esc(x.assetType)}</small></div><label>Ticker<input data-map-ticker value="${esc(x.ticker||'')}" placeholder="Ex. INVE-B.ST"></label><label>ISIN<input data-map-isin value="${esc(x.isin||'')}" placeholder="SE0000107419"></label><label>Valuta<input data-map-currency value="${esc(x.currency||'SEK')}" maxlength="3"></label></div>`).join('');
 return `<div class="eyebrow">Live Mapping Center 11.1.1</div><h2>Instrumentmappning</h2><p class="disclaimer">Ticker och ISIN sparas endast i live-lagret. Antal, GAV, kredit, transaktioner och Ledger ändras aldrig.</p><div class="mappingSummary"><b>${c.complete}/${c.total} mappade</b><span>${c.missing.length} behöver kompletteras</span></div><div class="mappingList">${rows}</div><button id="saveMappings" class="primary stickyAction">Spara instrumentmappning</button>`;
}
...
function mappingSuggestionFor(item){const n=String(item.name||'').toLowerCase(),known=[
 [/alibaba/,{ticker:'BABA',currency:'USD',exchange:'NYSE'}],[/amazon/,{ticker:'AMZN',currency:'USD',exchange:'NASDAQ'}],[/microsoft/,{ticker:'MSFT',currency:'USD',exchange:'NASDAQ'}],[/berkshire.*b/,{ticker:'BRK.B',currency:'USD',exchange:'NYSE'}],[/realty income/,{ticker:'O',currency:'USD',exchange:'NYSE'}],[/investor.*b/,{ticker:'INVE-B',currency:'SEK',exchange:'STO'}],[/xact.*norden.*högutdel/,{ticker:'XACTHDIV',currency:'SEK',exchange:'STO'}],[/essity.*b/,{ticker:'ESSITY-B',currency:'SEK',exchange:'STO'}],[/evolution/,{ticker:'EVO',currency:'SEK',exchange:'STO'}],[/novo nordisk/,{ticker:'NOVO-B',currency:'DKK',exchange:'CPH'}]
 ];const match=known.find(([rx])=>rx.test(n));return match?match[1]:null}
function applySafeMappingSuggestions(){const c=instrumentMappingCoverage();let suggested=0;c.items.forEach(item=>{const current=liveFoundation.mappings[item.key]||{};if(current.ticker||current.isin)return;const suggestion=mappingSuggestionFor(item);if(!suggestion)return;liveFoundation.mappings[item.key]={...current,...suggestion,name:item.name,account:item.account,assetType:item.assetType,updatedAt:new Date().toISOString(),source:'safe-suggestion',suggested:true};suggested++});if(suggested){liveFoundation.updatedAt=new Date().toISOString();persistLiveFoundation();liveEvent('success','Smart Mapping',`${suggested} försiktiga tickerförslag lades till`,{suggested})}return suggested}
...
function market(){
 const registry=providerRegistry(),providerHealth=providerHealthSummary(),cache=cacheHealth(),sync=liveSyncState(),foundation=foundationHealth(),mapping=instrumentMappingCoverage();
