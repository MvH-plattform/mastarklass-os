...
const RESOLVER_RUN_KEY='mastarklass_os_11_15_13_resolver_run';
const RESOLVER_DIAG_KEY='mastarklass_os_11_15_13_resolver_diag';
const RESOLVER_DIAG_LINES_KEY='mastarklass_os_11_15_13_resolver_diag_lines';
...
function permanentIdentityRegistry(){return merge({version:'11.15.13',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
...
function resolverDiagnosticState(){
 const meta=merge({version:'11.15.13',startedAt:'',updatedAt:'',heartbeat:0,currentInstrument:'',currentStep:'',expanded:false},parse(safeGet(RESOLVER_DIAG_KEY))||{});
 meta.lines=resolverDiagnosticLines();
 return meta
}
function persistResolverDiagnosticMeta(x){
 const meta={version:'11.15.13',startedAt:x.startedAt||'',updatedAt:new Date().toISOString(),heartbeat:num(x.heartbeat),currentInstrument:x.currentInstrument||'',currentStep:x.currentStep||'',expanded:!!x.expanded};
...
function resetResolverDiagnostics(){
 const d={version:'11.15.13',startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),heartbeat:0,currentInstrument:'',currentStep:'Start',expanded:false};
...
function resolverRunState(){return merge({version:'11.15.13',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],targetItems:[],suggestions:[],message:''},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.13';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.13',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
