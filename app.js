...
function resolverInteractionState(){return merge({version:'11.15.41',lastSync:'',lastConflictOpen:'',syncStatus:'idle',conflictStatus:'idle',lastError:''},parse(safeGet(RESOLVER_INTERACTION_KEY))||{})}
function persistResolverInteraction(x){x.version='11.15.41';safeSet(RESOLVER_INTERACTION_KEY,JSON.stringify(x));return x}
...
async function runPortfolioBridgeWithFeedback(event){
 ...
 resolverDiag('Portfolio Identity Bridge 11.15.41 knapptryck','Synk-knapp mottagen','info');
 ...
 try{
  ...
  resolverDiag('Portfolio Identity Bridge 11.15.41 klar',`${r.withIsin}/${r.total} med ISIN · ${r.withoutIsin} saknar · ${r.conflicts.length} konflikter`,'success');
  ...
  resolverDiag('Portfolio Identity Bridge 11.15.41 fel',message,'error');
...
function openConflictResolverWithFeedback(event){
 event?.preventDefault?.();event?.stopPropagation?.();
 resolverDiag('Conflict Resolver 11.15.41 knapptryck','Konfliktknapp mottagen','info');
 ...
 try{
  ...
  resolverDiag('Conflict Resolver 11.15.41 modal öppnad',`${report.withIsin}/${report.total} med ISIN · ${report.withoutIsin} saknar · ${report.conflicts.length} konflikter`,'success');
  ...
  resolverDiag('Conflict Resolver 11.15.41 fel',message,'error');
...
function renderIdentityConflictDetails(){
 ...
 openModal(`<div class="sectionHead"><div><h2>Unified Identity Engine 11.15.41</h2><p>${report.withIsin}/${report.total} med ISIN · ${report.withoutIsin} saknar · ${conflicts.length} konflikter</p></div></div><h3>ISIN-konflikter</h3>${conflictHtml}<h3 class="topGap">Saknar ISIN</h3><div class="missingIsinList">${missingHtml||'<div class="notice okbox">Alla innehav har ISIN.</div>'}</div>`);
 return report
}
...
function resolverReviewHtml(){
 ...
 return `<div class="eyebrow">Global Identity Resolver 11.15.41</div><h2>ISIN-first Candidate Inspector</h2><p class="disclaimer">ISIN är primär identitetsnyckel. Ticker, börs, valuta och instrumenttyp används som sekundära kontroller. Kandidater utan ISIN kan granskas men räknas inte som verifierade identiteter.</p><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`
}
