...
const LIVE_FOUNDATION_DEFAULT={
 ...
 registry:[
  ...
  {id:'market-primary',name:'Stooq Price Adapter',category:'prices',role:'primary',priority:90,quality:82,status:'ready',connector:'stooq',coverage:'Aktier och ETF:er med mappad ticker'},
  {id:'market-fallback',name:'Local Quote Fallback',category:'prices',role:'fallback',priority:70,quality:70,status:'standby',connector:'local-cache',coverage:'Senast validerade lokala kurs'},
...
async function fetchEcbFx(){const currencies=['USD','GBP','NOK','DKK','CAD','JPY','CHF'];const rates={SEK:1};let count=0;for(const cur of currencies){try{const url=`https://data-api.ecb.europa.eu/service/data/EXR/D.${cur}+SEK.EUR.SP00.A?startPeriod=${new Date(Date.now()-7*864e5).toISOString().slice(0,10)}&format=csvdata`;const res=await fetch(url,{headers:{Accept:'text/csv'}});if(!res.ok)throw new Error(`HTTP ${res.status}`);const txt=await res.text(),lines=txt.trim().split(/\r?\n/),head=lines[0].split(',');const ci=head.indexOf('CURRENCY'),vi=head.indexOf('OBS_VALUE'),ti=head.indexOf('TIME_PERIOD');const latest={};for(const line of lines.slice(1)){const cols=line.match(/("[^"]*"|[^,]+)/g)||[];const c=(cols[ci]||'').replaceAll('"',''),v=num((cols[vi]||'').replaceAll('"','')),t=(cols[ti]||'').replaceAll('"','');if(c&&v&&(!latest[c]||t>latest[c].t))latest[c]={v,t}}const eurSek=latest.SEK?.v,eurCur=latest[cur]?.v;if(eurSek&&eurCur){rates[cur]=eurSek/eurCur;count++}}catch(e){liveEvent('warning','ECB FX',`${cur}: ${e.message}`)}}return {rates,count}}
function stooqSymbol(raw){const x=String(raw||'').trim().toLowerCase();if(!x)return '';if(x.includes('.'))return x;return x+'.us'}
async function fetchStooqQuotes(){const mappings=instrumentMappingCoverage().items.filter(x=>x.ticker);const quotes=[];let errors=0;for(const m of mappings.slice(0,120)){try{const sym=stooqSymbol(m.ticker),url=`https://stooq.com/q/l/?s=${encodeURIComponent(sym)}&f=sd2t2ohlcv&h&e=csv`;const res=await fetch(url);if(!res.ok)throw new Error(`HTTP ${res.status}`);const txt=await res.text(),lines=txt.trim().split(/\r?\n/);if(lines.length<2)throw new Error('ingen kurs');const cols=lines[1].split(','),close=num(cols[6]),open=num(cols[3]);if(!close)throw new Error('ogiltig kurs');quotes.push({symbol:String(m.ticker).toUpperCase(),providerSymbol:sym,price:close,open,change:open?close-open:0,changePct:open?(close-open)/open*100:0,currency:m.currency||'SEK',timestamp:new Date().toISOString(),provider:'Stooq',quality:82})}catch(e){errors++}}return {quotes,errors}}
async function syncLiveData(){const button=document.getElementById('syncLive');if(button){button.disabled=true;button.textContent='Synkroniserar…'}const sync={...liveSyncState(),lastAttempt:new Date().toISOString(),status:'syncing',message:'Hämtar FX och priser',errors:[]};persistLiveSync(sync);try{buildInstrumentMappings();const [fxr,qr]=await Promise.all([fetchEcbFx(),fetchStooqQuotes()]);const old=liveState(),bySymbol=new Map((old.quotes||[]).map(q=>[String(q.symbol||q.ticker||'').toUpperCase(),q]));qr.quotes.forEach(q=>bySymbol.set(String(q.symbol).toUpperCase(),q));const next={...old,providers:liveFoundation.registry,fx:{...(old.fx||{}),...fxr.rates},quotes:[...bySymbol.values()],lastSync:new Date().toISOString(),updatedAt:new Date().toISOString(),foundationVersion:VERSION};safeSet(LIVE_KEY,JSON.stringify(next));validateLiveCache();Object.assign(sync,{lastSuccess:new Date().toISOString(),status:'success',message:`${qr.quotes.length} kurser och ${fxr.count} valutakurser uppdaterade`,fxUpdated:fxr.count,quotesUpdated:qr.quotes.length,errors:qr.errors?[`${qr.errors} kursförfrågningar saknade svar`]:[]});persistLiveSync(sync);liveEvent('success','Live Prices & FX',sync.message,{quoteErrors:qr.errors});render()}catch(e){Object.assign(sync,{status:'error',message:e.message,errors:[e.message]});persistLiveSync(sync);liveEvent('error','Live Prices & FX',`Synkronisering misslyckades: ${e.message}`);render()}}
...
function market(){
 ...
 const ls=liveSyncState(),ps=livePortfolioStats();
 return `<section class="hero liveHero"><div class="eyebrow">Mästarklass OS 11.1.1 · Live Prices & FX</div><h2>Kurser och valutor som får portföljen att leva.</h2><p>Live Price Engine och FX Engine arbetar i ett separat read-only-lager. Provider Registry, Data Router, validering och cache skyddar Portfolio Ledger. Ingen extern data får skriva över antal, GAV, kredit eller transaktioner.</p><div class="liveHeroStats"><div><span>Foundation Health</span><b>${fh.score}/100</b></div><div><span>Instrumenttäckning</span><b>${fh.coverage} %</b></div><div><span>Providers redo</span><b>${fh.ready}/${fh.registry.length}</b></div><div class="liveState ${r.code}"><span>Anslutningsstatus</span><b><i></i>${r.status}</b></div></div></section><section class="section liveSyncPanel"><div class="sectionHead"><div><div class="eyebrow">Live Prices & FX 11.1.1</div><h2>Synkronisering</h2></div><span class="syncStatus ${ls.status}">${esc(ls.status)}</span></div><button id="syncLive" class="primary">Hämta livekurser och FX</button><div class="grid topGap"><div class="card metric"><span>Livevärde</span><b>${kr(ps.liveValue)}</b></div><div class="card metric"><span>Idag</span><b class="${ps.daySEK>=0?'positive':'negative'}">${ps.daySEK>=0?'+':''}${kr(ps.daySEK)} · ${ps.dayPct>=0?'+':''}${pct(ps.dayPct)}</b></div><div class="card metric"><span>Kurstäckning</span><b>${ps.covered}/${ps.total}</b></div><div class="card metric"><span>Senast lyckad</span><b>${ls.lastSuccess?new Date(ls.lastSuccess).toLocaleString('sv-SE'):'—'}</b></div></div><div class="notice ${ls.status==='success'?'okbox':''}">${esc(ls.message)}</div><p class="disclaimer">Externa observationer ändrar aldrig antal, GAV, kredit, transaktioner eller Ledger.</p></section>
 <section class="section"><div class="sectionHead"><div><div class="eyebrow">Live Core</div><h2>Systemkontroller</h2></div><small>Read-only</small></div><div class="foundationActions"><button id="buildMappings" class="primary">Skapa/uppdatera instrumentmappning</button><button id="reviewMappings" class="secondary">Granska ticker och ISIN</button><button id="validateLive" class="secondary">Kör validering av live-cache</button></div><div class="grid topGap"><div class="card metric"><span>Mappade instrument</span><b>${mapping.complete}/${mapping.total}</b></div><div class="card metric"><span>Saknar identitet</span><b>${mapping.missing.length}</b></div><div class="card metric"><span>Cacheposter</span><b>${fh.cache.items||0}</b></div><div class="card metric"><span>Valideringsvarningar</span><b>${fh.validation.warnings||0}</b></div></div>${mapping.missing.length?`<div class="warning"><b>Instrument som behöver ticker eller ISIN</b><p>${mapping.missing.slice(0,18).map(x=>esc(x.name)).join(', ')}${mapping.missing.length>18?' …':''}</p></div>`:'<div class="notice okbox">Alla innehav har grundläggande instrumentidentitet.</div>'}</section>
