...
const RESOLVER_RUN_KEY='mastarklass_os_11_15_7_resolver_run';
...
function permanentIdentityRegistry(){return merge({version:'11.15.7',updatedAt:'',routes:{},audit:[]},parse(safeGet(PERMANENT_IDENTITY_KEY))||{})}
function persistPermanentIdentityRegistry(x){x.version='11.15.7';x.updatedAt=new Date().toISOString();safeSet(PERMANENT_IDENTITY_KEY,JSON.stringify(x))}
...
function savePermanentIdentityRoute(key,route,meta={}){
 ...
 registry.routes[key]={...previous,...route,locked:true,registryVersion:'11.15.7',savedAt:new Date().toISOString(),source:meta.source||route.source||'global-resolver'};
...
function hydratePermanentIdentityRoutes(){
 ...
 for(const [key,route] of Object.entries(registry.routes||{})){
  ...
  if(!current.permanentRoute||!current.verified){liveFoundation.mappings[key]={...current,...route,permanentRoute:route,verified:true,identityStatus:'verified',identityConfidence:num(route.confidence)||96,resolverVersion:'11.15.7',source:'permanent-identity-registry-11.15.7',updatedAt:new Date().toISOString()};restored++}
 }
 ...
 if(restored||migrated){persistLiveFoundation();liveEvent('success','Permanent Identity Registry 11.15.7',`${restored} rutter återställda · ${migrated} migrerade`,{})}
...
function resolverRunState(){return merge({version:'11.15.7',status:'idle',runId:'',startedAt:'',updatedAt:'',completedAt:'',cursor:0,total:0,processed:0,skipped:0,permanent:0,newSafe:0,review:0,failed:0,errors:0,stopRequested:false,targetKeys:[],suggestions:[],message:''},parse(safeGet(RESOLVER_RUN_KEY))||{})}
function persistResolverRun(x){x.version='11.15.7';x.updatedAt=new Date().toISOString();safeSet(RESOLVER_RUN_KEY,JSON.stringify(x));return x}
...
function syncResolverSummary(run){const previous=globalResolverState(),suggestions=run.suggestions||[];const state={...previous,version:'11.15.7',status:run.status==='complete'?(run.failed?'partial':'success'):run.status,lastRun:new Date().toISOString(),scanned:run.processed,resolved:run.newSafe,review:run.review,unresolved:run.failed,errors:run.errors,suggestions,routes:previous.routes||{},audit:previous.audit||[]};persistGlobalResolver(state);return state}
...
window.mkRunResolverBatch=function(event){
 ...
 const now=Date.now();
 if(window.__mkResolverTapAt&&now-window.__mkResolverTapAt<700)return false;
 window.__mkResolverTapAt=now;
...
async function startResolverBatchFromUI(){
 ...
 if(resolverWorkerActive){
  const busy=resolverRunState();
  busy.message=busy.message||'En resolverbatch arbetar redan.';
  updateResolverProgressUI(busy);
  return false
 }
 if(button){
  button.disabled=true;
  button.textContent='Knapptryck mottaget…';
  button.setAttribute('aria-busy','true');
 }
 let armed=resolverRunState();
 armed.status='preparing';
 armed.message='Knapptryck mottaget. Förbereder högst 8 instrument…';
 armed.stopRequested=false;
 persistResolverRun(armed);
 updateResolverProgressUI(armed);
 ...
 try{
  ...
  run.status='error';
  run.message=`Kunde inte starta batch: ${String(error?.message||error)}`;
  run.errors=num(run.errors)+1;
  persistResolverRun(run);
  syncResolverSummary(run);
  updateResolverProgressUI(run);
  const msg=document.getElementById('resolverMessage');if(msg)msg.textContent=run.message;
  return false
 }finally{
  const next=document.getElementById('runGlobalResolver');
  if(next){next.disabled=resolverRunState().status==='running';next.removeAttribute('aria-busy')}
 }
}
