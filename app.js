...
function providerTelemetry(){return merge({
 ...
 'stooq':{id:'stooq',name:'Stooq Prices',attempts:0,successes:0,failures:0,lastAttempt:'',lastSuccess:'',lastError:'',latencyMs:0,status:'idle'},
...
async function fetchStooqQuotes(){const mappings=instrumentMappingCoverage().items.filter(x=>x.ticker),quotes=[],failures=[];for(const m of mappings.slice(0,120)){let success=false,lastError='';for(const sym of stooqCandidates(m)){try{const url=`https://stooq.com/q/l/?s=${encodeURIComponent(sym)}&f=sd2t2ohlcv&h&e=csv`,res=await fetchWithPolicy(url,{provider:'stooq',timeoutMs:7000,retries:1}),txt=await res.text(),lines=txt.trim().split(/\r?\n/);if(lines.length<2)throw new Error('ingen kursrad');const cols=lines[1].split(','),close=num(cols[6]),open=num(cols[3]);if(!close)throw new Error('ogiltig kurs');quotes.push({symbol:String(m.ticker).toUpperCase(),providerSymbol:sym,price:close,open,change:open?close-open:0,changePct:open?(close-open)/open*100:0,currency:m.currency||'SEK',timestamp:new Date().toISOString(),provider:'Stooq',quality:82,exchange:m.exchange||''});success=true;break}catch(e){lastError=String(e?.message||e)}}if(!success)failures.push({name:m.name,ticker:m.ticker,error:lastError})}return {quotes,errors:failures.length,failures}}
async function syncLiveData(){const button=document.getElementById('syncLive');if(button){button.disabled=true;button.textContent='Synkroniserar…'}const sync={...liveSyncState(),lastAttempt:new Date().toISOString(),status:'syncing',message:'Router hämtar FX och priser',errors:[]};persistLiveSync(sync);render();try{buildInstrumentMappings();applySafeMappingSuggestions();const [fxr,qr]=await Promise.all([fetchEcbFx(),fetchStooqQuotes()]),old=liveState(),bySymbol=new Map((old.quotes||[]).map(q=>[String(q.symbol||q.ticker||'').toUpperCase(),q]));qr.quotes.forEach(q=>bySymbol.set(String(q.symbol).toUpperCase(),q));const cacheFallbacks=Math.max(0,(old.quotes||[]).length-qr.quotes.length);if(cacheFallbacks){const t=providerTelemetry()['local-cache']||{};updateProviderTelemetry('local-cache',{...t,attempts:num(t.attempts)+1,successes:num(t.successes)+1,lastAttempt:new Date().toISOString(),lastSuccess:new Date().toISOString(),status:'standby'})}const next={...old,providers:liveFoundation.registry,fx:{...(old.fx||{}),...fxr.rates},quotes:[...bySymbol.values()],lastSync:new Date().toISOString(),updatedAt:new Date().toISOString(),foundationVersion:VERSION};safeSet(LIVE_KEY,JSON.stringify(next));validateLiveCache();const parts=[`${qr.quotes.length} nya kurser`,`${fxr.count} FX-kurser`];if(cacheFallbacks)parts.push(`${cacheFallbacks} cache-fallbacks`);Object.assign(sync,{lastSuccess:new Date().toISOString(),status:(qr.quotes.length||fxr.count)?'success':'partial',message:parts.join(' · '),fxUpdated:fxr.count,quotesUpdated:qr.quotes.length,errors:qr.failures.slice(0,8).map(x=>`${x.name}: ${x.error}`)});persistLiveSync(sync);liveEvent(sync.status==='success'?'success':'warning','Multi-Provider Router',sync.message,{quoteErrors:qr.errors,fxErrors:fxr.errors,cacheFallbacks});render()}catch(e){Object.assign(sync,{status:'error',message:String(e?.message||e),errors:[String(e?.message||e)]});persistLiveSync(sync);liveEvent('error','Multi-Provider Router',`Synkronisering misslyckades: ${sync.message}`);render()}}
...
function market(){
 ...
 return `<section class="hero"><div class="eyebrow">Live Intelligence 11.1.3</div><h2>Marknad & live-data</h2><p>Read-only marknadsdata från flera utbytbara providers. Din lokala portföljdata ändras aldrig av live-lagret.</p><div class="liveHeroStats"><div><span>Synkstatus</span><b>${esc(sync.status)}</b></div><div><span>Senaste synk</span><b>${sync.lastSuccess?new Date(sync.lastSuccess).toLocaleString('sv-SE'):'Aldrig'}</b></div><div><span>Instrumentmappning</span><b>${mapping.complete}/${mapping.total}</b></div><div><span>Cache</span><b>${cache.items} kurser</b></div></div></section>
 <section class="section liveSyncPanel"><div class="sectionHead"><div><div class="eyebrow">Multi-Provider Router</div><h2>Synkronisering</h2></div><span class="syncStatus ${esc(sync.status)}">${esc(sync.status)}</span></div><p>${esc(sync.message)}</p><div class="foundationActions"><button id="syncLive" class="primary" type="button">Synkronisera live-data</button><button id="reviewMappings" class="secondary" type="button">Granska instrumentmappning</button><button id="suggestMappings" class="secondary" type="button">Lägg till säkra tickerförslag</button><button id="validateLive" class="secondary" type="button">Validera cache</button></div>${sync.errors?.length?`<div class="warning"><b>Senaste fel</b><p>${sync.errors.map(esc).join('<br>')}</p></div>`:''}</section>
