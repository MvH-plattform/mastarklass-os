...
function buildIdentityConflictReport(){
 const hs=canonicalHoldings(),byIsin=new Map(),missing=[];
 for(const h of hs){
  const id=mergedIdentityForHolding(h,h._key),isin=normalizeIsinValue(id.isin);
  if(isin){
   if(!byIsin.has(isin))byIsin.set(isin,[]);
   byIsin.get(isin).push({key:h._key,name:h._name,account:h._account||'',type:h._type||'',ticker:id.ticker||'',isin});
  }else{
   missing.push({key:h._key,name:h._name,account:h._account||'',type:h._type||'',ticker:id.ticker||''});
  }
 }
 const conflicts=[];
 for(const [isin,rows] of byIsin){
  const uniqueNames=new Set(rows.map(r=>normalizeInstrumentName(r.name)));
  if(rows.length>1&&uniqueNames.size>1)conflicts.push({isin,rows});
 }
 return persistIdentityConflictState({
  status:conflicts.length?'warning':'success',
  withIsin:hs.length-missing.length,
...
function renderIdentityConflictDetails(){
 ...
 openModal(`<div class="sectionHead"><div><h2>Identity Conflict Resolver 11.15.38</h2><p>${report.withIsin}/101 med ISIN · ${report.withoutIsin} saknar · ${conflicts.length} konflikter</p></div></div><h3>ISIN-konflikter</h3>${conflictHtml}<h3 class="topGap">Saknar ISIN</h3><div class="missingIsinList">${missingHtml||'<div class="notice okbox">Alla innehav har ISIN.</div>'}</div>`);
 return report
}
...
function refreshIdentityConflictCounters(){
 ...
 resolverDiag('Identity Conflict Resolver 11.15.38',`${r.withIsin} med ISIN · ${r.withoutIsin} saknar · ${r.conflicts.length} konflikter`,r.conflicts.length?'warning':'success');
 return r
}
...
function resolverReviewHtml(){
 ...
 return `<div class="eyebrow">Global Identity Resolver 11.15.38</div><h2>ISIN-first Candidate Inspector</h2><p class="disclaimer">ISIN är primär identitetsnyckel. Ticker, börs, valuta och instrumenttyp används som sekundära kontroller. Kandidater utan ISIN kan granskas men räknas inte som verifierade identiteter.</p><div class="resolverAccounting"><b>${run.total||0} mål</b><span>${run.processed||0} behandlade · ${Math.max(0,(run.total||0)-(run.processed||0))} återstår</span><small>${categorized} kategoriserade resultat · ${run.permanent||0} permanent sparade totalt</small></div><div class="grid"><div class="card metric"><span>Sökta</span><b>${run.processed}</b></div><div class="card metric"><span>Säkra</span><b>${run.newSafe}</b></div><div class="card metric"><span>Granskning</span><b>${run.review}</b></div><div class="card metric"><span>Behöver fondidentitet</span><b>${run.fundPending||0}</b></div><div class="card metric"><span>Utan träff</span><b>${run.failed}</b></div></div><div class="resolverList">${rows||'<div class="empty">Inga behandlade instrument i denna körning ännu.</div>'}</div>`
}
...
async function boot(){
 ...
 refreshIdentityConflictCounters();
